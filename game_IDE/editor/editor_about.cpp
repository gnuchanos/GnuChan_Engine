...
void EditorAbout::_notification(int p_what) {
	switch (p_what) {
		...
		case NOTIFICATION_THEME_CHANGED: {
			...
			// app_icon.png (icon.png) kullan, Godot SVG logosu yerine
			Ref<Image> logo_img = memnew(Image(app_icon_png));
			logo_img->resize(128 * EDSCALE, 128 * EDSCALE, Image::INTERPOLATE_LANCZOS);
			Ref<ImageTexture> logo_tex = memnew(ImageTexture);
			logo_tex->create_from_image(logo_img);
			_logo->set_texture(logo_tex);
		} break;
	}
}
...
EditorAbout::EditorAbout() {
	set_title(TTR("Thanks from the GnuChan community!"));
	set_hide_on_ok(true);
	set_resizable(true);
	...
	about_text->set_text(
			String::utf8("\xc2\xa9 2014-present ") + TTR("GnuChan Engine contributors") + "." +
	...
	tpl_label->set_text(TTR("GnuChan Engine relies on a number of third-party free and open source libraries, all compatible with the terms of its MIT license. The following is an exhaustive list of all such third-party components with their respective copyright statements and license terms."));
