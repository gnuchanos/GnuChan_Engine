...
void Main::print_help(const char *p_binary) {
    ...
    OS::get_singleton()->print("(c) 2014-present GnuChan Engine contributors.\n");
...
Error Main::setup(const char *execpath, int argc, char *argv[], bool p_second_phase) {
	...
	while (I) {
		...
		if (I->get() == "-h" || I->get() == "--help" || I->get() == "/?") { // display help
			...
			auto_quit = true;
		} else if (I->get().ends_with("project.gnuchan")) {
			String path;
			String file = I->get();
	...
	// 'project.gnuchan' file which will only be available through the network if this is enabled
	...
	ProjectSettings::get_singleton()->set_custom_property_info("rendering/quality/driver/driver_name", PropertyInfo(Variant::STRING, "rendering/quality/driver/driver_name", PROPERTY_HINT_ENUM, "GLES3"));
	if (video_driver == "") {
	...
	if (tablet_driver == "") { // specified in project.gnuchan
	...
	if (audio_driver == "") { // specified in project.gnuchan
...
Error Main::setup2(Thread::ID p_main_tid_override) {
	...
	GLOBAL_DEF("application/config/icon", "res://icon.png");
