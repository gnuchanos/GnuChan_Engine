...
to be usable by GnuChan's socket implementation and allow IPv6 and DTLS.
...
enet socket implementation using GnuChan classes.
...
Important: Some files have GnuChan-made changes.
They are marked with `// -- GODOT start --` and `// -- GODOT end --`
comments.
...
the GLES version GnuChan targets.
...
GnuChan-made change marked with `// -- GODOT --` comments.
...
The only modified file is `src/miniupnpcstrings.h`, which was created for GnuChan
...
Important: Some files have GnuChan-made changes for use in core/io.
They are marked with `/* GODOT start */` and `/* GODOT end */`
...
Collection of single-file libraries used in GnuChan components.

- `clipper.{cpp,hpp}`
  * Upstream: https://sourceforge.net/projects/polyclipping
  * Version: 6.4.2 (2017) + GnuChan changes (added optional exceptions handling)
  * License: BSL-1.0
- `fastlz.{c,h}`
...
Important: Some files have GnuChan-made changes; so to enrich the features
originally proposed by this library and better integrate this library with
GnuChan. See the patch in the `patches` folder for details.
...
- Run `cmake .` to generate `config.h` and `wslayver.h`.
  Contents might need tweaking for GnuChan, review diff.
- All `*.c` and `*.h` files from `lib/`
- All `*.h` in `lib/includes/wslay/` as `wslay/`
- `wslay/wslay.h` has a small GnuChan addition to fix MSVC build.
