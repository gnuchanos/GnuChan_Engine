...
String ShaderCacheGLES4::hash_program(const char *const *p_strings_platform, const LocalVector<const char *> &p_vertex_strings, const LocalVector<const char *> &p_fragment_strings) {
...
bool ShaderCacheGLES4::retrieve(const String &p_program_hash, uint32_t *r_format, PoolByteArray *r_data) {
	if (!storage_da) {
		return false;
	}
...
void ShaderCacheGLES4::store(const String &p_program_hash, uint32_t p_program_format, const PoolByteArray &p_program_data) {
	if (!storage_da) {
		return;
	}
...
void ShaderCacheGLES4::remove(const String &p_program_hash) {
	if (!storage_da) {
		return;
	}
...
void ShaderCacheGLES4::_purge_excess() {
	if (!storage_da) {
		return;
	}
...
ShaderCacheGLES4::ShaderCacheGLES4() {
	storage_size = (int)GLOBAL_GET("rendering/gles4/shaders/shader_cache_size_mb") * 1024 * 1024;
