...
#include "drivers/gles4/rasterizer_gles4.h"
...
void RasterizerCanvasGLES4::canvas_end() {
	batch_canvas_end();
	RasterizerCanvasBaseGLES4::canvas_end();
}

void RasterizerCanvasGLES4::canvas_begin() {
	batch_canvas_begin();
	RasterizerCanvasBaseGLES4::canvas_begin();
}

void RasterizerCanvasGLES4::canvas_render_items_begin(const Color &p_modulate, Light *p_light, const Transform2D &p_base_transform) {
	batch_canvas_render_items_begin(p_modulate, p_light, p_base_transform);
}

void RasterizerCanvasGLES4::canvas_render_items_end() {
	batch_canvas_render_items_end();
}

void RasterizerCanvasGLES4::canvas_render_items(Item *p_item_list, int p_z, const Color &p_modulate, Light *p_light, const Transform2D &p_base_transform) {
	batch_canvas_render_items(p_item_list, p_z, p_modulate, p_light, p_base_transform);
}

void RasterizerCanvasGLES4::gl_enable_scissor(int p_x, int p_y, int p_width, int p_height) const {
	glEnable(GL_SCISSOR_TEST);
	glScissor(p_x, p_y, p_width, p_height);
}

void RasterizerCanvasGLES4::gl_disable_scissor() const {
...
void RasterizerCanvasGLES4::_legacy_canvas_render_item(Item *p_ci, RenderItemState &r_ris) {
	storage->info.render._2d_item_count++;

	if (r_ris.prev_distance_field != p_ci->distance_field) {
		state.canvas_shader.set_conditional(CanvasShaderGLES4::USE_DISTANCE_FIELD, p_ci->distance_field);
...

	{
		...
		if (r_ris.prev_use_skeleton != use_skeleton) {
			r_ris.rebind_shader = true;
			state.canvas_shader.set_conditional(CanvasShaderGLES4::USE_SKELETON, use_skeleton);
	...
	if (material != r_ris.canvas_last_material || r_ris.rebind_shader) {
		RasterizerStorageGLES4::Material *material_ptr = storage->material_owner.getornull(material);
		RasterizerStorageGLES4::Shader *shader_ptr = nullptr;
		...
		if (shader_ptr) {
			if (shader_ptr->canvas_item.uses_screen_texture && !state.canvas_texscreen_used) {
				...
				r_ris.last_blend_mode = r_ris.last_blend_mode != RasterizerStorageGLES4::Shader::CanvasItem::BLEND_MODE_DISABLED ? r_ris.last_blend_mode : -1;
			}
			...
			for (int i = 0; i < tc; i++) {
				...
				RasterizerStorageGLES4::Texture *t = storage->texture_owner.getornull(textures[i]);
				if (!t) {
	...
	int blend_mode = r_ris.shader_cache ? r_ris.shader_cache->canvas_item.blend_mode : RasterizerStorageGLES4::Shader::CanvasItem::BLEND_MODE_MIX;
	if (blend_mode == RasterizerStorageGLES4::Shader::CanvasItem::BLEND_MODE_DISABLED && (!storage->frame.current_rt || !storage->frame.current_rt->flags[RasterizerStorage::RENDER_TARGET_TRANSPARENT])) {
		blend_mode = RasterizerStorageGLES4::Shader::CanvasItem::BLEND_MODE_MIX;
	}
	bool unshaded = r_ris.shader_cache && (r_ris.shader_cache->canvas_item.light_mode == RasterizerStorageGLES4::Shader::CanvasItem::LIGHT_MODE_UNSHADED || (blend_mode != RasterizerStorageGLES4::Shader::CanvasItem::BLEND_MODE_MIX && blend_mode != RasterizerStorageGLES4::Shader::CanvasItem::BLEND_MODE_PMALPHA));
	bool reclip = false;

	if (r_ris.last_blend_mode != blend_mode) {
		if (r_ris.last_blend_mode == RasterizerStorageGLES4::Shader::CanvasItem::BLEND_MODE_DISABLED) {
			// re-enable it
			glEnable(GL_BLEND);
		} else if (blend_mode == RasterizerStorageGLES4::Shader::CanvasItem::BLEND_MODE_DISABLED) {
			// disable it
			glDisable(GL_BLEND);
		}

		switch (blend_mode) {
			case RasterizerStorageGLES4::Shader::CanvasItem::BLEND_MODE_DISABLED: {
				// nothing to do here

			} break;
			case RasterizerStorageGLES4::Shader::CanvasItem::BLEND_MODE_MIX: {
			...
			case RasterizerStorageGLES4::Shader::CanvasItem::BLEND_MODE_ADD: {
				glBlendEquation(GL_FUNC_ADD);
				if (storage->frame.current_rt && storage->frame.current_rt->flags[RasterizerStorage::RENDER_TARGET_TRANSPARENT]) {
			...
			case RasterizerStorageGLES4::Shader::CanvasItem::BLEND_MODE_SUB: {
			...
			case RasterizerStorageGLES4::Shader::CanvasItem::BLEND_MODE_MUL: {
				glBlendEquation(GL_FUNC_ADD);
				if (storage->frame.current_rt && storage->frame.current_rt->flags[RasterizerStorage::RENDER_TARGET_TRANSPARENT]) {
			...
			case RasterizerStorageGLES4::Shader::CanvasItem::BLEND_MODE_PMALPHA: {
				glBlendEquation(GL_FUNC_ADD);
				if (storage->frame.current_rt && storage->frame.current_rt->flags[RasterizerStorage::RENDER_TARGET_TRANSPARENT]) {
	...
	if (state.using_skeleton) {
		state.canvas_shader.set_uniform(CanvasShaderGLES4::SKELETON_TRANSFORM, state.skeleton_transform);
		state.canvas_shader.set_uniform(CanvasShaderGLES4::SKELETON_TRANSFORM_INVERSE, state.skeleton_transform_inverse);
	}

	state.canvas_shader.set_uniform(CanvasShaderGLES4::FINAL_MODULATE, state.canvas_item_modulate);
	state.canvas_shader.set_uniform(CanvasShaderGLES4::MODELVIEW_MATRIX, state.final_transform);
	state.canvas_shader.set_uniform(CanvasShaderGLES4::EXTRA_MATRIX, state.extra_matrix);
	if (storage->frame.current_rt) {
		state.canvas_shader.set_uniform(CanvasShaderGLES4::SCREEN_PIXEL_SIZE, Vector2(1.0 / storage->frame.current_rt->width, 1.0 / storage->frame.current_rt->height));
	} else {
		state.canvas_shader.set_uniform(CanvasShaderGLES4::SCREEN_PIXEL_SIZE, Vector2(1.0, 1.0));
	}
	if (unshaded || (state.canvas_item_modulate.a > 0.001 && (!r_ris.shader_cache || r_ris.shader_cache->canvas_item.light_mode != RasterizerStorageGLES4::Shader::CanvasItem::LIGHT_MODE_LIGHT_ONLY) && !p_ci->light_masked)) {
		_legacy_canvas_item_render_commands(p_ci, r_ris.current_clip, reclip, nullptr);
	}

	if ((blend_mode == RasterizerStorageGLES4::Shader::CanvasItem::BLEND_MODE_MIX || blend_mode == RasterizerStorageGLES4::Shader::CanvasItem::BLEND_MODE_PMALPHA) && r_ris.item_group_light && !unshaded) {
		...
		while (light) {
			if (p_ci->light_mask & light->item_mask && r_ris.item_group_z >= light->z_min && r_ris.item_group_z <= light->z_max && p_ci->global_rect_cache.intersects_transformed(light->xform_cache, light->rect_cache)) {
				...
				if (!light_used) {
					state.canvas_shader.set_conditional(CanvasShaderGLES4::USE_LIGHTING, true);
					light_used = true;
				}

				bool has_shadow = light->shadow_buffer.is_valid() && p_ci->light_mask & light->item_shadow_mask;

				state.canvas_shader.set_conditional(CanvasShaderGLES4::USE_SHADOWS, has_shadow);
				if (has_shadow) {
					state.canvas_shader.set_conditional(CanvasShaderGLES4::SHADOW_USE_GRADIENT, light->shadow_gradient_length > 0);
					state.canvas_shader.set_conditional(CanvasShaderGLES4::SHADOW_FILTER_NEAREST, light->shadow_filter == VS::CANVAS_LIGHT_FILTER_NONE);
					state.canvas_shader.set_conditional(CanvasShaderGLES4::SHADOW_FILTER_PCF3, light->shadow_filter == VS::CANVAS_LIGHT_FILTER_PCF3);
					state.canvas_shader.set_conditional(CanvasShaderGLES4::SHADOW_FILTER_PCF5, light->shadow_filter == VS::CANVAS_LIGHT_FILTER_PCF5);
					state.canvas_shader.set_conditional(CanvasShaderGLES4::SHADOW_FILTER_PCF7, light->shadow_filter == VS::CANVAS_LIGHT_FILTER_PCF7);
					state.canvas_shader.set_conditional(CanvasShaderGLES4::SHADOW_FILTER_PCF9, light->shadow_filter == VS::CANVAS_LIGHT_FILTER_PCF9);
					state.canvas_shader.set_conditional(CanvasShaderGLES4::SHADOW_FILTER_PCF13, light->shadow_filter == VS::CANVAS_LIGHT_FILTER_PCF13);
				}
				...
				if (light_rebind) {
					state.canvas_shader.set_uniform(CanvasShaderGLES4::FINAL_MODULATE, state.canvas_item_modulate);
					state.canvas_shader.set_uniform(CanvasShaderGLES4::MODELVIEW_MATRIX, state.final_transform);
					state.canvas_shader.set_uniform(CanvasShaderGLES4::EXTRA_MATRIX, Transform2D());
					if (storage->frame.current_rt) {
						state.canvas_shader.set_uniform(CanvasShaderGLES4::SCREEN_PIXEL_SIZE, Vector2(1.0 / storage->frame.current_rt->width, 1.0 / storage->frame.current_rt->height));
					} else {
						state.canvas_shader.set_uniform(CanvasShaderGLES4::SCREEN_PIXEL_SIZE, Vector2(1.0, 1.0));
				...
				if (has_shadow) {
					RasterizerStorageGLES4::CanvasLightShadow *cls = storage->canvas_light_shadow_owner.get(light->shadow_buffer);
					WRAPPED_GL_ACTIVE_TEXTURE(GL_TEXTURE0 + storage->config.max_texture_image_units - 2);
					glBindTexture(GL_TEXTURE_2D, cls->distance);

					/*canvas_shader.set_uniform(CanvasShaderGLES4::SHADOW_MATRIX,light->shadow_matrix_cache);
					canvas_shader.set_uniform(CanvasShaderGLES4::SHADOW_ESM_MULTIPLIER,light->shadow_esm_mult);
					canvas_shader.set_uniform(CanvasShaderGLES4::LIGHT_SHADOW_COLOR,light->shadow_color);*/
				}

				WRAPPED_GL_ACTIVE_TEXTURE(GL_TEXTURE0 + storage->config.max_texture_image_units - 1);
				RasterizerStorageGLES4::Texture *t = storage->texture_owner.getornull(light->texture);
				if (!t) {
					glBindTexture(GL_TEXTURE_2D, storage->resources.white_tex);
				} else {
					t = t->get_ptr();
		...
		if (light_used) {
			state.canvas_shader.set_conditional(CanvasShaderGLES4::USE_LIGHTING, false);
			state.canvas_shader.set_conditional(CanvasShaderGLES4::USE_SHADOWS, false);
			state.canvas_shader.set_conditional(CanvasShaderGLES4::SHADOW_FILTER_NEAREST, false);
			state.canvas_shader.set_conditional(CanvasShaderGLES4::SHADOW_FILTER_PCF3, false);
			state.canvas_shader.set_conditional(CanvasShaderGLES4::SHADOW_FILTER_PCF5, false);
			state.canvas_shader.set_conditional(CanvasShaderGLES4::SHADOW_FILTER_PCF7, false);
			state.canvas_shader.set_conditional(CanvasShaderGLES4::SHADOW_FILTER_PCF9, false);
			state.canvas_shader.set_conditional(CanvasShaderGLES4::SHADOW_FILTER_PCF13, false);
...
void RasterizerCanvasGLES4::render_batches(Item *p_current_clip, bool &r_reclip, RasterizerStorageGLES4::Material *p_material) {
	//	bdata.reset_flush();
	//	return;
				...
				for (int i = batch.first_command; i < end_command; i++) {
					...
					switch (c->type) {
						...
						case Item::Command::TYPE_RECT: {
							...
							RasterizerStorageGLES4::Texture *texture = _bind_canvas_texture(rect->texture, rect->normal_map);
							...
							if (use_nvidia_rect_workaround) {
								...
								if (texture) {
									...
									state.canvas_shader.set_uniform(CanvasShaderGLES4::COLOR_TEXPIXEL_SIZE, texpixel_size);

									state.canvas_shader.set_uniform(CanvasShaderGLES4::DST_RECT, Color(dst_rect.position.x, dst_rect.position.y, dst_rect.size.x, dst_rect.size.y));
									state.canvas_shader.set_uniform(CanvasShaderGLES4::SRC_RECT, Color(src_rect.position.x, src_rect.position.y, src_rect.size.x, src_rect.size.y));
									state.canvas_shader.set_uniform(CanvasShaderGLES4::CLIP_RECT_UV, rect->flags & CANVAS_RECT_CLIP_UV);
									...
									state.canvas_shader.set_uniform(CanvasShaderGLES4::DST_RECT, Color(dst_rect.position.x, dst_rect.position.y, dst_rect.size.x, dst_rect.size.y));
									state.canvas_shader.set_uniform(CanvasShaderGLES4::SRC_RECT, Color(0, 0, 1, 1));
									state.canvas_shader.set_uniform(CanvasShaderGLES4::CLIP_RECT_UV, false);
									glDrawArrays(GL_TRIANGLE_FAN, 0, 4);
									storage->info.render._2d_draw_call_count++;
								}
						...
						case Item::Command::TYPE_NINEPATCH: {
							...
							RasterizerStorageGLES4::Texture *texture = _bind_canvas_texture(np->texture, np->normal_map);
							...
							if (!texture) {
								...
								if (np->source != Rect2()) {
									...
									state.canvas_shader.set_uniform(CanvasShaderGLES4::SRC_RECT, Color(np->source.position.x / texture->width, np->source.position.y / texture->height, np->source.size.x / texture->width, np->source.size.y / texture->height));
								} else {
							...
							state.canvas_shader.set_uniform(CanvasShaderGLES4::COLOR_TEXPIXEL_SIZE, texpixel_size);
							state.canvas_shader.set_uniform(CanvasShaderGLES4::CLIP_RECT_UV, false);
							state.canvas_shader.set_uniform(CanvasShaderGLES4::NP_REPEAT_H, int(np->axis_x));
							state.canvas_shader.set_uniform(CanvasShaderGLES4::NP_REPEAT_V, int(np->axis_y));
							state.canvas_shader.set_uniform(CanvasShaderGLES4::NP_DRAW_CENTER, np->draw_center);
							state.canvas_shader.set_uniform(CanvasShaderGLES4::NP_MARGINS, Color(np->margin[MARGIN_LEFT], np->margin[MARGIN_TOP], np->margin[MARGIN_RIGHT], np->margin[MARGIN_BOTTOM]));
							state.canvas_shader.set_uniform(CanvasShaderGLES4::DST_RECT, Color(np->rect.position.x, np->rect.position.y, np->rect.size.x, np->rect.size.y));
						...
						case Item::Command::TYPE_PRIMITIVE: {
							...
							RasterizerStorageGLES4::Texture *texture = _bind_canvas_texture(primitive->texture, primitive->normal_map);
						...
						case Item::Command::TYPE_POLYGON: {
							...
							RasterizerStorageGLES4::Texture *texture = _bind_canvas_texture(polygon->texture, polygon->normal_map);
						...
						case Item::Command::TYPE_MESH: {
							...
							RasterizerStorageGLES4::Texture *texture = _bind_canvas_texture(mesh->texture, mesh->normal_map);
							...
							state.canvas_shader.set_uniform(CanvasShaderGLES4::MODELVIEW_MATRIX, state.final_transform * mesh->transform);

							RasterizerStorageGLES4::Mesh *mesh_data = storage->mesh_owner.getornull(mesh->mesh);
							if (mesh_data) {
								for (int j = 0; j < mesh_data->surfaces.size(); j++) {
									RasterizerStorageGLES4::Surface *s = mesh_data->surfaces[j];
						...
						case Item::Command::TYPE_MULTIMESH: {
							...
							RasterizerStorageGLES4::MultiMesh *multi_mesh = storage->multimesh_owner.getornull(mmesh->multimesh);

							if (!multi_mesh) {
								break;
							}

							RasterizerStorageGLES4::Mesh *mesh_data = storage->mesh_owner.getornull(multi_mesh->mesh);
							...
							RasterizerStorageGLES4::Texture *texture = _bind_canvas_texture(mmesh->texture, mmesh->normal_map);

							state.canvas_shader.set_conditional(CanvasShaderGLES4::USE_INSTANCE_CUSTOM, multi_mesh->custom_data_format != VS::MULTIMESH_CUSTOM_DATA_NONE);
							state.canvas_shader.set_conditional(CanvasShaderGLES4::USE_INSTANCING, true);
							...
							state.canvas_shader.set_conditional(CanvasShaderGLES4::USE_INSTANCE_CUSTOM, false);
							state.canvas_shader.set_conditional(CanvasShaderGLES4::USE_INSTANCING, false);
							state.using_texture_rect = true;
							_set_texture_rect_mode(false);
						...
						case Item::Command::TYPE_PARTICLES: {
							...
							RasterizerStorageGLES4::Particles *particles = storage->particles_owner.getornull(particles_cmd->particles);
							if (!particles) {
								break;
							}
							...
							state.canvas_shader.set_conditional(CanvasShaderGLES4::USE_INSTANCE_CUSTOM, true);
							state.canvas_shader.set_conditional(CanvasShaderGLES4::USE_PARTICLES, true);
							state.canvas_shader.set_conditional(CanvasShaderGLES4::USE_INSTANCING, true);
							//reset shader and force rebind
							state.using_texture_rect = true;
							_set_texture_rect_mode(false);

							RasterizerStorageGLES4::Texture *texture = _bind_canvas_texture(particles_cmd->texture, particles_cmd->normal_map);

							if (texture) {
								Size2 texpixel_size(1.0 / texture->width, 1.0 / texture->height);
								state.canvas_shader.set_uniform(CanvasShaderGLES4::COLOR_TEXPIXEL_SIZE, texpixel_size);
							} else {
								state.canvas_shader.set_uniform(CanvasShaderGLES4::COLOR_TEXPIXEL_SIZE, Vector2(1.0, 1.0));
							}
							...
							if (!particles->use_local_coords) {
								...
								state.canvas_shader.set_uniform(CanvasShaderGLES4::MODELVIEW_MATRIX, state.final_transform * inv_xf);
							}
							...
							state.canvas_shader.set_conditional(CanvasShaderGLES4::USE_INSTANCE_CUSTOM, false);
							state.canvas_shader.set_conditional(CanvasShaderGLES4::USE_PARTICLES, false);
							state.canvas_shader.set_conditional(CanvasShaderGLES4::USE_INSTANCING, false);
							state.using_texture_rect = true;
							_set_texture_rect_mode(false);
...
void RasterizerCanvasGLES4::render_joined_item(const BItemJoined &p_bij, RenderItemState &r_ris) {
	storage->info.render._2d_item_count++;
	...
	if (unshaded || (state.canvas_item_modulate.a > 0.001 && (!r_ris.shader_cache || r_ris.shader_cache->canvas_item.light_mode != RasterizerStorageGLES4::Shader::CanvasItem::LIGHT_MODE_LIGHT_ONLY) && !p_ci->light_masked)) {
		RasterizerStorageGLES4::Material *material_ptr = nullptr;
...
bool RasterizerCanvasGLES4::try_join_item(Item *p_ci, RenderItemState &r_ris, bool &r_batch_break) {
...
void RasterizerCanvasGLES4::canvas_render_items_implementation(Item *p_item_list, int p_z, const Color &p_modulate, Light *p_light, const Transform2D &p_base_transform) {
	...
	//	state.canvas_shader.set_conditional(CanvasShaderGLES4::USE_SKELETON, false);
	//	state.current_tex = RID();
...
void RasterizerCanvasGLES4::_batch_upload_buffers() {
	// noop?
...
void RasterizerCanvasGLES4::_batch_render_lines(const Batch &p_batch, RasterizerStorageGLES4::Material *p_material, bool p_anti_alias) {
	_set_texture_rect_mode(false);
...
void RasterizerCanvasGLES4::_batch_render_prepare() {
	...
	//	state.canvas_shader.set_uniform(CanvasShaderGLES4::CLIP_RECT_UV, p_rect->flags & CANVAS_RECT_CLIP_UV);
	state.canvas_shader.set_uniform(CanvasShaderGLES4::CLIP_RECT_UV, false);
...
void RasterizerCanvasGLES4::_batch_render_generic(const Batch &p_batch, RasterizerStorageGLES4::Material *p_material) {
	...
	state.canvas_shader.set_uniform(CanvasShaderGLES4::COLOR_TEXPIXEL_SIZE, tps);
...
void RasterizerCanvasGLES4::initialize() {
	RasterizerGLES4::gl_check_errors();
	RasterizerCanvasBaseGLES4::initialize();
...
RasterizerCanvasGLES4::RasterizerCanvasGLES4() {
	batch_constructor();
}
