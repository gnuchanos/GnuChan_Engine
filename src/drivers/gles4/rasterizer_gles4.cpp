...
#include "rasterizer_gles4.h"

#include "core/os/os.h"
#include "core/project_settings.h"

RasterizerStorage *RasterizerGLES4::get_storage() {
	return storage;
}

RasterizerCanvas *RasterizerGLES4::get_canvas() {
	return canvas;
}

RasterizerScene *RasterizerGLES4::get_scene() {
	return scene;
}
...
Error RasterizerGLES4::is_viable() {
#ifdef GLAD_ENABLED
	if (!gladLoadGL()) {
	...
	// OpenGL 4.3 Core Profile required
...
void RasterizerGLES4::initialize() {
	print_verbose("Using GLES4 video driver");
	...
	*/
	...
	print_line("OpenGL 4.3 Renderer: " + VisualServer::get_singleton()->get_video_adapter_name());
	storage->initialize();
	canvas->initialize();
	scene->initialize();
}

void RasterizerGLES4::begin_frame(double frame_step) {
	...
	storage->info.render.shader_compiles_in_progress_count = ShaderGLES4::active_compiles_count;

	ShaderGLES4::current_frame = storage->frame.count;

	scene->iteration();
}

void RasterizerGLES4::set_current_render_target(RID p_render_target) {
	...
	if (p_render_target.is_valid()) {
		RasterizerStorageGLES4::RenderTarget *rt = storage->render_target_owner.getornull(p_render_target);
...
void RasterizerGLES4::restore_render_target(bool p_3d_was_drawn) {
	ERR_FAIL_COND(storage->frame.current_rt == nullptr);
	RasterizerStorageGLES4::RenderTarget *rt = storage->frame.current_rt;
...
void RasterizerGLES4::clear_render_target(const Color &p_color) {
...
void RasterizerGLES4::set_boot_image(const Ref<Image> &p_image, const Color &p_color, bool p_scale, bool p_use_filter) {
	...
	RasterizerStorageGLES4::Texture *t = storage->texture_owner.get(texture);
...
void RasterizerGLES4::set_shader_time_scale(float p_scale) {
	time_scale = p_scale;
}

void RasterizerGLES4::blit_render_target_to_screen(RID p_render_target, const Rect2 &p_screen_rect, int p_screen) {
	ERR_FAIL_COND(storage->frame.current_rt);
	...
	if (rt->flags[RasterizerStorage::RENDER_TARGET_KEEP_3D_LINEAR]) {
		...
		canvas->state.canvas_shader.set_conditional(CanvasShaderGLES4::LINEAR_TO_SRGB, true);
		...
		canvas->state.canvas_shader.set_conditional(CanvasShaderGLES4::LINEAR_TO_SRGB, false);
	} else {
		...
		glBindFramebuffer(GL_DRAW_FRAMEBUFFER, RasterizerStorageGLES4::system_fbo);
...
void RasterizerGLES4::output_lens_distorted_to_screen(RID p_render_target, const Rect2 &p_screen_rect, float p_k1, float p_k2, const Vector2 &p_eye_center, float p_oversample) {
	ERR_FAIL_COND(storage->frame.current_rt);
...
void RasterizerGLES4::end_frame(bool p_swap_buffers) {
	...
	ShaderGLES4::advance_async_shaders_compilation();
...
void RasterizerGLES4::finalize() {
	storage->finalize();
	canvas->finalize();
}

Rasterizer *RasterizerGLES4::_create_current() {
	return memnew(RasterizerGLES4);
}

void RasterizerGLES4::make_current() {
	_create_func = _create_current;
}

void RasterizerGLES4::register_config() {
}

bool RasterizerGLES4::gl_check_errors() {
...
RasterizerGLES4::RasterizerGLES4() {
	storage = memnew(RasterizerStorageGLES4);
	canvas = memnew(RasterizerCanvasGLES4);
	scene = memnew(RasterizerSceneGLES4);
	...
	ShaderGLES4::compiles_started_this_frame = &storage->info.render.shader_compiles_started_count;
	ShaderGLES4::max_frame_compiles_in_progress = &storage->info.render.shader_compiles_in_progress_count;
}
