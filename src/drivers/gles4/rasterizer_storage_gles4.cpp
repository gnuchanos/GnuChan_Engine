...
GLuint RasterizerStorageGLES4::system_fbo = 0;

void RasterizerStorageGLES4::GLWrapper::initialize(int p_max_texture_image_units) {
	texture_unit_table.create(p_max_texture_image_units);
}

void RasterizerStorageGLES4::GLWrapper::reset() {
...
int32_t RasterizerStorageGLES4::safe_gl_get_integer(unsigned int p_gl_param_name, int32_t p_max_accepted) {
	int64_t temp;
...
Ref<Image> RasterizerStorageGLES4::_get_gl_image_and_format(const Ref<Image> &p_image, Image::Format p_format, uint32_t p_flags, Image::Format &r_real_format, GLenum &r_gl_format, GLenum &r_gl_internal_format, GLenum &r_gl_type, bool &r_compressed, bool &r_srgb, bool p_force_decompress) const {
	r_compressed = false;
	r_gl_format = 0;
	...
	switch (p_format) {
		...
		case Image::FORMAT_LA8: {
			...
			r_gl_type = GL_UNSIGNED_BYTE;
#else
			r_gl_internal_format = GL_RED_ALPHA;
			r_gl_format = GL_RED_ALPHA;
			r_gl_type = GL_UNSIGNED_BYTE;
#endif
		} break;
...
RID RasterizerStorageGLES4::texture_create() {
...
void RasterizerStorageGLES4::texture_allocate(RID p_texture, int p_width, int p_height, int p_depth_3d, Image::Format p_format, VisualServer::TextureType p_type, uint32_t p_flags) {
	GLenum format;
...
void RasterizerStorageGLES4::texture_set_data(RID p_texture, const Ref<Image> &p_image, int p_layer) {
	Texture *texture = texture_owner.get(p_texture);
...
void RasterizerStorageGLES4::texture_set_data_partial(RID p_texture, const Ref<Image> &p_image, int src_x, int src_y, int src_w, int src_h, int dst_x, int dst_y, int p_dst_mip, int p_layer) {
	Texture *texture = texture_owner.get(p_texture);
...
Ref<Image> RasterizerStorageGLES4::texture_get_data(RID p_texture, int p_layer) const {
	Texture *texture = texture_owner.get(p_texture);
	...
	if (texture->type == VS::TEXTURE_TYPE_2D_ARRAY || texture->type == VS::TEXTURE_TYPE_3D) {
		...
		// now bring the OpenGL context into the correct state
		{
			...
			shaders.copy.set_conditional(CopyShaderGLES4::LINEAR_TO_SRGB, !srgb);
			shaders.copy.set_conditional(CopyShaderGLES4::USE_TEXTURE3D, texture->type == VS::TEXTURE_TYPE_3D);
			shaders.copy.set_conditional(CopyShaderGLES4::USE_TEXTURE2DARRAY, texture->type == VS::TEXTURE_TYPE_2D_ARRAY);
			shaders.copy.bind();
			...
			shaders.copy.set_uniform(CopyShaderGLES4::LAYER, layer);
		...
		{
			shaders.copy.set_conditional(CopyShaderGLES4::USE_TEXTURE3D, false);
			shaders.copy.set_conditional(CopyShaderGLES4::USE_TEXTURE2DARRAY, false);
			shaders.copy.set_conditional(CopyShaderGLES4::LINEAR_TO_SRGB, false);
...
void RasterizerStorageGLES4::texture_set_flags(RID p_texture, uint32_t p_flags) {
	Texture *texture = texture_owner.get(p_texture);
	ERR_FAIL_COND(!texture);
...
uint32_t RasterizerStorageGLES4::texture_get_flags(RID p_texture) const {
	Texture *texture = texture_owner.get(p_texture);
...
Image::Format RasterizerStorageGLES4::texture_get_format(RID p_texture) const {
	Texture *texture = texture_owner.get(p_texture);
...
VisualServer::TextureType RasterizerStorageGLES4::texture_get_type(RID p_texture) const {
	Texture *texture = texture_owner.get(p_texture);
...
uint32_t RasterizerStorageGLES4::texture_get_texid(RID p_texture) const {
	Texture *texture = texture_owner.get(p_texture);
...
void RasterizerStorageGLES4::texture_bind(RID p_texture, uint32_t p_texture_no) {
...
uint32_t RasterizerStorageGLES4::texture_get_width(RID p_texture) const {
	Texture *texture = texture_owner.get(p_texture);
...
uint32_t RasterizerStorageGLES4::texture_get_height(RID p_texture) const {
	Texture *texture = texture_owner.get(p_texture);
...
uint32_t RasterizerStorageGLES4::texture_get_depth(RID p_texture) const {
	Texture *texture = texture_owner.get(p_texture);
...
void RasterizerStorageGLES4::texture_set_size_override(RID p_texture, int p_width, int p_height, int p_depth) {
	Texture *texture = texture_owner.get(p_texture);
...
void RasterizerStorageGLES4::texture_set_path(RID p_texture, const String &p_path) {
	Texture *texture = texture_owner.get(p_texture);
	ERR_FAIL_COND(!texture);
...
String RasterizerStorageGLES4::texture_get_path(RID p_texture) const {
	Texture *texture = texture_owner.get(p_texture);
	ERR_FAIL_COND_V(!texture, String());
	return texture->path;
}
void RasterizerStorageGLES4::texture_debug_usage(List<VS::TextureInfo> *r_info) {
	List<RID> textures;
...
void RasterizerStorageGLES4::texture_set_shrink_all_x2_on_set_data(bool p_enable) {
	config.shrink_textures_x2 = p_enable;
}

void RasterizerStorageGLES4::textures_keep_original(bool p_enable) {
	config.keep_original_textures = p_enable;
}

void RasterizerStorageGLES4::texture_set_detect_3d_callback(RID p_texture, VisualServer::TextureDetectCallback p_callback, void *p_userdata) {
	Texture *texture = texture_owner.get(p_texture);
	ERR_FAIL_COND(!texture);
...
void RasterizerStorageGLES4::texture_set_detect_srgb_callback(RID p_texture, VisualServer::TextureDetectCallback p_callback, void *p_userdata) {
	Texture *texture = texture_owner.get(p_texture);
	ERR_FAIL_COND(!texture);
...
void RasterizerStorageGLES4::texture_set_detect_normal_callback(RID p_texture, VisualServer::TextureDetectCallback p_callback, void *p_userdata) {
	Texture *texture = texture_owner.get(p_texture);
	ERR_FAIL_COND(!texture);
...
RID RasterizerStorageGLES4::texture_create_radiance_cubemap(RID p_source, int p_resolution) const {
	...
	shaders.cubemap_filter.set_conditional(CubemapFilterShaderGLES4::USE_DUAL_PARABOLOID, false);
	...
	while (mm_level) {
		for (int i = 0; i < 6; i++) {
			...
			shaders.cubemap_filter.set_uniform(CubemapFilterShaderGLES4::FACE_ID, i);
			shaders.cubemap_filter.set_uniform(CubemapFilterShaderGLES4::ROUGHNESS, lod / float(mipmaps - 1));
	...
	glBindFramebuffer(GL_FRAMEBUFFER, RasterizerStorageGLES4::system_fbo);
...
Size2 RasterizerStorageGLES4::texture_size_with_proxy(RID p_texture) const {
...
void RasterizerStorageGLES4::texture_set_proxy(RID p_texture, RID p_proxy) {
	Texture *texture = texture_owner.get(p_texture);
	ERR_FAIL_COND(!texture);
...
void RasterizerStorageGLES4::texture_set_force_redraw_if_visible(RID p_texture, bool p_enable) {
	Texture *texture = texture_owner.get(p_texture);
	ERR_FAIL_COND(!texture);
	texture->redraw_if_visible = p_enable;
}

RID RasterizerStorageGLES4::sky_create() {
	Sky *sky = memnew(Sky);
	sky->radiance = 0;
	sky->irradiance = 0;
...
void RasterizerStorageGLES4::sky_set_texture(RID p_sky, RID p_panorama, int p_radiance_size) {
...

	{
		...
		shaders.cubemap_filter.set_conditional(CubemapFilterShaderGLES4::USE_DUAL_PARABOLOID, true);
		shaders.cubemap_filter.set_conditional(CubemapFilterShaderGLES4::USE_SOURCE_PANORAMA, true);
		shaders.cubemap_filter.set_conditional(CubemapFilterShaderGLES4::COMPUTE_IRRADIANCE, true);
		shaders.cubemap_filter.bind();
		...
		shaders.cubemap_filter.set_uniform(CubemapFilterShaderGLES4::SOURCE_MIP_LEVEL, MAX(Math::floor(Math::log(float(texture->width)) / Math::log(2.0f)) - 10.0f, 0.0f));
		...
		for (int i = 0; i < 2; i++) {
			...
			shaders.cubemap_filter.set_uniform(CubemapFilterShaderGLES4::Z_FLIP, i > 0);
		...
		shaders.cubemap_filter.set_conditional(CubemapFilterShaderGLES4::USE_DUAL_PARABOLOID, false);
		shaders.cubemap_filter.set_conditional(CubemapFilterShaderGLES4::USE_SOURCE_PANORAMA, false);
		shaders.cubemap_filter.set_conditional(CubemapFilterShaderGLES4::COMPUTE_IRRADIANCE, false);

		shaders.copy.set_conditional(CopyShaderGLES4::USE_LOD, true);
		shaders.copy.bind();
		shaders.copy.set_uniform(CopyShaderGLES4::MIP_LEVEL, MAX(Math::floor(Math::log(float(upscale_size)) / Math::log(2.0f)) - 5.0f, 0.0f)); // Mip level that corresponds to a 32x32 texture
		...
		shaders.copy.set_conditional(CopyShaderGLES4::USE_LOD, false);
	...
	if (config.use_texture_array_environment) {
		...
		for (int j = 0; j < array_level; j++) {
			...
			if (j == 0) {
				...
				shaders.cubemap_filter.set_conditional(CubemapFilterShaderGLES4::USE_DUAL_PARABOLOID, true);
				shaders.cubemap_filter.set_conditional(CubemapFilterShaderGLES4::USE_SOURCE_PANORAMA, true);
				shaders.cubemap_filter.set_conditional(CubemapFilterShaderGLES4::USE_SOURCE_DUAL_PARABOLOID_ARRAY, false);
				shaders.cubemap_filter.bind();
				gl_wrapper.gl_active_texture(GL_TEXTURE0);
				glBindTexture(texture->target, texture->tex_id);
				shaders.cubemap_filter.set_uniform(CubemapFilterShaderGLES4::SOURCE_RESOLUTION, float(texture->width / 4));
			} else {
				shaders.cubemap_filter.set_conditional(CubemapFilterShaderGLES4::USE_DUAL_PARABOLOID, true);
				shaders.cubemap_filter.set_conditional(CubemapFilterShaderGLES4::USE_SOURCE_PANORAMA, false);
				shaders.cubemap_filter.set_conditional(CubemapFilterShaderGLES4::USE_SOURCE_DUAL_PARABOLOID_ARRAY, true);
				shaders.cubemap_filter.bind();
				gl_wrapper.gl_active_texture(GL_TEXTURE0);
				glBindTexture(GL_TEXTURE_2D_ARRAY, sky->radiance);
				shaders.cubemap_filter.set_uniform(CubemapFilterShaderGLES4::SOURCE_ARRAY_INDEX, j - 1); //read from previous to ensure better blur
			}
			...
			for (int i = 0; i < 2; i++) {
				...
				shaders.cubemap_filter.set_uniform(CubemapFilterShaderGLES4::Z_FLIP, i > 0);
				shaders.cubemap_filter.set_uniform(CubemapFilterShaderGLES4::ROUGHNESS, j / float(array_level - 1));
		...
		while (mm_level) {
			...
			if (lod == 0) {
				...
				shaders.cubemap_filter.set_conditional(CubemapFilterShaderGLES4::USE_DUAL_PARABOLOID, true);
				shaders.cubemap_filter.set_conditional(CubemapFilterShaderGLES4::USE_SOURCE_PANORAMA, true);
				shaders.cubemap_filter.set_conditional(CubemapFilterShaderGLES4::USE_SOURCE_DUAL_PARABOLOID, false);
				shaders.cubemap_filter.bind();
				gl_wrapper.gl_active_texture(GL_TEXTURE0);
				glBindTexture(texture->target, texture->tex_id);
				shaders.cubemap_filter.set_uniform(CubemapFilterShaderGLES4::SOURCE_RESOLUTION, float(texture->width / 4));
				...
				shaders.cubemap_filter.set_conditional(CubemapFilterShaderGLES4::USE_DUAL_PARABOLOID, true);
				shaders.cubemap_filter.set_conditional(CubemapFilterShaderGLES4::USE_SOURCE_PANORAMA, false);
				shaders.cubemap_filter.set_conditional(CubemapFilterShaderGLES4::USE_SOURCE_DUAL_PARABOLOID, true);
				shaders.cubemap_filter.bind();
				gl_wrapper.gl_active_texture(GL_TEXTURE0);
				glBindTexture(GL_TEXTURE_2D, sky->radiance);
				shaders.cubemap_filter.set_uniform(CubemapFilterShaderGLES4::SOURCE_MIP_LEVEL, float(lod - 1)); //read from previous to ensure better blur
			}
...
RID RasterizerStorageGLES4::shader_create() {
...
void RasterizerStorageGLES4::_shader_make_dirty(Shader *p_shader) {
...
void RasterizerStorageGLES4::shader_set_code(RID p_shader, const String &p_code) {
	...
	ShaderGLES4 *shaders[VS::SHADER_MAX] = {
...
String RasterizerStorageGLES4::shader_get_code(RID p_shader) const {
...
void RasterizerStorageGLES4::_update_shader(Shader *p_shader) const {
	...
	ShaderCompilerGLES4::GeneratedCode gen_code;
	ShaderCompilerGLES4::IdentifierActions *actions = nullptr;

	int async_mode = (int)ShaderGLES4::ASYNC_MODE_VISIBLE;
	...
	switch (p_shader->mode) {
		...
		case VS::SHADER_SPATIAL: {
			...
			shaders.actions_scene.render_mode_values["async_visible"] = Pair<int *, int>(&async_mode, (int)ShaderGLES4::ASYNC_MODE_VISIBLE);
			shaders.actions_scene.render_mode_values["async_hidden"] = Pair<int *, int>(&async_mode, (int)ShaderGLES4::ASYNC_MODE_HIDDEN);
	...
	p_shader->shader->set_custom_shader_code(p_shader->custom_code_id, gen_code.vertex, gen_code.vertex_global, gen_code.fragment, gen_code.light, gen_code.fragment_global, gen_code.uniforms, gen_code.texture_uniforms, gen_code.defines, (ShaderGLES4::AsyncMode)async_mode);
...
void RasterizerStorageGLES4::update_dirty_shaders() {
	while (_shader_dirty_list.first()) {
		_update_shader(_shader_dirty_list.first()->self());
	}
}

void RasterizerStorageGLES4::shader_get_param_list(RID p_shader, List<PropertyInfo> *p_param_list) const {
	Shader *shader = shader_owner.get(p_shader);
	ERR_FAIL_COND(!shader);
...
void RasterizerStorageGLES4::shader_set_default_texture_param(RID p_shader, const StringName &p_name, RID p_texture) {
	Shader *shader = shader_owner.get(p_shader);
	ERR_FAIL_COND(!shader);
...
RID RasterizerStorageGLES4::shader_get_default_texture_param(RID p_shader, const StringName &p_name) const {
	const Shader *shader = shader_owner.get(p_shader);
...
void RasterizerStorageGLES4::shader_add_custom_define(RID p_shader, const String &p_define) {
	Shader *shader = shader_owner.get(p_shader);
	ERR_FAIL_COND(!shader);
...
void RasterizerStorageGLES4::shader_get_custom_defines(RID p_shader, Vector<String> *p_defines) const {
	Shader *shader = shader_owner.get(p_shader);
	ERR_FAIL_COND(!shader);
...
void RasterizerStorageGLES4::shader_remove_custom_define(RID p_shader, const String &p_define) {
	Shader *shader = shader_owner.get(p_shader);
	ERR_FAIL_COND(!shader);
...
void RasterizerStorageGLES4::set_shader_async_hidden_forbidden(bool p_forbidden) {
	ShaderGLES4::async_hidden_forbidden = p_forbidden;
}

bool RasterizerStorageGLES4::is_shader_async_hidden_forbidden() {
	return ShaderGLES4::async_hidden_forbidden;
}

/* COMMON MATERIAL API */

void RasterizerStorageGLES4::_material_make_dirty(Material *p_material) const {
...
RID RasterizerStorageGLES4::material_create() {
	Material *material = memnew(Material);

	return material_owner.make_rid(material);
}

void RasterizerStorageGLES4::material_set_shader(RID p_material, RID p_shader) {
...
RID RasterizerStorageGLES4::material_get_shader(RID p_material) const {
...
void RasterizerStorageGLES4::material_set_param(RID p_material, const StringName &p_param, const Variant &p_value) {
	Material *material = material_owner.get(p_material);
	ERR_FAIL_COND(!material);
...
Variant RasterizerStorageGLES4::material_get_param(RID p_material, const StringName &p_param) const {
	const Material *material = material_owner.get(p_material);
...
Variant RasterizerStorageGLES4::material_get_param_default(RID p_material, const StringName &p_param) const {
	const Material *material = material_owner.get(p_material);
	ERR_FAIL_COND_V(!material, Variant());
...
void RasterizerStorageGLES4::material_set_line_width(RID p_material, float p_width) {
	Material *material = material_owner.get(p_material);
	ERR_FAIL_COND(!material);
...
void RasterizerStorageGLES4::material_set_next_pass(RID p_material, RID p_next_material) {
	Material *material = material_owner.get(p_material);
	ERR_FAIL_COND(!material);
...
bool RasterizerStorageGLES4::material_is_animated(RID p_material) {
	Material *material = material_owner.get(p_material);
...
bool RasterizerStorageGLES4::material_casts_shadows(RID p_material) {
	Material *material = material_owner.get(p_material);
	ERR_FAIL_COND_V(!material, false);
	if (material->dirty_list.in_list()) {
		_update_material(material);
...
bool RasterizerStorageGLES4::material_uses_tangents(RID p_material) {
	Material *material = material_owner.get(p_material);
	ERR_FAIL_COND_V(!material, false);
...
bool RasterizerStorageGLES4::material_uses_ensure_correct_normals(RID p_material) {
	Material *material = material_owner.get(p_material);
	ERR_FAIL_COND_V(!material, false);
...
void RasterizerStorageGLES4::material_add_instance_owner(RID p_material, RasterizerScene::InstanceBase *p_instance) {
	Material *material = material_owner.get(p_material);
	ERR_FAIL_COND(!material);
...
void RasterizerStorageGLES4::material_remove_instance_owner(RID p_material, RasterizerScene::InstanceBase *p_instance) {
	Material *material = material_owner.get(p_material);
	ERR_FAIL_COND(!material);
...
void RasterizerStorageGLES4::material_set_render_priority(RID p_material, int priority) {
...
void RasterizerStorageGLES4::_update_material(Material *material) {
	if (material->dirty_list.in_list()) {
...
void RasterizerStorageGLES4::_material_add_geometry(RID p_material, Geometry *p_geometry) {
...
void RasterizerStorageGLES4::_material_remove_geometry(RID p_material, Geometry *p_geometry) {
	Material *material = material_owner.getornull(p_material);
	ERR_FAIL_COND(!material);
...
void RasterizerStorageGLES4::update_dirty_materials() {
...
RID RasterizerStorageGLES4::mesh_create() {
	Mesh *mesh = memnew(Mesh);

	return mesh_owner.make_rid(mesh);
}

void RasterizerStorageGLES4::mesh_add_surface(RID p_mesh, uint32_t p_format, VS::PrimitiveType p_primitive, const PoolVector<uint8_t> &p_array, int p_vertex_count, const PoolVector<uint8_t> &p_index_array, int p_index_count, const AABB &p_aabb, const Vector<PoolVector<uint8_t>> &p_blend_shapes, const Vector<AABB> &p_bone_aabbs) {
...
void RasterizerStorageGLES4::mesh_set_blend_shape_count(RID p_mesh, int p_amount) {
	Mesh *mesh = mesh_owner.getornull(p_mesh);
	ERR_FAIL_COND(!mesh);
...
int RasterizerStorageGLES4::mesh_get_blend_shape_count(RID p_mesh) const {
...
void RasterizerStorageGLES4::mesh_set_blend_shape_mode(RID p_mesh, VS::BlendShapeMode p_mode) {
	Mesh *mesh = mesh_owner.getornull(p_mesh);
	ERR_FAIL_COND(!mesh);

	mesh->blend_shape_mode = p_mode;
}
VS::BlendShapeMode RasterizerStorageGLES4::mesh_get_blend_shape_mode(RID p_mesh) const {
	const Mesh *mesh = mesh_owner.getornull(p_mesh);
...
void RasterizerStorageGLES4::mesh_set_blend_shape_values(RID p_mesh, PoolVector<float> p_values) {
	Mesh *mesh = mesh_owner.getornull(p_mesh);
	ERR_FAIL_COND(!mesh);
	mesh->blend_shape_values = p_values;
}

PoolVector<float> RasterizerStorageGLES4::mesh_get_blend_shape_values(RID p_mesh) const {
	const Mesh *mesh = mesh_owner.getornull(p_mesh);
	ERR_FAIL_COND_V(!mesh, PoolVector<float>());
	return mesh->blend_shape_values;
}

void RasterizerStorageGLES4::mesh_surface_update_region(RID p_mesh, int p_surface, int p_offset, const PoolVector<uint8_t> &p_data) {
	Mesh *mesh = mesh_owner.getornull(p_mesh);
	ERR_FAIL_COND(!mesh);
...
void RasterizerStorageGLES4::mesh_surface_set_material(RID p_mesh, int p_surface, RID p_material) {
	Mesh *mesh = mesh_owner.getornull(p_mesh);
	ERR_FAIL_COND(!mesh);
	ERR_FAIL_INDEX(p_surface, mesh->surfaces.size());
...
RID RasterizerStorageGLES4::mesh_surface_get_material(RID p_mesh, int p_surface) const {
	const Mesh *mesh = mesh_owner.getornull(p_mesh);
...
int RasterizerStorageGLES4::mesh_surface_get_array_len(RID p_mesh, int p_surface) const {
	const Mesh *mesh = mesh_owner.getornull(p_mesh);
	ERR_FAIL_COND_V(!mesh, 0);
...
int RasterizerStorageGLES4::mesh_surface_get_array_index_len(RID p_mesh, int p_surface) const {
	const Mesh *mesh = mesh_owner.getornull(p_mesh);
	ERR_FAIL_COND_V(!mesh, 0);
	ERR_FAIL_INDEX_V(p_surface, mesh->surfaces.size(), 0);
...
PoolVector<uint8_t> RasterizerStorageGLES4::mesh_surface_get_array(RID p_mesh, int p_surface) const {
	const Mesh *mesh = mesh_owner.getornull(p_mesh);
...
PoolVector<uint8_t> RasterizerStorageGLES4::mesh_surface_get_index_array(RID p_mesh, int p_surface) const {
	const Mesh *mesh = mesh_owner.getornull(p_mesh);
	ERR_FAIL_COND_V(!mesh, PoolVector<uint8_t>());
	ERR_FAIL_INDEX_V(p_surface, mesh->surfaces.size(), PoolVector<uint8_t>());
...
uint32_t RasterizerStorageGLES4::mesh_surface_get_format(RID p_mesh, int p_surface) const {
	const Mesh *mesh = mesh_owner.getornull(p_mesh);
...
VS::PrimitiveType RasterizerStorageGLES4::mesh_surface_get_primitive_type(RID p_mesh, int p_surface) const {
	const Mesh *mesh = mesh_owner.getornull(p_mesh);
...
AABB RasterizerStorageGLES4::mesh_surface_get_aabb(RID p_mesh, int p_surface) const {
	const Mesh *mesh = mesh_owner.getornull(p_mesh);
...
Vector<PoolVector<uint8_t>> RasterizerStorageGLES4::mesh_surface_get_blend_shapes(RID p_mesh, int p_surface) const {
	const Mesh *mesh = mesh_owner.getornull(p_mesh);
...
Vector<AABB> RasterizerStorageGLES4::mesh_surface_get_skeleton_aabb(RID p_mesh, int p_surface) const {
	const Mesh *mesh = mesh_owner.getornull(p_mesh);
...
void RasterizerStorageGLES4::mesh_remove_surface(RID p_mesh, int p_surface) {
	Mesh *mesh = mesh_owner.getornull(p_mesh);
	ERR_FAIL_COND(!mesh);
	ERR_FAIL_INDEX(p_surface, mesh->surfaces.size());
...
int RasterizerStorageGLES4::mesh_get_surface_count(RID p_mesh) const {
	const Mesh *mesh = mesh_owner.getornull(p_mesh);
	ERR_FAIL_COND_V(!mesh, 0);
	return mesh->surfaces.size();
}

void RasterizerStorageGLES4::mesh_set_custom_aabb(RID p_mesh, const AABB &p_aabb) {
	Mesh *mesh = mesh_owner.getornull(p_mesh);
	ERR_FAIL_COND(!mesh);
...
AABB RasterizerStorageGLES4::mesh_get_custom_aabb(RID p_mesh) const {
	const Mesh *mesh = mesh_owner.getornull(p_mesh);
	ERR_FAIL_COND_V(!mesh, AABB());
...
AABB RasterizerStorageGLES4::mesh_get_aabb(RID p_mesh, RID p_skeleton) const {
...
void RasterizerStorageGLES4::mesh_clear(RID p_mesh) {
	Mesh *mesh = mesh_owner.getornull(p_mesh);
	ERR_FAIL_COND(!mesh);
...
void RasterizerStorageGLES4::mesh_render_blend_shapes(Surface *s, const float *p_weights) {
	glBindVertexArray(s->array_id);

	BlendShapeShaderGLES4::Conditionals cond[VS::ARRAY_MAX - 1] = {
		BlendShapeShaderGLES4::ENABLE_NORMAL, //will be ignored
		BlendShapeShaderGLES4::ENABLE_NORMAL,
		BlendShapeShaderGLES4::ENABLE_TANGENT,
		BlendShapeShaderGLES4::ENABLE_COLOR,
		BlendShapeShaderGLES4::ENABLE_UV,
		BlendShapeShaderGLES4::ENABLE_UV2,
		BlendShapeShaderGLES4::ENABLE_SKELETON,
		BlendShapeShaderGLES4::ENABLE_SKELETON,
	};
	...
	shaders.blend_shapes.set_conditional(BlendShapeShaderGLES4::ENABLE_BLEND, false); //first pass does not blend
	shaders.blend_shapes.set_conditional(BlendShapeShaderGLES4::USE_2D_VERTEX, s->format & VS::ARRAY_FLAG_USE_2D_VERTICES); //use 2D vertices if needed
	shaders.blend_shapes.set_conditional(BlendShapeShaderGLES4::ENABLE_OCTAHEDRAL_COMPRESSION, s->format & VS::ARRAY_FLAG_USE_OCTAHEDRAL_COMPRESSION); //use octahedral normal compression

	shaders.blend_shapes.bind();

	shaders.blend_shapes.set_uniform(BlendShapeShaderGLES4::BLEND_AMOUNT, base_weight);
	...
	shaders.blend_shapes.set_conditional(BlendShapeShaderGLES4::ENABLE_BLEND, true); //first pass does not blend
	shaders.blend_shapes.bind();
	...
	for (int ti = 0; ti < mtc; ti++) {
		...
		shaders.blend_shapes.set_uniform(BlendShapeShaderGLES4::BLEND_AMOUNT, weight);
...
RID RasterizerStorageGLES4::_multimesh_create() {
	MultiMesh *multimesh = memnew(MultiMesh);
	return multimesh_owner.make_rid(multimesh);
}

void RasterizerStorageGLES4::_multimesh_allocate(RID p_multimesh, int p_instances, VS::MultimeshTransformFormat p_transform_format, VS::MultimeshColorFormat p_color_format, VS::MultimeshCustomDataFormat p_data_format) {
...
int RasterizerStorageGLES4::_multimesh_get_instance_count(RID p_multimesh) const {
	MultiMesh *multimesh = multimesh_owner.getornull(p_multimesh);
...
void RasterizerStorageGLES4::_multimesh_set_mesh(RID p_multimesh, RID p_mesh) {
	MultiMesh *multimesh = multimesh_owner.getornull(p_multimesh);
	ERR_FAIL_COND(!multimesh);
...
void RasterizerStorageGLES4::_multimesh_instance_set_transform(RID p_multimesh, int p_index, const Transform &p_transform) {
	MultiMesh *multimesh = multimesh_owner.getornull(p_multimesh);
	ERR_FAIL_COND(!multimesh);
...
void RasterizerStorageGLES4::_multimesh_instance_set_transform_2d(RID p_multimesh, int p_index, const Transform2D &p_transform) {
	MultiMesh *multimesh = multimesh_owner.getornull(p_multimesh);
	ERR_FAIL_COND(!multimesh);
	ERR_FAIL_INDEX(p_index, multimesh->size);
...
void RasterizerStorageGLES4::_multimesh_instance_set_color(RID p_multimesh, int p_index, const Color &p_color) {
	MultiMesh *multimesh = multimesh_owner.getornull(p_multimesh);
	ERR_FAIL_COND(!multimesh);
	ERR_FAIL_INDEX(p_index, multimesh->size);
...
void RasterizerStorageGLES4::_multimesh_instance_set_custom_data(RID p_multimesh, int p_index, const Color &p_custom_data) {
	MultiMesh *multimesh = multimesh_owner.getornull(p_multimesh);
	ERR_FAIL_COND(!multimesh);
	ERR_FAIL_INDEX(p_index, multimesh->size);
...
RID RasterizerStorageGLES4::_multimesh_get_mesh(RID p_multimesh) const {
	MultiMesh *multimesh = multimesh_owner.getornull(p_multimesh);
...
Transform RasterizerStorageGLES4::_multimesh_instance_get_transform(RID p_multimesh, int p_index) const {
	MultiMesh *multimesh = multimesh_owner.getornull(p_multimesh);
...
Transform2D RasterizerStorageGLES4::_multimesh_instance_get_transform_2d(RID p_multimesh, int p_index) const {
	MultiMesh *multimesh = multimesh_owner.getornull(p_multimesh);
...
Color RasterizerStorageGLES4::_multimesh_instance_get_color(RID p_multimesh, int p_index) const {
	MultiMesh *multimesh = multimesh_owner.getornull(p_multimesh);
...
Color RasterizerStorageGLES4::_multimesh_instance_get_custom_data(RID p_multimesh, int p_index) const {
	MultiMesh *multimesh = multimesh_owner.getornull(p_multimesh);
	ERR_FAIL_COND_V(!multimesh, Color());
	ERR_FAIL_INDEX_V(p_index, multimesh->size, Color());
...
void RasterizerStorageGLES4::_multimesh_set_as_bulk_array(RID p_multimesh, const PoolVector<float> &p_array) {
	MultiMesh *multimesh = multimesh_owner.getornull(p_multimesh);
	ERR_FAIL_COND(!multimesh);
...
void RasterizerStorageGLES4::_multimesh_set_visible_instances(RID p_multimesh, int p_visible) {
	MultiMesh *multimesh = multimesh_owner.getornull(p_multimesh);
	ERR_FAIL_COND(!multimesh);

	multimesh->visible_instances = p_visible;
}
int RasterizerStorageGLES4::_multimesh_get_visible_instances(RID p_multimesh) const {
	MultiMesh *multimesh = multimesh_owner.getornull(p_multimesh);
...
AABB RasterizerStorageGLES4::_multimesh_get_aabb(RID p_multimesh) const {
	MultiMesh *multimesh = multimesh_owner.getornull(p_multimesh);
	ERR_FAIL_COND_V(!multimesh, AABB());

	const_cast<RasterizerStorageGLES4 *>(this)->update_dirty_multimeshes(); //update pending AABBs

	return multimesh->aabb;
}

RasterizerStorage::MMInterpolator *RasterizerStorageGLES4::_multimesh_get_interpolator(RID p_multimesh) const {
	MultiMesh *multimesh = multimesh_owner.getornull(p_multimesh);
...
void RasterizerStorageGLES4::multimesh_attach_canvas_item(RID p_multimesh, RID p_canvas_item, bool p_attach) {
	MultiMesh *multimesh = multimesh_owner.getornull(p_multimesh);
...
void RasterizerStorageGLES4::update_dirty_multimeshes() {
...
RID RasterizerStorageGLES4::immediate_create() {
	Immediate *im = memnew(Immediate);
	return immediate_owner.make_rid(im);
}

void RasterizerStorageGLES4::immediate_begin(RID p_immediate, VS::PrimitiveType p_primitive, RID p_texture) {
...
void RasterizerStorageGLES4::immediate_vertex(RID p_immediate, const Vector3 &p_vertex) {
	Immediate *im = immediate_owner.get(p_immediate);
	ERR_FAIL_COND(!im);
	ERR_FAIL_COND(!im->building);
...
void RasterizerStorageGLES4::immediate_normal(RID p_immediate, const Vector3 &p_normal) {
	Immediate *im = immediate_owner.get(p_immediate);
	ERR_FAIL_COND(!im);
	ERR_FAIL_COND(!im->building);
...
void RasterizerStorageGLES4::immediate_tangent(RID p_immediate, const Plane &p_tangent) {
	Immediate *im = immediate_owner.get(p_immediate);
	ERR_FAIL_COND(!im);
	ERR_FAIL_COND(!im->building);
...
void RasterizerStorageGLES4::immediate_color(RID p_immediate, const Color &p_color) {
	Immediate *im = immediate_owner.get(p_immediate);
	ERR_FAIL_COND(!im);
	ERR_FAIL_COND(!im->building);
...
void RasterizerStorageGLES4::immediate_uv(RID p_immediate, const Vector2 &tex_uv) {
	Immediate *im = immediate_owner.get(p_immediate);
	ERR_FAIL_COND(!im);
	ERR_FAIL_COND(!im->building);
...
void RasterizerStorageGLES4::immediate_uv2(RID p_immediate, const Vector2 &tex_uv) {
	Immediate *im = immediate_owner.get(p_immediate);
	ERR_FAIL_COND(!im);
	ERR_FAIL_COND(!im->building);
...
void RasterizerStorageGLES4::immediate_end(RID p_immediate) {
	Immediate *im = immediate_owner.get(p_immediate);
	ERR_FAIL_COND(!im);
	ERR_FAIL_COND(!im->building);
...
void RasterizerStorageGLES4::immediate_clear(RID p_immediate) {
	Immediate *im = immediate_owner.get(p_immediate);
	ERR_FAIL_COND(!im);
	ERR_FAIL_COND(im->building);
...
AABB RasterizerStorageGLES4::immediate_get_aabb(RID p_immediate) const {
	Immediate *im = immediate_owner.get(p_immediate);
	ERR_FAIL_COND_V(!im, AABB());
	return im->aabb;
}

void RasterizerStorageGLES4::immediate_set_material(RID p_immediate, RID p_material) {
	Immediate *im = immediate_owner.get(p_immediate);
	ERR_FAIL_COND(!im);
...
RID RasterizerStorageGLES4::immediate_get_material(RID p_immediate) const {
...
RID RasterizerStorageGLES4::skeleton_create() {
...
void RasterizerStorageGLES4::skeleton_allocate(RID p_skeleton, int p_bones, bool p_2d_skeleton) {
...
int RasterizerStorageGLES4::skeleton_get_bone_count(RID p_skeleton) const {
	Skeleton *skeleton = skeleton_owner.getornull(p_skeleton);
...
void RasterizerStorageGLES4::skeleton_bone_set_transform(RID p_skeleton, int p_bone, const Transform &p_transform) {
	Skeleton *skeleton = skeleton_owner.getornull(p_skeleton);
...
Transform RasterizerStorageGLES4::skeleton_bone_get_transform(RID p_skeleton, int p_bone) const {
	Skeleton *skeleton = skeleton_owner.getornull(p_skeleton);
...
void RasterizerStorageGLES4::skeleton_bone_set_transform_2d(RID p_skeleton, int p_bone, const Transform2D &p_transform) {
	Skeleton *skeleton = skeleton_owner.getornull(p_skeleton);
...
Transform2D RasterizerStorageGLES4::skeleton_bone_get_transform_2d(RID p_skeleton, int p_bone) const {
	Skeleton *skeleton = skeleton_owner.getornull(p_skeleton);
...
void RasterizerStorageGLES4::skeleton_set_base_transform_2d(RID p_skeleton, const Transform2D &p_base_transform) {
	Skeleton *skeleton = skeleton_owner.getornull(p_skeleton);
...
void RasterizerStorageGLES4::skeleton_attach_canvas_item(RID p_skeleton, RID p_canvas_item, bool p_attach) {
	Skeleton *skeleton = skeleton_owner.getornull(p_skeleton);
...
uint32_t RasterizerStorageGLES4::skeleton_get_revision(RID p_skeleton) const {
	const Skeleton *skeleton = skeleton_owner.getornull(p_skeleton);
	ERR_FAIL_COND_V(!skeleton, 0);
	return skeleton->revision;
}

void RasterizerStorageGLES4::update_dirty_skeletons() {
...
RID RasterizerStorageGLES4::light_create(VS::LightType p_type) {
...
void RasterizerStorageGLES4::light_set_color(RID p_light, const Color &p_color) {
	Light *light = light_owner.getornull(p_light);
	ERR_FAIL_COND(!light);

	light->color = p_color;
}
void RasterizerStorageGLES4::light_set_param(RID p_light, VS::LightParam p_param, float p_value) {
	Light *light = light_owner.getornull(p_light);
	ERR_FAIL_COND(!light);
...
void RasterizerStorageGLES4::light_set_shadow(RID p_light, bool p_enabled) {
	Light *light = light_owner.getornull(p_light);
	ERR_FAIL_COND(!light);
...
void RasterizerStorageGLES4::light_set_shadow_color(RID p_light, const Color &p_color) {
	Light *light = light_owner.getornull(p_light);
	ERR_FAIL_COND(!light);
	light->shadow_color = p_color;
}

void RasterizerStorageGLES4::light_set_projector(RID p_light, RID p_texture) {
	Light *light = light_owner.getornull(p_light);
	ERR_FAIL_COND(!light);
...
void RasterizerStorageGLES4::light_set_negative(RID p_light, bool p_enable) {
	Light *light = light_owner.getornull(p_light);
	ERR_FAIL_COND(!light);

	light->negative = p_enable;
}
void RasterizerStorageGLES4::light_set_cull_mask(RID p_light, uint32_t p_mask) {
	Light *light = light_owner.getornull(p_light);
	ERR_FAIL_COND(!light);
...
void RasterizerStorageGLES4::light_set_reverse_cull_face_mode(RID p_light, bool p_enabled) {
	Light *light = light_owner.getornull(p_light);
	ERR_FAIL_COND(!light);
...
void RasterizerStorageGLES4::light_set_use_gi(RID p_light, bool p_enabled) {
...
void RasterizerStorageGLES4::light_set_bake_mode(RID p_light, VS::LightBakeMode p_bake_mode) {
	Light *light = light_owner.getornull(p_light);
	ERR_FAIL_COND(!light);
...
void RasterizerStorageGLES4::light_omni_set_shadow_mode(RID p_light, VS::LightOmniShadowMode p_mode) {
	Light *light = light_owner.getornull(p_light);
	ERR_FAIL_COND(!light);
...
VS::LightOmniShadowMode RasterizerStorageGLES4::light_omni_get_shadow_mode(RID p_light) {
...
void RasterizerStorageGLES4::light_omni_set_shadow_detail(RID p_light, VS::LightOmniShadowDetail p_detail) {
	Light *light = light_owner.getornull(p_light);
	ERR_FAIL_COND(!light);
...
void RasterizerStorageGLES4::light_directional_set_shadow_mode(RID p_light, VS::LightDirectionalShadowMode p_mode) {
	Light *light = light_owner.getornull(p_light);
	ERR_FAIL_COND(!light);
...
void RasterizerStorageGLES4::light_directional_set_blend_splits(RID p_light, bool p_enable) {
	Light *light = light_owner.getornull(p_light);
	ERR_FAIL_COND(!light);
...
bool RasterizerStorageGLES4::light_directional_get_blend_splits(RID p_light) const {
	const Light *light = light_owner.getornull(p_light);
...
VS::LightDirectionalShadowMode RasterizerStorageGLES4::light_directional_get_shadow_mode(RID p_light) {
	const Light *light = light_owner.getornull(p_light);
...
void RasterizerStorageGLES4::light_directional_set_shadow_depth_range_mode(RID p_light, VS::LightDirectionalShadowDepthRangeMode p_range_mode) {
	Light *light = light_owner.getornull(p_light);
	ERR_FAIL_COND(!light);
...
VS::LightDirectionalShadowDepthRangeMode RasterizerStorageGLES4::light_directional_get_shadow_depth_range_mode(RID p_light) const {
	const Light *light = light_owner.getornull(p_light);
...
VS::LightType RasterizerStorageGLES4::light_get_type(RID p_light) const {
	const Light *light = light_owner.getornull(p_light);
...
float RasterizerStorageGLES4::light_get_param(RID p_light, VS::LightParam p_param) {
	const Light *light = light_owner.getornull(p_light);
	ERR_FAIL_COND_V(!light, VS::LIGHT_DIRECTIONAL);
...
Color RasterizerStorageGLES4::light_get_color(RID p_light) {
	const Light *light = light_owner.getornull(p_light);
...
bool RasterizerStorageGLES4::light_get_use_gi(RID p_light) {
	return light_get_bake_mode(p_light) != VS::LightBakeMode::LIGHT_BAKE_DISABLED;
}

VS::LightBakeMode RasterizerStorageGLES4::light_get_bake_mode(RID p_light) {
	Light *light = light_owner.getornull(p_light);
...
bool RasterizerStorageGLES4::light_has_shadow(RID p_light) const {
	const Light *light = light_owner.getornull(p_light);
	ERR_FAIL_COND_V(!light, VS::LIGHT_DIRECTIONAL);
...
uint64_t RasterizerStorageGLES4::light_get_version(RID p_light) const {
	const Light *light = light_owner.getornull(p_light);
...
AABB RasterizerStorageGLES4::light_get_aabb(RID p_light) const {
	const Light *light = light_owner.getornull(p_light);
...
RID RasterizerStorageGLES4::reflection_probe_create() {
...
void RasterizerStorageGLES4::reflection_probe_set_update_mode(RID p_probe, VS::ReflectionProbeUpdateMode p_mode) {
...
void RasterizerStorageGLES4::reflection_probe_set_intensity(RID p_probe, float p_intensity) {
	ReflectionProbe *reflection_probe = reflection_probe_owner.getornull(p_probe);
	ERR_FAIL_COND(!reflection_probe);
...
void RasterizerStorageGLES4::reflection_probe_set_interior_ambient(RID p_probe, const Color &p_ambient) {
	ReflectionProbe *reflection_probe = reflection_probe_owner.getornull(p_probe);
	ERR_FAIL_COND(!reflection_probe);
...
void RasterizerStorageGLES4::reflection_probe_set_interior_ambient_energy(RID p_probe, float p_energy) {
	ReflectionProbe *reflection_probe = reflection_probe_owner.getornull(p_probe);
	ERR_FAIL_COND(!reflection_probe);
...
void RasterizerStorageGLES4::reflection_probe_set_interior_ambient_probe_contribution(RID p_probe, float p_contrib) {
	ReflectionProbe *reflection_probe = reflection_probe_owner.getornull(p_probe);
	ERR_FAIL_COND(!reflection_probe);
...
void RasterizerStorageGLES4::reflection_probe_set_max_distance(RID p_probe, float p_distance) {
	ReflectionProbe *reflection_probe = reflection_probe_owner.getornull(p_probe);
	ERR_FAIL_COND(!reflection_probe);
...
void RasterizerStorageGLES4::reflection_probe_set_extents(RID p_probe, const Vector3 &p_extents) {
	ReflectionProbe *reflection_probe = reflection_probe_owner.getornull(p_probe);
	ERR_FAIL_COND(!reflection_probe);
...
void RasterizerStorageGLES4::reflection_probe_set_origin_offset(RID p_probe, const Vector3 &p_offset) {
	ReflectionProbe *reflection_probe = reflection_probe_owner.getornull(p_probe);
	ERR_FAIL_COND(!reflection_probe);
...
void RasterizerStorageGLES4::reflection_probe_set_as_interior(RID p_probe, bool p_enable) {
	ReflectionProbe *reflection_probe = reflection_probe_owner.getornull(p_probe);
	ERR_FAIL_COND(!reflection_probe);
...
void RasterizerStorageGLES4::reflection_probe_set_enable_box_projection(RID p_probe, bool p_enable) {
	ReflectionProbe *reflection_probe = reflection_probe_owner.getornull(p_probe);
	ERR_FAIL_COND(!reflection_probe);
...
void RasterizerStorageGLES4::reflection_probe_set_enable_shadows(RID p_probe, bool p_enable) {
	ReflectionProbe *reflection_probe = reflection_probe_owner.getornull(p_probe);
	ERR_FAIL_COND(!reflection_probe);
...
void RasterizerStorageGLES4::reflection_probe_set_cull_mask(RID p_probe, uint32_t p_layers) {
	ReflectionProbe *reflection_probe = reflection_probe_owner.getornull(p_probe);
	ERR_FAIL_COND(!reflection_probe);
...
void RasterizerStorageGLES4::reflection_probe_set_resolution(RID p_probe, int p_resolution) {
}

AABB RasterizerStorageGLES4::reflection_probe_get_aabb(RID p_probe) const {
...
VS::ReflectionProbeUpdateMode RasterizerStorageGLES4::reflection_probe_get_update_mode(RID p_probe) const {
	const ReflectionProbe *reflection_probe = reflection_probe_owner.getornull(p_probe);
...
uint32_t RasterizerStorageGLES4::reflection_probe_get_cull_mask(RID p_probe) const {
	const ReflectionProbe *reflection_probe = reflection_probe_owner.getornull(p_probe);
...
Vector3 RasterizerStorageGLES4::reflection_probe_get_extents(RID p_probe) const {
	const ReflectionProbe *reflection_probe = reflection_probe_owner.getornull(p_probe);
	ERR_FAIL_COND_V(!reflection_probe, Vector3());

	return reflection_probe->extents;
}
Vector3 RasterizerStorageGLES4::reflection_probe_get_origin_offset(RID p_probe) const {
	const ReflectionProbe *reflection_probe = reflection_probe_owner.getornull(p_probe);
	ERR_FAIL_COND_V(!reflection_probe, Vector3());
...
bool RasterizerStorageGLES4::reflection_probe_renders_shadows(RID p_probe) const {
	const ReflectionProbe *reflection_probe = reflection_probe_owner.getornull(p_probe);
...
float RasterizerStorageGLES4::reflection_probe_get_origin_max_distance(RID p_probe) const {
	const ReflectionProbe *reflection_probe = reflection_probe_owner.getornull(p_probe);
	ERR_FAIL_COND_V(!reflection_probe, 0);
...
RID RasterizerStorageGLES4::gi_probe_create() {
...
void RasterizerStorageGLES4::gi_probe_set_bounds(RID p_probe, const AABB &p_bounds) {
...
AABB RasterizerStorageGLES4::gi_probe_get_bounds(RID p_probe) const {
...
void RasterizerStorageGLES4::gi_probe_set_cell_size(RID p_probe, float p_size) {
	GIProbe *gip = gi_probe_owner.getornull(p_probe);
	ERR_FAIL_COND(!gip);
...
float RasterizerStorageGLES4::gi_probe_get_cell_size(RID p_probe) const {
	const GIProbe *gip = gi_probe_owner.getornull(p_probe);
...
void RasterizerStorageGLES4::gi_probe_set_to_cell_xform(RID p_probe, const Transform &p_xform) {
	GIProbe *gip = gi_probe_owner.getornull(p_probe);
	ERR_FAIL_COND(!gip);
...
Transform RasterizerStorageGLES4::gi_probe_get_to_cell_xform(RID p_probe) const {
	const GIProbe *gip = gi_probe_owner.getornull(p_probe);
...
void RasterizerStorageGLES4::gi_probe_set_dynamic_data(RID p_probe, const PoolVector<int> &p_data) {
	GIProbe *gip = gi_probe_owner.getornull(p_probe);
	ERR_FAIL_COND(!gip);
...
PoolVector<int> RasterizerStorageGLES4::gi_probe_get_dynamic_data(RID p_probe) const {
	const GIProbe *gip = gi_probe_owner.getornull(p_probe);
...
void RasterizerStorageGLES4::gi_probe_set_dynamic_range(RID p_probe, int p_range) {
	GIProbe *gip = gi_probe_owner.getornull(p_probe);
	ERR_FAIL_COND(!gip);

	gip->dynamic_range = p_range;
}
int RasterizerStorageGLES4::gi_probe_get_dynamic_range(RID p_probe) const {
	const GIProbe *gip = gi_probe_owner.getornull(p_probe);
	ERR_FAIL_COND_V(!gip, 0);
...
void RasterizerStorageGLES4::gi_probe_set_energy(RID p_probe, float p_range) {
	GIProbe *gip = gi_probe_owner.getornull(p_probe);
	ERR_FAIL_COND(!gip);
...
void RasterizerStorageGLES4::gi_probe_set_bias(RID p_probe, float p_range) {
	GIProbe *gip = gi_probe_owner.getornull(p_probe);
	ERR_FAIL_COND(!gip);
...
void RasterizerStorageGLES4::gi_probe_set_normal_bias(RID p_probe, float p_range) {
	GIProbe *gip = gi_probe_owner.getornull(p_probe);
	ERR_FAIL_COND(!gip);
...
void RasterizerStorageGLES4::gi_probe_set_propagation(RID p_probe, float p_range) {
	GIProbe *gip = gi_probe_owner.getornull(p_probe);
	ERR_FAIL_COND(!gip);
...
void RasterizerStorageGLES4::gi_probe_set_interior(RID p_probe, bool p_enable) {
	GIProbe *gip = gi_probe_owner.getornull(p_probe);
	ERR_FAIL_COND(!gip);
...
bool RasterizerStorageGLES4::gi_probe_is_interior(RID p_probe) const {
	const GIProbe *gip = gi_probe_owner.getornull(p_probe);
...
void RasterizerStorageGLES4::gi_probe_set_compress(RID p_probe, bool p_enable) {
	if (p_enable) {
		WARN_DEPRECATED_MSG("GIProbe's Compress property has been deprecated due to known bugs and will be removed in GnuChan 4.0.");
	}
...
bool RasterizerStorageGLES4::gi_probe_is_compressed(RID p_probe) const {
	const GIProbe *gip = gi_probe_owner.getornull(p_probe);
	ERR_FAIL_COND_V(!gip, false);

	return gip->compress;
}
float RasterizerStorageGLES4::gi_probe_get_energy(RID p_probe) const {
	const GIProbe *gip = gi_probe_owner.getornull(p_probe);
	ERR_FAIL_COND_V(!gip, 0);
...
float RasterizerStorageGLES4::gi_probe_get_bias(RID p_probe) const {
	const GIProbe *gip = gi_probe_owner.getornull(p_probe);
	ERR_FAIL_COND_V(!gip, 0);
...
float RasterizerStorageGLES4::gi_probe_get_normal_bias(RID p_probe) const {
	const GIProbe *gip = gi_probe_owner.getornull(p_probe);
	ERR_FAIL_COND_V(!gip, 0);
...
float RasterizerStorageGLES4::gi_probe_get_propagation(RID p_probe) const {
	const GIProbe *gip = gi_probe_owner.getornull(p_probe);
	ERR_FAIL_COND_V(!gip, 0);
...
uint32_t RasterizerStorageGLES4::gi_probe_get_version(RID p_probe) {
	const GIProbe *gip = gi_probe_owner.getornull(p_probe);
	ERR_FAIL_COND_V(!gip, 0);
...
RID RasterizerStorageGLES4::gi_probe_dynamic_data_create(int p_width, int p_height, int p_depth, GIProbeCompression p_compression) {
...
void RasterizerStorageGLES4::gi_probe_dynamic_data_update(RID p_gi_probe_data, int p_depth_slice, int p_slice_count, int p_mipmap, const void *p_data) {
...
RID RasterizerStorageGLES4::lightmap_capture_create() {
	LightmapCapture *capture = memnew(LightmapCapture);
	return lightmap_capture_data_owner.make_rid(capture);
}

void RasterizerStorageGLES4::lightmap_capture_set_bounds(RID p_capture, const AABB &p_bounds) {
	LightmapCapture *capture = lightmap_capture_data_owner.getornull(p_capture);
	ERR_FAIL_COND(!capture);
	capture->bounds = p_bounds;
	capture->instance_change_notify(true, false);
}
AABB RasterizerStorageGLES4::lightmap_capture_get_bounds(RID p_capture) const {
	const LightmapCapture *capture = lightmap_capture_data_owner.getornull(p_capture);
	ERR_FAIL_COND_V(!capture, AABB());
	return capture->bounds;
}
void RasterizerStorageGLES4::lightmap_capture_set_octree(RID p_capture, const PoolVector<uint8_t> &p_octree) {
	LightmapCapture *capture = lightmap_capture_data_owner.getornull(p_capture);
	ERR_FAIL_COND(!capture);
...
PoolVector<uint8_t> RasterizerStorageGLES4::lightmap_capture_get_octree(RID p_capture) const {
	const LightmapCapture *capture = lightmap_capture_data_owner.getornull(p_capture);
...
void RasterizerStorageGLES4::lightmap_capture_set_octree_cell_transform(RID p_capture, const Transform &p_xform) {
	LightmapCapture *capture = lightmap_capture_data_owner.getornull(p_capture);
	ERR_FAIL_COND(!capture);
	capture->cell_xform = p_xform;
}

Transform RasterizerStorageGLES4::lightmap_capture_get_octree_cell_transform(RID p_capture) const {
	const LightmapCapture *capture = lightmap_capture_data_owner.getornull(p_capture);
	ERR_FAIL_COND_V(!capture, Transform());
	return capture->cell_xform;
}

void RasterizerStorageGLES4::lightmap_capture_set_octree_cell_subdiv(RID p_capture, int p_subdiv) {
	LightmapCapture *capture = lightmap_capture_data_owner.getornull(p_capture);
	ERR_FAIL_COND(!capture);
	capture->cell_subdiv = p_subdiv;
}

int RasterizerStorageGLES4::lightmap_capture_get_octree_cell_subdiv(RID p_capture) const {
	const LightmapCapture *capture = lightmap_capture_data_owner.getornull(p_capture);
	ERR_FAIL_COND_V(!capture, 0);
	return capture->cell_subdiv;
}

void RasterizerStorageGLES4::lightmap_capture_set_energy(RID p_capture, float p_energy) {
	LightmapCapture *capture = lightmap_capture_data_owner.getornull(p_capture);
	ERR_FAIL_COND(!capture);
...
float RasterizerStorageGLES4::lightmap_capture_get_energy(RID p_capture) const {
	const LightmapCapture *capture = lightmap_capture_data_owner.getornull(p_capture);
	ERR_FAIL_COND_V(!capture, 0);
	return capture->energy;
}

void RasterizerStorageGLES4::lightmap_capture_set_interior(RID p_capture, bool p_interior) {
	LightmapCapture *capture = lightmap_capture_data_owner.getornull(p_capture);
	ERR_FAIL_COND(!capture);
...
bool RasterizerStorageGLES4::lightmap_capture_is_interior(RID p_capture) const {
	const LightmapCapture *capture = lightmap_capture_data_owner.getornull(p_capture);
	ERR_FAIL_COND_V(!capture, false);
	return capture->interior;
}

const PoolVector<RasterizerStorage::LightmapCaptureOctree> *RasterizerStorageGLES4::lightmap_capture_get_octree_ptr(RID p_capture) const {
	const LightmapCapture *capture = lightmap_capture_data_owner.getornull(p_capture);
	ERR_FAIL_COND_V(!capture, nullptr);
	return &capture->octree;
}

void RasterizerStorageGLES4::update_dirty_captures() {
...
RID RasterizerStorageGLES4::particles_create() {
	Particles *particles = memnew(Particles);

	return particles_owner.make_rid(particles);
}

void RasterizerStorageGLES4::particles_set_emitting(RID p_particles, bool p_emitting) {
...
bool RasterizerStorageGLES4::particles_get_emitting(RID p_particles) {
	Particles *particles = particles_owner.getornull(p_particles);
...
void RasterizerStorageGLES4::particles_set_amount(RID p_particles, int p_amount) {
	Particles *particles = particles_owner.getornull(p_particles);
	ERR_FAIL_COND(!particles);
...
void RasterizerStorageGLES4::particles_set_lifetime(RID p_particles, float p_lifetime) {
	Particles *particles = particles_owner.getornull(p_particles);
	ERR_FAIL_COND(!particles);
	particles->lifetime = p_lifetime;
}

void RasterizerStorageGLES4::particles_set_one_shot(RID p_particles, bool p_one_shot) {
	Particles *particles = particles_owner.getornull(p_particles);
	ERR_FAIL_COND(!particles);
	particles->one_shot = p_one_shot;
}

void RasterizerStorageGLES4::particles_set_pre_process_time(RID p_particles, float p_time) {
	Particles *particles = particles_owner.getornull(p_particles);
	ERR_FAIL_COND(!particles);
	particles->pre_process_time = p_time;
}
void RasterizerStorageGLES4::particles_set_explosiveness_ratio(RID p_particles, float p_ratio) {
	Particles *particles = particles_owner.getornull(p_particles);
	ERR_FAIL_COND(!particles);
	particles->explosiveness = p_ratio;
}
void RasterizerStorageGLES4::particles_set_randomness_ratio(RID p_particles, float p_ratio) {
	Particles *particles = particles_owner.getornull(p_particles);
	ERR_FAIL_COND(!particles);
	particles->randomness = p_ratio;
}

void RasterizerStorageGLES4::_particles_update_histories(Particles *particles) {
...
void RasterizerStorageGLES4::particles_set_custom_aabb(RID p_particles, const AABB &p_aabb) {
	Particles *particles = particles_owner.getornull(p_particles);
	ERR_FAIL_COND(!particles);
...
void RasterizerStorageGLES4::particles_set_speed_scale(RID p_particles, float p_scale) {
	Particles *particles = particles_owner.getornull(p_particles);
	ERR_FAIL_COND(!particles);

	particles->speed_scale = p_scale;
}
void RasterizerStorageGLES4::particles_set_use_local_coordinates(RID p_particles, bool p_enable) {
	Particles *particles = particles_owner.getornull(p_particles);
	ERR_FAIL_COND(!particles);
...
void RasterizerStorageGLES4::particles_set_fixed_fps(RID p_particles, int p_fps) {
	Particles *particles = particles_owner.getornull(p_particles);
	ERR_FAIL_COND(!particles);
...
void RasterizerStorageGLES4::particles_set_fractional_delta(RID p_particles, bool p_enable) {
	Particles *particles = particles_owner.getornull(p_particles);
	ERR_FAIL_COND(!particles);
...
void RasterizerStorageGLES4::particles_set_process_material(RID p_particles, RID p_material) {
	Particles *particles = particles_owner.getornull(p_particles);
	ERR_FAIL_COND(!particles);
...
void RasterizerStorageGLES4::particles_set_draw_order(RID p_particles, VS::ParticlesDrawOrder p_order) {
	Particles *particles = particles_owner.getornull(p_particles);
	ERR_FAIL_COND(!particles);
...
void RasterizerStorageGLES4::particles_set_draw_passes(RID p_particles, int p_passes) {
	Particles *particles = particles_owner.getornull(p_particles);
	ERR_FAIL_COND(!particles);
...
void RasterizerStorageGLES4::particles_set_draw_pass_mesh(RID p_particles, int p_pass, RID p_mesh) {
	Particles *particles = particles_owner.getornull(p_particles);
	ERR_FAIL_COND(!particles);
...
void RasterizerStorageGLES4::particles_restart(RID p_particles) {
	Particles *particles = particles_owner.getornull(p_particles);
	ERR_FAIL_COND(!particles);
...
void RasterizerStorageGLES4::particles_request_process(RID p_particles) {
	Particles *particles = particles_owner.getornull(p_particles);
	ERR_FAIL_COND(!particles);
...
AABB RasterizerStorageGLES4::particles_get_current_aabb(RID p_particles) {
...
AABB RasterizerStorageGLES4::particles_get_aabb(RID p_particles) const {
	const Particles *particles = particles_owner.getornull(p_particles);
	ERR_FAIL_COND_V(!particles, AABB());
...
void RasterizerStorageGLES4::particles_set_emission_transform(RID p_particles, const Transform &p_transform) {
	Particles *particles = particles_owner.getornull(p_particles);
	ERR_FAIL_COND(!particles);
...
int RasterizerStorageGLES4::particles_get_draw_passes(RID p_particles) const {
	const Particles *particles = particles_owner.getornull(p_particles);
...
RID RasterizerStorageGLES4::particles_get_draw_pass_mesh(RID p_particles, int p_pass) const {
	const Particles *particles = particles_owner.getornull(p_particles);
...
void RasterizerStorageGLES4::_particles_process(Particles *p_particles, float p_delta) {
	...
	if (p_particles->clear) {
		...
		if (p_particles->one_shot) {
			...
			shaders.particles.set_uniform(ParticlesShaderGLES4::EMITTING, false);
		}
		p_particles->cycle_number++;
	}

	shaders.particles.set_uniform(ParticlesShaderGLES4::SYSTEM_PHASE, new_phase);
	shaders.particles.set_uniform(ParticlesShaderGLES4::PREV_SYSTEM_PHASE, p_particles->phase);
	p_particles->phase = new_phase;

	shaders.particles.set_uniform(ParticlesShaderGLES4::DELTA, p_delta * p_particles->speed_scale);
	shaders.particles.set_uniform(ParticlesShaderGLES4::CLEAR, p_particles->clear);
	glUniform1ui(shaders.particles.get_uniform_location(ParticlesShaderGLES4::RANDOM_SEED), p_particles->random_seed);

	if (p_particles->use_local_coords) {
		shaders.particles.set_uniform(ParticlesShaderGLES4::EMISSION_TRANSFORM, Transform());
	} else {
		shaders.particles.set_uniform(ParticlesShaderGLES4::EMISSION_TRANSFORM, p_particles->emission_transform);
	}

	glUniform1ui(shaders.particles.get_uniform(ParticlesShaderGLES4::CYCLE), p_particles->cycle_number);
...
void RasterizerStorageGLES4::update_particles() {
	glEnable(GL_RASTERIZER_DISCARD);
	...
	while (particle_update_list.first()) {
		...
		if (!material || !material->shader || material->shader->mode != VS::SHADER_PARTICLES) {
			...
			for (int i = 0; i < tc; i++) {
				...
				RasterizerStorageGLES4::Texture *t = texture_owner.getornull(textures[i]);
		...
		shaders.particles.set_conditional(ParticlesShaderGLES4::USE_FRACTIONAL_DELTA, particles->fractional_delta);

		shaders.particles.bind();

		shaders.particles.set_uniform(ParticlesShaderGLES4::TOTAL_PARTICLES, particles->amount);
		shaders.particles.set_uniform(ParticlesShaderGLES4::TIME, frame.time[0]);
		shaders.particles.set_uniform(ParticlesShaderGLES4::EXPLOSIVENESS, particles->explosiveness);
		shaders.particles.set_uniform(ParticlesShaderGLES4::LIFETIME, particles->lifetime);
		shaders.particles.set_uniform(ParticlesShaderGLES4::ATTRACTOR_COUNT, 0);
		shaders.particles.set_uniform(ParticlesShaderGLES4::EMITTING, particles->emitting);
		shaders.particles.set_uniform(ParticlesShaderGLES4::RANDOMNESS, particles->randomness);
...
bool RasterizerStorageGLES4::particles_is_inactive(RID p_particles) const {
	const Particles *particles = particles_owner.getornull(p_particles);
	ERR_FAIL_COND_V(!particles, false);
...
void RasterizerStorageGLES4::instance_add_skeleton(RID p_skeleton, RasterizerScene::InstanceBase *p_instance) {
	Skeleton *skeleton = skeleton_owner.getornull(p_skeleton);
	ERR_FAIL_COND(!skeleton);
...
void RasterizerStorageGLES4::instance_remove_skeleton(RID p_skeleton, RasterizerScene::InstanceBase *p_instance) {
	Skeleton *skeleton = skeleton_owner.getornull(p_skeleton);
	ERR_FAIL_COND(!skeleton);
...
void RasterizerStorageGLES4::instance_add_dependency(RID p_base, RasterizerScene::InstanceBase *p_instance) {
...
void RasterizerStorageGLES4::instance_remove_dependency(RID p_base, RasterizerScene::InstanceBase *p_instance) {
	Instantiable *inst = nullptr;
...
void RasterizerStorageGLES4::_render_target_clear(RenderTarget *rt) {
	if (rt->fbo) {
...
void RasterizerStorageGLES4::_render_target_allocate(RenderTarget *rt) {
	...
	if (!rt->flags[RENDER_TARGET_NO_3D] && (!rt->flags[RENDER_TARGET_NO_3D_EFFECTS] || rt->msaa != VS::VIEWPORT_MSAA_DISABLED)) {
		...
		static const int msaa_value[] = { 0, 2, 4, 8, 16, 4, 16 }; // MSAA_EXT_nX is a GLES2 temporary hack ignored in GLES4 for now...
...
RID RasterizerStorageGLES4::render_target_create() {
...
void RasterizerStorageGLES4::render_target_set_position(RID p_render_target, int p_x, int p_y) {
	//only used in GLES2
}

void RasterizerStorageGLES4::render_target_set_size(RID p_render_target, int p_width, int p_height) {
...
RID RasterizerStorageGLES4::render_target_get_texture(RID p_render_target) const {
	RenderTarget *rt = render_target_owner.getornull(p_render_target);
...
uint32_t RasterizerStorageGLES4::render_target_get_depth_texture_id(RID p_render_target) const {
	RenderTarget *rt = render_target_owner.getornull(p_render_target);
...
void RasterizerStorageGLES4::render_target_set_external_texture(RID p_render_target, unsigned int p_texture_id, unsigned int p_depth_id) {
	RenderTarget *rt = render_target_owner.getornull(p_render_target);
	ERR_FAIL_COND(!rt);
...
void RasterizerStorageGLES4::render_target_set_flag(RID p_render_target, RenderTargetFlags p_flag, bool p_value) {
	RenderTarget *rt = render_target_owner.getornull(p_render_target);
	ERR_FAIL_COND(!rt);
...
bool RasterizerStorageGLES4::render_target_was_used(RID p_render_target) {
	RenderTarget *rt = render_target_owner.getornull(p_render_target);
...
void RasterizerStorageGLES4::render_target_clear_used(RID p_render_target) {
	RenderTarget *rt = render_target_owner.getornull(p_render_target);
	ERR_FAIL_COND(!rt);
...
void RasterizerStorageGLES4::render_target_set_msaa(RID p_render_target, VS::ViewportMSAA p_msaa) {
	RenderTarget *rt = render_target_owner.getornull(p_render_target);
	ERR_FAIL_COND(!rt);
...
void RasterizerStorageGLES4::render_target_set_use_fxaa(RID p_render_target, bool p_fxaa) {
	RenderTarget *rt = render_target_owner.getornull(p_render_target);
	ERR_FAIL_COND(!rt);
...
void RasterizerStorageGLES4::render_target_set_use_debanding(RID p_render_target, bool p_debanding) {
	RenderTarget *rt = render_target_owner.getornull(p_render_target);
	ERR_FAIL_COND(!rt);
...
void RasterizerStorageGLES4::render_target_set_sharpen_intensity(RID p_render_target, float p_intensity) {
	RenderTarget *rt = render_target_owner.getornull(p_render_target);
	ERR_FAIL_COND(!rt);
...
RID RasterizerStorageGLES4::canvas_light_shadow_buffer_create(int p_width) {
...
RID RasterizerStorageGLES4::canvas_light_occluder_create() {
...
void RasterizerStorageGLES4::canvas_light_occluder_set_polylines(RID p_occluder, const PoolVector<Vector2> &p_lines) {
...
VS::InstanceType RasterizerStorageGLES4::get_base_type(RID p_rid) const {
...
bool RasterizerStorageGLES4::free(RID p_rid) {
...
bool RasterizerStorageGLES4::has_os_feature(const String &p_feature) const {
	if (p_feature == "bptc") {
...
void RasterizerStorageGLES4::set_debug_generate_wireframes(bool p_generate) {
	config.generate_wireframes = p_generate;
}

void RasterizerStorageGLES4::render_info_begin_capture() {
	info.snap = info.render;
}

void RasterizerStorageGLES4::render_info_end_capture() {
...
int RasterizerStorageGLES4::get_captured_render_info(VS::RenderInfo p_info) {
	switch (p_info) {
...
uint64_t RasterizerStorageGLES4::get_render_info(VS::RenderInfo p_info) {
	switch (p_info) {
		case VS::INFO_OBJECTS_IN_FRAME:
...
String RasterizerStorageGLES4::get_video_adapter_name() const {
	return (const char *)glGetString(GL_RENDERER);
}

String RasterizerStorageGLES4::get_video_adapter_vendor() const {
	return (const char *)glGetString(GL_VENDOR);
}

void RasterizerStorageGLES4::initialize() {
	RasterizerStorageGLES4::system_fbo = 0;
	...
	// not yet detected on GLES4 (is this mandated?)
	...
	if (!(Engine::get_singleton()->is_editor_hint() || Main::is_project_manager())) {
		compilation_mode = ProjectSettings::get_singleton()->get("rendering/gles4/shaders/shader_compilation_mode");
	}
	...
	if (config.async_compilation_enabled) {
		ShaderGLES4::max_simultaneous_compiles = MAX(1, (int)ProjectSettings::get_singleton()->get("rendering/gles4/shaders/max_simultaneous_compiles"));
#ifdef GLES_OVER_GL
		if (GLAD_GL_ARB_parallel_shader_compile) {
			glMaxShaderCompilerThreadsARB(ShaderGLES4::max_simultaneous_compiles);
		} else if (GLAD_GL_KHR_parallel_shader_compile) {
			glMaxShaderCompilerThreadsKHR(ShaderGLES4::max_simultaneous_compiles);
		}
#else
#if defined(IPHONE_ENABLED) || defined(ANDROID_ENABLED) // TODO: Consider more platforms?
		void *gles4_lib = nullptr;
		void (*MaxShaderCompilerThreads)(GLuint) = nullptr;
#if defined(IPHONE_ENABLED)
		gles4_lib = dlopen(nullptr, RTLD_LAZY);
#elif defined(ANDROID_ENABLED)
		gles4_lib = dlopen("libGLESv3.so", RTLD_LAZY);
#endif
		if (gles4_lib) {
			MaxShaderCompilerThreads = (void (*)(GLuint))dlsym(gles4_lib, "glMaxShaderCompilerThreadsARB");
			if (!MaxShaderCompilerThreads) {
				MaxShaderCompilerThreads = (void (*)(GLuint))dlsym(gles4_lib, "glMaxShaderCompilerThreadsKHR");
			}
		}
		if (MaxShaderCompilerThreads) {
			MaxShaderCompilerThreads(ShaderGLES4::max_simultaneous_compiles);
		} else {
#ifdef DEBUG_ENABLED
		...
		ShaderGLES4::max_simultaneous_compiles = 0;
	}
#ifdef DEBUG_ENABLED
	ShaderGLES4::log_active_async_compiles_count = (bool)ProjectSettings::get_singleton()->get("rendering/gles4/shaders/log_active_async_compiles_count");
#endif
		...
		if (effectively_on) {
			if (config.shader_cache_enabled) {
				if (config.program_binary_supported) {
					...
					shaders.cache = memnew(ShaderCacheGLES4);
	...
	ShaderGLES4::compile_queue = shaders.compile_queue;
	ShaderGLES4::parallel_compile_supported = config.parallel_shader_compile_supported;
	ShaderGLES4::shader_cache = shaders.cache;
	ShaderGLES4::cache_write_queue = shaders.cache_write_queue;
	...
	shaders.cubemap_filter.set_conditional(CubemapFilterShaderGLES4::LOW_QUALITY, !ggx_hq);
...
void RasterizerStorageGLES4::finalize() {
...
void RasterizerStorageGLES4::update_dirty_resources() {
	update_dirty_multimeshes();
...
RasterizerStorageGLES4::RasterizerStorageGLES4() {
	config.should_orphan = true;
}
