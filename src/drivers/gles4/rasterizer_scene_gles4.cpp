...
#include "rasterizer_scene_gles4.h"

#include "core/math/math_funcs.h"
#include "core/os/os.h"
#include "core/project_settings.h"
#include "rasterizer_canvas_gles4.h"
...
const GLenum RasterizerSceneGLES4::_cube_side_enum[6] = {
...
void RasterizerSceneGLES4::store_transform(const Transform &p_mtx, float *p_array) {
...
void RasterizerSceneGLES4::store_camera(const CameraMatrix &p_mtx, float *p_array) {
	for (int i = 0; i < 4; i++) {
		for (int j = 0; j < 4; j++) {
...
void RasterizerSceneGLES4::directional_shadow_create() {
...
RID RasterizerSceneGLES4::shadow_atlas_create() {
...
void RasterizerSceneGLES4::shadow_atlas_set_size(RID p_atlas, int p_size) {
...
void RasterizerSceneGLES4::shadow_atlas_set_quadrant_subdivision(RID p_atlas, int p_quadrant, int p_subdivision) {
	ShadowAtlas *shadow_atlas = shadow_atlas_owner.getornull(p_atlas);
	ERR_FAIL_COND(!shadow_atlas);
...
bool RasterizerSceneGLES4::_shadow_atlas_find_shadow(ShadowAtlas *shadow_atlas, int *p_in_quadrants, int p_quadrant_count, int p_current_subdiv, uint64_t p_tick, int &r_quadrant, int &r_shadow) {
...
bool RasterizerSceneGLES4::shadow_atlas_update_light(RID p_atlas, RID p_light_intance, float p_coverage, uint64_t p_light_version) {
	ShadowAtlas *shadow_atlas = shadow_atlas_owner.getornull(p_atlas);
...
void RasterizerSceneGLES4::set_directional_shadow_count(int p_count) {
	directional_shadow.light_count = p_count;
	directional_shadow.current_light = 0;
}

int RasterizerSceneGLES4::get_directional_light_shadow_size(RID p_light_intance) {
...
RID RasterizerSceneGLES4::reflection_atlas_create() {
...
void RasterizerSceneGLES4::reflection_atlas_set_size(RID p_ref_atlas, int p_size) {
...
void RasterizerSceneGLES4::reflection_atlas_set_subdivision(RID p_ref_atlas, int p_subdiv) {
	ReflectionAtlas *reflection_atlas = reflection_atlas_owner.getornull(p_ref_atlas);
	ERR_FAIL_COND(!reflection_atlas);
...
RID RasterizerSceneGLES4::reflection_probe_instance_create(RID p_probe) {
	RasterizerStorageGLES4::ReflectionProbe *probe = storage->reflection_probe_owner.getornull(p_probe);
...
void RasterizerSceneGLES4::reflection_probe_instance_set_transform(RID p_instance, const Transform &p_transform) {
	ReflectionProbeInstance *rpi = reflection_probe_instance_owner.getornull(p_instance);
	ERR_FAIL_COND(!rpi);
	rpi->transform = p_transform;
}

void RasterizerSceneGLES4::reflection_probe_release_atlas_index(RID p_instance) {
	ReflectionProbeInstance *rpi = reflection_probe_instance_owner.getornull(p_instance);
	ERR_FAIL_COND(!rpi);
...
bool RasterizerSceneGLES4::reflection_probe_instance_needs_redraw(RID p_instance) {
	ReflectionProbeInstance *rpi = reflection_probe_instance_owner.getornull(p_instance);
...
bool RasterizerSceneGLES4::reflection_probe_instance_has_reflection(RID p_instance) {
	ReflectionProbeInstance *rpi = reflection_probe_instance_owner.getornull(p_instance);
	ERR_FAIL_COND_V(!rpi, false);
...
bool RasterizerSceneGLES4::reflection_probe_instance_begin_render(RID p_instance, RID p_reflection_atlas) {
	ReflectionProbeInstance *rpi = reflection_probe_instance_owner.getornull(p_instance);
	ERR_FAIL_COND_V(!rpi, false);
...
bool RasterizerSceneGLES4::reflection_probe_instance_postprocess_step(RID p_instance) {
	ReflectionProbeInstance *rpi = reflection_probe_instance_owner.getornull(p_instance);
	...
	storage->shaders.cubemap_filter.set_conditional(CubemapFilterShaderGLES4::USE_DUAL_PARABOLOID, true);
	...
	storage->shaders.cubemap_filter.set_conditional(CubemapFilterShaderGLES4::USE_DIRECT_WRITE, rpi->render_step == 0);
	storage->shaders.cubemap_filter.set_conditional(CubemapFilterShaderGLES4::LOW_QUALITY, rpi->probe_ptr->update_mode == VS::REFLECTION_PROBE_UPDATE_ALWAYS);
	for (int i = 0; i < 2; i++) {
		storage->shaders.cubemap_filter.set_uniform(CubemapFilterShaderGLES4::Z_FLIP, i == 0);
		storage->shaders.cubemap_filter.set_uniform(CubemapFilterShaderGLES4::ROUGHNESS, rpi->render_step / 5.0);
	...
	storage->shaders.cubemap_filter.set_conditional(CubemapFilterShaderGLES4::USE_DIRECT_WRITE, false);
	storage->shaders.cubemap_filter.set_conditional(CubemapFilterShaderGLES4::LOW_QUALITY, false);
...
RID RasterizerSceneGLES4::environment_create() {
	Environment *env = memnew(Environment);

	return environment_owner.make_rid(env);
}

void RasterizerSceneGLES4::environment_set_background(RID p_env, VS::EnvironmentBG p_bg) {
	Environment *env = environment_owner.getornull(p_env);
	ERR_FAIL_COND(!env);
	env->bg_mode = p_bg;
}

void RasterizerSceneGLES4::environment_set_sky(RID p_env, RID p_sky) {
	Environment *env = environment_owner.getornull(p_env);
	ERR_FAIL_COND(!env);
...
void RasterizerSceneGLES4::environment_set_sky_custom_fov(RID p_env, float p_scale) {
	Environment *env = environment_owner.getornull(p_env);
	ERR_FAIL_COND(!env);
...
void RasterizerSceneGLES4::environment_set_sky_orientation(RID p_env, const Basis &p_orientation) {
	Environment *env = environment_owner.getornull(p_env);
	ERR_FAIL_COND(!env);
...
void RasterizerSceneGLES4::environment_set_bg_color(RID p_env, const Color &p_color) {
	Environment *env = environment_owner.getornull(p_env);
	ERR_FAIL_COND(!env);

	env->bg_color = p_color;
}
void RasterizerSceneGLES4::environment_set_bg_energy(RID p_env, float p_energy) {
	Environment *env = environment_owner.getornull(p_env);
	ERR_FAIL_COND(!env);
...
void RasterizerSceneGLES4::environment_set_canvas_max_layer(RID p_env, int p_max_layer) {
	Environment *env = environment_owner.getornull(p_env);
	ERR_FAIL_COND(!env);

	env->canvas_max_layer = p_max_layer;
}
void RasterizerSceneGLES4::environment_set_ambient_light(RID p_env, const Color &p_color, float p_energy, float p_sky_contribution) {
	Environment *env = environment_owner.getornull(p_env);
	ERR_FAIL_COND(!env);
...
void RasterizerSceneGLES4::environment_set_camera_feed_id(RID p_env, int p_camera_feed_id) {
	Environment *env = environment_owner.getornull(p_env);
	ERR_FAIL_COND(!env);
...
void RasterizerSceneGLES4::environment_set_dof_blur_far(RID p_env, bool p_enable, float p_distance, float p_transition, float p_amount, VS::EnvironmentDOFBlurQuality p_quality) {
	Environment *env = environment_owner.getornull(p_env);
	ERR_FAIL_COND(!env);
...
void RasterizerSceneGLES4::environment_set_dof_blur_near(RID p_env, bool p_enable, float p_distance, float p_transition, float p_amount, VS::EnvironmentDOFBlurQuality p_quality) {
	Environment *env = environment_owner.getornull(p_env);
	ERR_FAIL_COND(!env);
...
void RasterizerSceneGLES4::environment_set_glow(RID p_env, bool p_enable, int p_level_flags, float p_intensity, float p_strength, float p_bloom_threshold, VS::EnvironmentGlowBlendMode p_blend_mode, float p_hdr_bleed_threshold, float p_hdr_bleed_scale, float p_hdr_luminance_cap, bool p_bicubic_upscale, bool p_high_quality) {
	Environment *env = environment_owner.getornull(p_env);
	ERR_FAIL_COND(!env);
...
void RasterizerSceneGLES4::environment_set_fog(RID p_env, bool p_enable, float p_begin, float p_end, RID p_gradient_texture) {
}

void RasterizerSceneGLES4::environment_set_ssr(RID p_env, bool p_enable, int p_max_steps, float p_fade_in, float p_fade_out, float p_depth_tolerance, bool p_roughness) {
	Environment *env = environment_owner.getornull(p_env);
	ERR_FAIL_COND(!env);
...
void RasterizerSceneGLES4::environment_set_ssao(RID p_env, bool p_enable, float p_radius, float p_intensity, float p_radius2, float p_intensity2, float p_bias, float p_light_affect, float p_ao_channel_affect, const Color &p_color, VS::EnvironmentSSAOQuality p_quality, VisualServer::EnvironmentSSAOBlur p_blur, float p_bilateral_sharpness) {
	Environment *env = environment_owner.getornull(p_env);
	ERR_FAIL_COND(!env);
...
void RasterizerSceneGLES4::environment_set_tonemap(RID p_env, VS::EnvironmentToneMapper p_tone_mapper, float p_exposure, float p_white, bool p_auto_exposure, float p_min_luminance, float p_max_luminance, float p_auto_exp_speed, float p_auto_exp_scale) {
	Environment *env = environment_owner.getornull(p_env);
	ERR_FAIL_COND(!env);
...
void RasterizerSceneGLES4::environment_set_adjustment(RID p_env, bool p_enable, float p_brightness, float p_contrast, float p_saturation, RID p_ramp) {
	Environment *env = environment_owner.getornull(p_env);
	ERR_FAIL_COND(!env);
...
void RasterizerSceneGLES4::environment_set_fog(RID p_env, bool p_enable, const Color &p_color, const Color &p_sun_color, float p_sun_amount) {
	Environment *env = environment_owner.getornull(p_env);
	ERR_FAIL_COND(!env);
...
void RasterizerSceneGLES4::environment_set_fog_depth(RID p_env, bool p_enable, float p_depth_begin, float p_depth_end, float p_depth_curve, bool p_transmit, float p_transmit_curve) {
	Environment *env = environment_owner.getornull(p_env);
	ERR_FAIL_COND(!env);
...
void RasterizerSceneGLES4::environment_set_fog_height(RID p_env, bool p_enable, float p_min_height, float p_max_height, float p_height_curve) {
	Environment *env = environment_owner.getornull(p_env);
	ERR_FAIL_COND(!env);
...
bool RasterizerSceneGLES4::is_environment(RID p_env) {
	return environment_owner.owns(p_env);
}

VS::EnvironmentBG RasterizerSceneGLES4::environment_get_background(RID p_env) {
...
int RasterizerSceneGLES4::environment_get_canvas_max_layer(RID p_env) {
	const Environment *env = environment_owner.getornull(p_env);
...
RID RasterizerSceneGLES4::light_instance_create(RID p_light) {
...
void RasterizerSceneGLES4::light_instance_set_transform(RID p_light_instance, const Transform &p_transform) {
...
void RasterizerSceneGLES4::light_instance_set_shadow_transform(RID p_light_instance, const CameraMatrix &p_projection, const Transform &p_transform, float p_far, float p_split, int p_pass, float p_bias_scale) {
	LightInstance *light_instance = light_instance_owner.getornull(p_light_instance);
	ERR_FAIL_COND(!light_instance);
...
void RasterizerSceneGLES4::light_instance_mark_visible(RID p_light_instance) {
	LightInstance *light_instance = light_instance_owner.getornull(p_light_instance);
	ERR_FAIL_COND(!light_instance);
...
RID RasterizerSceneGLES4::gi_probe_instance_create() {
	GIProbeInstance *gipi = memnew(GIProbeInstance);

	return gi_probe_instance_owner.make_rid(gipi);
}

void RasterizerSceneGLES4::gi_probe_instance_set_light_data(RID p_probe, RID p_base, RID p_data) {
	GIProbeInstance *gipi = gi_probe_instance_owner.getornull(p_probe);
	ERR_FAIL_COND(!gipi);
	gipi->data = p_data;
	gipi->probe = storage->gi_probe_owner.getornull(p_base);
	if (p_data.is_valid()) {
		RasterizerStorageGLES4::GIProbeData *gipd = storage->gi_probe_data_owner.getornull(p_data);
		ERR_FAIL_COND(!gipd);
...
void RasterizerSceneGLES4::gi_probe_instance_set_transform_to_data(RID p_probe, const Transform &p_xform) {
	GIProbeInstance *gipi = gi_probe_instance_owner.getornull(p_probe);
	ERR_FAIL_COND(!gipi);
	gipi->transform_to_data = p_xform;
}

void RasterizerSceneGLES4::gi_probe_instance_set_bounds(RID p_probe, const Vector3 &p_bounds) {
	GIProbeInstance *gipi = gi_probe_instance_owner.getornull(p_probe);
	ERR_FAIL_COND(!gipi);
...
bool RasterizerSceneGLES4::_setup_material(RasterizerStorageGLES4::Material *p_material, bool p_depth_pass, bool p_alpha_pass) {
	/* this is handled outside
	if (p_material->shader->spatial.cull_mode == RasterizerStorageGLES4::Shader::Spatial::CULL_MODE_DISABLED) {
		glDisable(GL_CULL_FACE);
	} else {
	...
	if (state.current_depth_draw != p_material->shader->spatial.depth_draw_mode) {
		switch (p_material->shader->spatial.depth_draw_mode) {
			case RasterizerStorageGLES4::Shader::Spatial::DEPTH_DRAW_ALPHA_PREPASS: {
			...
			case RasterizerStorageGLES4::Shader::Spatial::DEPTH_DRAW_OPAQUE: {
				glDepthMask(!p_alpha_pass);
			} break;
			case RasterizerStorageGLES4::Shader::Spatial::DEPTH_DRAW_ALWAYS: {
				glDepthMask(GL_TRUE);
				// If some transparent objects write to depth, we need to re-copy depth texture when we need it
				if (p_alpha_pass) {
					state.prepared_depth_texture = false;
			...
			case RasterizerStorageGLES4::Shader::Spatial::DEPTH_DRAW_NEVER: {
	...
	for (int i = 0; i < tc; i++) {
		...
		RasterizerStorageGLES4::Texture *t = storage->texture_owner.getptr(textures[i]);
...
struct RasterizerGLES4Particle {
	float color[4];
...
struct RasterizerGLES4ParticleSort {
	Vector3 z_dir;
	bool operator()(const RasterizerGLES4Particle &p_a, const RasterizerGLES4Particle &p_b) const {
...
void RasterizerSceneGLES4::_setup_geometry(RenderList::Element *e, const Transform &p_view_transform) {
	switch (e->instance->base_type) {
		case VS::INSTANCE_MESH: {
			RasterizerStorageGLES4::Surface *s = static_cast<RasterizerStorageGLES4::Surface *>(e->geometry);
		...
		case VS::INSTANCE_MULTIMESH: {
			RasterizerStorageGLES4::MultiMesh *multi_mesh = static_cast<RasterizerStorageGLES4::MultiMesh *>(e->owner);
			RasterizerStorageGLES4::Surface *s = static_cast<RasterizerStorageGLES4::Surface *>(e->geometry);
#ifdef DEBUG_ENABLED
		...
		case VS::INSTANCE_PARTICLES: {
			RasterizerStorageGLES4::Particles *particles = static_cast<RasterizerStorageGLES4::Particles *>(e->owner);
			RasterizerStorageGLES4::Surface *s = static_cast<RasterizerStorageGLES4::Surface *>(e->geometry);
			...
			if (particles->draw_order == VS::PARTICLES_DRAW_ORDER_VIEW_DEPTH && particles->particle_valid_histories[1]) {
				...
				RasterizerGLES4Particle *particle_array;
#ifndef __EMSCRIPTEN__
				particle_array = static_cast<RasterizerGLES4Particle *>(glMapBufferRange(GL_ARRAY_BUFFER, 0, particles->amount * 24 * sizeof(float), GL_MAP_READ_BIT | GL_MAP_WRITE_BIT));
#else
				PoolVector<RasterizerGLES4Particle> particle_vector;
				particle_vector.resize(particles->amount);
				PoolVector<RasterizerGLES4Particle>::Write particle_writer = particle_vector.write();
				particle_array = particle_writer.ptr();
				glGetBufferSubData(GL_ARRAY_BUFFER, 0, particles->amount * sizeof(RasterizerGLES4Particle), particle_array);
#endif

				SortArray<RasterizerGLES4Particle, RasterizerGLES4ParticleSort> sorter;
				...
				particle_array = NULL;
				{
					PoolVector<RasterizerGLES4Particle>::Read r = particle_vector.read();
					glBufferSubData(GL_ARRAY_BUFFER, 0, particles->amount * sizeof(RasterizerGLES4Particle), r.ptr());
				}
				particle_vector = PoolVector<RasterizerGLES4Particle>();
#endif
#ifdef DEBUG_ENABLED
				if (state.debug_draw == VS::VIEWPORT_DEBUG_DRAW_WIREFRAME && s->instancing_array_wireframe_id) {
...
const GLenum RasterizerSceneGLES4::gl_primitive[] = {
	GL_POINTS,
	GL_LINES,
	GL_LINE_STRIP,
	GL_LINE_LOOP,
...
void RasterizerSceneGLES4::_render_geometry(RenderList::Element *e) {
	switch (e->instance->base_type) {
		case VS::INSTANCE_MESH: {
			RasterizerStorageGLES4::Surface *s = static_cast<RasterizerStorageGLES4::Surface *>(e->geometry);
		...
		case VS::INSTANCE_IMMEDIATE: {
			...
			const RasterizerStorageGLES4::Immediate *im = static_cast<const RasterizerStorageGLES4::Immediate *>(e->geometry);
			...
			for (const List<RasterizerStorageGLES4::Immediate::Chunk>::Element *E = im->chunks.front(); E; E = E->next()) {
				const RasterizerStorageGLES4::Immediate::Chunk &c = E->get();
				if (c.vertices.empty()) {
					continue;
				}
				...
				if (c.texture.is_valid() && storage->texture_owner.owns(c.texture)) {
					RasterizerStorageGLES4::Texture *t = storage->texture_owner.get(c.texture);
		...
		case VS::INSTANCE_PARTICLES: {
			...
			if (!particles->use_local_coords) { //not using local coordinates? then clear transform..
				state.scene_shader.set_uniform(SceneShaderGLES4::WORLD_TRANSFORM, Transform());
			}
...
void RasterizerSceneGLES4::_setup_light(RenderList::Element *e, const Transform &p_view_transform) {
	...
	state.scene_shader.set_uniform(SceneShaderGLES4::OMNI_LIGHT_COUNT, omni_count);

	if (omni_count) {
		glUniform1iv(state.scene_shader.get_uniform(SceneShaderGLES4::OMNI_LIGHT_INDICES), omni_count, omni_indices);
	}

	state.scene_shader.set_uniform(SceneShaderGLES4::SPOT_LIGHT_COUNT, spot_count);
	if (spot_count) {
		glUniform1iv(state.scene_shader.get_uniform(SceneShaderGLES4::SPOT_LIGHT_INDICES), spot_count, spot_indices);
	}
	...
	state.scene_shader.set_uniform(SceneShaderGLES4::REFLECTION_COUNT, reflection_count);
	if (reflection_count) {
		glUniform1iv(state.scene_shader.get_uniform(SceneShaderGLES4::REFLECTION_INDICES), reflection_count, reflection_indices);
	}
	...
	if (gi_probe_count) {
		...
		state.scene_shader.set_uniform(SceneShaderGLES4::GI_PROBE_XFORM1, gipi->transform_to_data * p_view_transform);
		state.scene_shader.set_uniform(SceneShaderGLES4::GI_PROBE_BOUNDS1, gipi->bounds);
		state.scene_shader.set_uniform(SceneShaderGLES4::GI_PROBE_MULTIPLIER1, gipi->probe ? gipi->probe->dynamic_range * gipi->probe->energy : 0.0);
		state.scene_shader.set_uniform(SceneShaderGLES4::GI_PROBE_BIAS1, gipi->probe ? gipi->probe->bias * bias_scale : 0.0);
		state.scene_shader.set_uniform(SceneShaderGLES4::GI_PROBE_NORMAL_BIAS1, gipi->probe ? gipi->probe->normal_bias * bias_scale : 0.0);
		state.scene_shader.set_uniform(SceneShaderGLES4::GI_PROBE_BLEND_AMBIENT1, gipi->probe ? !gipi->probe->interior : false);
		state.scene_shader.set_uniform(SceneShaderGLES4::GI_PROBE_CELL_SIZE1, gipi->cell_size_cache);
		if (gi_probe_count > 1) {
			...
			state.scene_shader.set_uniform(SceneShaderGLES4::GI_PROBE_XFORM2, gipi2->transform_to_data * p_view_transform);
			state.scene_shader.set_uniform(SceneShaderGLES4::GI_PROBE_BOUNDS2, gipi2->bounds);
			state.scene_shader.set_uniform(SceneShaderGLES4::GI_PROBE_CELL_SIZE2, gipi2->cell_size_cache);
			state.scene_shader.set_uniform(SceneShaderGLES4::GI_PROBE_MULTIPLIER2, gipi2->probe ? gipi2->probe->dynamic_range * gipi2->probe->energy : 0.0);
			state.scene_shader.set_uniform(SceneShaderGLES4::GI_PROBE_BIAS2, gipi2->probe ? gipi2->probe->bias * bias_scale : 0.0);
			state.scene_shader.set_uniform(SceneShaderGLES4::GI_PROBE_NORMAL_BIAS2, gipi2->probe ? gipi2->probe->normal_bias * bias_scale : 0.0);
			state.scene_shader.set_uniform(SceneShaderGLES4::GI_PROBE_BLEND_AMBIENT2, gipi2->probe ? !gipi2->probe->interior : false);
			state.scene_shader.set_uniform(SceneShaderGLES4::GI_PROBE2_ENABLED, true);
		} else {
			state.scene_shader.set_uniform(SceneShaderGLES4::GI_PROBE2_ENABLED, false);
		}
	} else if (!e->instance->lightmap_capture_data.empty()) {
		glUniform4fv(state.scene_shader.get_uniform_location(SceneShaderGLES4::LIGHTMAP_CAPTURES), 12, (const GLfloat *)e->instance->lightmap_capture_data.ptr());

	} else if (e->instance->lightmap.is_valid()) {
		RasterizerStorageGLES4::Texture *lightmap = storage->texture_owner.getornull(e->instance->lightmap);
		RasterizerStorageGLES4::LightmapCapture *capture = storage->lightmap_capture_data_owner.getornull(e->instance->lightmap_capture->base);
		...
		if (lightmap && capture) {
			if (e->instance->lightmap_slice == -1) {
				...
				state.scene_shader.set_uniform(SceneShaderGLES4::LIGHTMAP_LAYER, e->instance->lightmap_slice);
			}
			const Rect2 &uvr = e->instance->lightmap_uv_rect;
			state.scene_shader.set_uniform(SceneShaderGLES4::LIGHTMAP_UV_RECT, Color(uvr.get_position().x, uvr.get_position().y, uvr.get_size().x, uvr.get_size().y));
			if (storage->config.use_lightmap_filter_bicubic) {
				state.scene_shader.set_uniform(SceneShaderGLES4::LIGHTMAP_TEXTURE_SIZE, Vector2(lightmap->width, lightmap->height));
			}
			state.scene_shader.set_uniform(SceneShaderGLES4::LIGHTMAP_ENERGY, capture->energy);
		}
	}
}

void RasterizerSceneGLES4::_set_cull(bool p_front, bool p_disabled, bool p_reverse_cull) {
	bool front = p_front;
	if (p_reverse_cull) {
		front = !front;
	}
...
void RasterizerSceneGLES4::_render_list(RenderList::Element **p_elements, int p_element_count, const Transform &p_view_transform, const CameraMatrix &p_projection, RasterizerStorageGLES4::Sky *p_sky, bool p_reverse_cull, bool p_alpha_pass, bool p_shadow, bool p_directional_add, bool p_directional_shadows) {
	...
	if (!p_shadow && !p_directional_add) {
		...
		if (p_sky != nullptr) {
			...
			state.scene_shader.set_conditional(SceneShaderGLES4::USE_RADIANCE_MAP, true);
			state.scene_shader.set_conditional(SceneShaderGLES4::USE_RADIANCE_MAP_ARRAY, storage->config.use_texture_array_environment);
			use_radiance_map = true;
		} else {
			state.scene_shader.set_conditional(SceneShaderGLES4::USE_RADIANCE_MAP, false);
			state.scene_shader.set_conditional(SceneShaderGLES4::USE_RADIANCE_MAP_ARRAY, false);
		}
	} else {
		state.scene_shader.set_conditional(SceneShaderGLES4::USE_RADIANCE_MAP, false);
		state.scene_shader.set_conditional(SceneShaderGLES4::USE_RADIANCE_MAP_ARRAY, false);
	...
	RasterizerStorageGLES4::Material *prev_material = nullptr;
	RasterizerStorageGLES4::Geometry *prev_geometry = nullptr;
	RasterizerStorageGLES4::GeometryOwner *prev_owner = nullptr;
	VS::InstanceType prev_base_type = VS::INSTANCE_MAX;

	int current_blend_mode = -1;

	uint32_t prev_shading = 0xFFFFFFFF;
	state.scene_shader.set_conditional(SceneShaderGLES4::SHADELESS, true); //by default unshaded (easier to set)

	RasterizerStorageGLES4::Skeleton *prev_skeleton = nullptr;
	state.scene_shader.set_conditional(SceneShaderGLES4::USE_SKELETON, false);

	bool first = true;
	bool prev_use_instancing = false;
	state.scene_shader.set_conditional(SceneShaderGLES4::USE_INSTANCING, false);
	bool prev_octahedral_compression = false;
	state.scene_shader.set_conditional(SceneShaderGLES4::ENABLE_OCTAHEDRAL_COMPRESSION, false);

	storage->info.render.draw_call_count += p_element_count;
	bool prev_opaque_prepass = false;
	state.scene_shader.set_conditional(SceneShaderGLES4::USE_OPAQUE_PREPASS, false);

	for (int i = 0; i < p_element_count; i++) {
		RenderList::Element *e = p_elements[i];
		RasterizerStorageGLES4::Material *material = e->material;
		RasterizerStorageGLES4::Skeleton *skeleton = nullptr;
		...
		if (!p_shadow) {
			...
			if (shading != prev_shading) {
				if (e->sort_key & SORT_KEY_UNSHADED_FLAG) {
					state.scene_shader.set_conditional(SceneShaderGLES4::SHADELESS, true);
					state.scene_shader.set_conditional(SceneShaderGLES4::USE_FORWARD_LIGHTING, false);
					state.scene_shader.set_conditional(SceneShaderGLES4::USE_VERTEX_LIGHTING, false);
					state.scene_shader.set_conditional(SceneShaderGLES4::USE_LIGHT_DIRECTIONAL, false);
					state.scene_shader.set_conditional(SceneShaderGLES4::LIGHT_DIRECTIONAL_SHADOW, false);
					state.scene_shader.set_conditional(SceneShaderGLES4::LIGHT_USE_PSSM4, false);
					state.scene_shader.set_conditional(SceneShaderGLES4::LIGHT_USE_PSSM3, false);
					state.scene_shader.set_conditional(SceneShaderGLES4::LIGHT_USE_PSSM2, false);
					state.scene_shader.set_conditional(SceneShaderGLES4::LIGHT_USE_PSSM_BLEND, false);
					state.scene_shader.set_conditional(SceneShaderGLES4::SHADOW_MODE_PCF_5, false);
					state.scene_shader.set_conditional(SceneShaderGLES4::SHADOW_MODE_PCF_13, false);
					state.scene_shader.set_conditional(SceneShaderGLES4::USE_GI_PROBES, false);
					state.scene_shader.set_conditional(SceneShaderGLES4::USE_LIGHTMAP_CAPTURE, false);
					state.scene_shader.set_conditional(SceneShaderGLES4::USE_LIGHTMAP, false);
					state.scene_shader.set_conditional(SceneShaderGLES4::USE_LIGHTMAP_LAYERED, false);
					state.scene_shader.set_conditional(SceneShaderGLES4::USE_RADIANCE_MAP, false);
					state.scene_shader.set_conditional(SceneShaderGLES4::USE_CONTACT_SHADOWS, false);

					//state.scene_shader.set_conditional(SceneShaderGLES4::SHADELESS,true);
				} else {
					state.scene_shader.set_conditional(SceneShaderGLES4::USE_GI_PROBES, e->instance->gi_probe_instances.size() > 0);
					state.scene_shader.set_conditional(SceneShaderGLES4::USE_LIGHTMAP, e->instance->lightmap.is_valid() && e->instance->gi_probe_instances.size() == 0);
					state.scene_shader.set_conditional(SceneShaderGLES4::USE_LIGHTMAP_LAYERED, e->instance->lightmap_slice != -1);
					state.scene_shader.set_conditional(SceneShaderGLES4::USE_LIGHTMAP_CAPTURE, !e->instance->lightmap_capture_data.empty() && !e->instance->lightmap.is_valid() && e->instance->gi_probe_instances.size() == 0);

					state.scene_shader.set_conditional(SceneShaderGLES4::SHADELESS, false);

					state.scene_shader.set_conditional(SceneShaderGLES4::USE_FORWARD_LIGHTING, !p_directional_add);
					state.scene_shader.set_conditional(SceneShaderGLES4::USE_VERTEX_LIGHTING, (e->sort_key & SORT_KEY_VERTEX_LIT_FLAG));

					state.scene_shader.set_conditional(SceneShaderGLES4::USE_LIGHT_DIRECTIONAL, use_directional);
					state.scene_shader.set_conditional(SceneShaderGLES4::LIGHT_DIRECTIONAL_SHADOW, false);
					state.scene_shader.set_conditional(SceneShaderGLES4::LIGHT_USE_PSSM4, false);
					state.scene_shader.set_conditional(SceneShaderGLES4::LIGHT_USE_PSSM3, false);
					state.scene_shader.set_conditional(SceneShaderGLES4::LIGHT_USE_PSSM2, false);
					state.scene_shader.set_conditional(SceneShaderGLES4::LIGHT_USE_PSSM_BLEND, false);
					state.scene_shader.set_conditional(SceneShaderGLES4::SHADOW_MODE_PCF_5, shadow_filter_mode == SHADOW_FILTER_PCF5);
					state.scene_shader.set_conditional(SceneShaderGLES4::SHADOW_MODE_PCF_13, shadow_filter_mode == SHADOW_FILTER_PCF13);
					state.scene_shader.set_conditional(SceneShaderGLES4::USE_RADIANCE_MAP, use_radiance_map);
					state.scene_shader.set_conditional(SceneShaderGLES4::USE_CONTACT_SHADOWS, state.used_contact_shadows);

					if (use_directional) {
						if (p_directional_shadows && directional_light->light_ptr->shadow) {
							state.scene_shader.set_conditional(SceneShaderGLES4::LIGHT_DIRECTIONAL_SHADOW, true);

							switch (directional_light->light_ptr->directional_shadow_mode) {
								case VS::LIGHT_DIRECTIONAL_SHADOW_ORTHOGONAL:
									break; //none
								case VS::LIGHT_DIRECTIONAL_SHADOW_PARALLEL_2_SPLITS:
									state.scene_shader.set_conditional(SceneShaderGLES4::LIGHT_USE_PSSM2, true);
									state.scene_shader.set_conditional(SceneShaderGLES4::LIGHT_USE_PSSM_BLEND, directional_light->light_ptr->directional_blend_splits);
									break;
								case VS::LIGHT_DIRECTIONAL_SHADOW_PARALLEL_3_SPLITS:
									state.scene_shader.set_conditional(SceneShaderGLES4::LIGHT_USE_PSSM3, true);
									state.scene_shader.set_conditional(SceneShaderGLES4::LIGHT_USE_PSSM_BLEND, directional_light->light_ptr->directional_blend_splits);
									break;
								case VS::LIGHT_DIRECTIONAL_SHADOW_PARALLEL_4_SPLITS:
									state.scene_shader.set_conditional(SceneShaderGLES4::LIGHT_USE_PSSM4, true);
									state.scene_shader.set_conditional(SceneShaderGLES4::LIGHT_USE_PSSM_BLEND, directional_light->light_ptr->directional_blend_splits);
									break;
							}
						}
			...
			if (p_alpha_pass || p_directional_add) {
				...
				if (p_directional_add) {
					desired_blend_mode = RasterizerStorageGLES4::Shader::Spatial::BLEND_MODE_ADD;
				} else {
				...
				if (desired_blend_mode != current_blend_mode) {
					switch (desired_blend_mode) {
						case RasterizerStorageGLES4::Shader::Spatial::BLEND_MODE_MIX: {
						...
						case RasterizerStorageGLES4::Shader::Spatial::BLEND_MODE_ADD: {
							glBlendEquation(GL_FUNC_ADD);
							glBlendFunc(p_alpha_pass ? GL_SRC_ALPHA : GL_ONE, GL_ONE);

						} break;
						case RasterizerStorageGLES4::Shader::Spatial::BLEND_MODE_SUB: {
							glBlendEquation(GL_FUNC_REVERSE_SUBTRACT);
							glBlendFunc(GL_SRC_ALPHA, GL_ONE);
						} break;
						case RasterizerStorageGLES4::Shader::Spatial::BLEND_MODE_MUL: {
							glBlendEquation(GL_FUNC_ADD);
							if (storage->frame.current_rt && storage->frame.current_rt->flags[RasterizerStorage::RENDER_TARGET_TRANSPARENT]) {
		...
		if (use_opaque_prepass != prev_opaque_prepass) {
			state.scene_shader.set_conditional(SceneShaderGLES4::USE_OPAQUE_PREPASS, use_opaque_prepass);
			rebind = true;
		}
		...
		if (use_instancing != prev_use_instancing) {
			state.scene_shader.set_conditional(SceneShaderGLES4::USE_INSTANCING, use_instancing);
			rebind = true;
		}

		if (prev_skeleton != skeleton) {
			if ((prev_skeleton == nullptr) != (skeleton == nullptr)) {
				state.scene_shader.set_conditional(SceneShaderGLES4::USE_SKELETON, skeleton != nullptr);
				rebind = true;
			}
		...
		state.scene_shader.set_conditional(SceneShaderGLES4::USE_PHYSICAL_LIGHT_ATTENUATION, storage->config.use_physical_light_attenuation);

		bool octahedral_compression = e->instance->base_type != VS::INSTANCE_IMMEDIATE &&
				((RasterizerStorageGLES4::Surface *)e->geometry)->format & VisualServer::ArrayFormat::ARRAY_FLAG_USE_OCTAHEDRAL_COMPRESSION &&
				!(((RasterizerStorageGLES4::Surface *)e->geometry)->blend_shapes.size() && e->instance->blend_values.size());
		if (octahedral_compression != prev_octahedral_compression) {
			state.scene_shader.set_conditional(SceneShaderGLES4::ENABLE_OCTAHEDRAL_COMPRESSION, octahedral_compression);
			rebind = true;
		}
		...
		state.scene_shader.set_uniform(SceneShaderGLES4::WORLD_TRANSFORM, e->instance->transform);
...
void RasterizerSceneGLES4::_add_geometry(RasterizerStorageGLES4::Geometry *p_geometry, InstanceBase *p_instance, RasterizerStorageGLES4::GeometryOwner *p_owner, int p_material, bool p_depth_pass, bool p_shadow_pass) {
	RasterizerStorageGLES4::Material *m = nullptr;
...
void RasterizerSceneGLES4::_add_geometry_with_material(RasterizerStorageGLES4::Geometry *p_geometry, InstanceBase *p_instance, RasterizerStorageGLES4::GeometryOwner *p_owner, RasterizerStorageGLES4::Material *p_material, bool p_depth_pass, bool p_shadow_pass) {
	...
	bool has_blend_alpha = p_material->shader->spatial.blend_mode != RasterizerStorageGLES4::Shader::Spatial::BLEND_MODE_MIX;
	...
	if (p_material->shader->spatial.cull_mode == RasterizerStorageGLES4::Shader::Spatial::CULL_MODE_DISABLED) {
		no_cull = true;
		mirror = false;
	} else if (p_material->shader->spatial.cull_mode == RasterizerStorageGLES4::Shader::Spatial::CULL_MODE_FRONT) {
		mirror = !mirror;
	}
	...
	if (p_depth_pass) {
		...
		if (!p_material->shader->spatial.uses_alpha_scissor && !p_material->shader->spatial.writes_modelview_or_projection && !p_material->shader->spatial.uses_vertex && !p_material->shader->spatial.uses_discard && p_material->shader->spatial.depth_draw_mode != RasterizerStorageGLES4::Shader::Spatial::DEPTH_DRAW_ALPHA_PREPASS) {
	...
	if (first_directional_light.is_valid() && light_instance_owner.owns(first_directional_light)) {
		RasterizerStorageGLES4::Light *directional = light_instance_owner.getptr(first_directional_light)->light_ptr;
	...
	/*
	if (e->geometry->type==RasterizerStorageGLES4::Geometry::GEOMETRY_MULTISURFACE)
	...
	*/
	...
	if (p_depth_pass && p_material->shader->spatial.depth_draw_mode == RasterizerStorageGLES4::Shader::Spatial::DEPTH_DRAW_ALPHA_PREPASS) {
...
void RasterizerSceneGLES4::_draw_sky(RasterizerStorageGLES4::Sky *p_sky, const CameraMatrix &p_projection, const Transform &p_transform, bool p_vflip, float p_custom_fov, float p_energy, const Basis &p_sky_orientation) {
	ERR_FAIL_COND(!p_sky);

	RasterizerStorageGLES4::Texture *tex = storage->texture_owner.getornull(p_sky->panorama);
	...
	*/
	...
	storage->shaders.copy.set_conditional(CopyShaderGLES4::USE_ASYM_PANO, asymmetrical);
	storage->shaders.copy.set_conditional(CopyShaderGLES4::USE_PANORAMA, !asymmetrical);
	storage->shaders.copy.set_conditional(CopyShaderGLES4::USE_MULTIPLIER, true);
	storage->shaders.copy.bind();

	storage->shaders.copy.set_uniform(CopyShaderGLES4::MULTIPLIER, p_energy);

	// don't know why but I always have problems setting a uniform mat3, so we're using a transform
	storage->shaders.copy.set_uniform(CopyShaderGLES4::SKY_TRANSFORM, Transform(p_sky_orientation, Vector3(0.0, 0.0, 0.0)).affine_inverse());

	if (asymmetrical) {
		// pack the bits we need from our projection matrix
		storage->shaders.copy.set_uniform(CopyShaderGLES4::ASYM_PROJ, camera.matrix[2][0], camera.matrix[0][0], camera.matrix[2][1], camera.matrix[1][1]);
		///@TODO I couldn't get mat3 + p_transform.basis to work, that would be better here.
		storage->shaders.copy.set_uniform(CopyShaderGLES4::PANO_TRANSFORM, p_transform);
	}
	...
	storage->shaders.copy.set_conditional(CopyShaderGLES4::USE_ASYM_PANO, false);
	storage->shaders.copy.set_conditional(CopyShaderGLES4::USE_MULTIPLIER, false);
	storage->shaders.copy.set_conditional(CopyShaderGLES4::USE_PANORAMA, false);
}

void RasterizerSceneGLES4::_setup_environment(Environment *env, const CameraMatrix &p_cam_projection, const Transform &p_cam_transform, const int p_eye, bool p_no_fog) {
...
void RasterizerSceneGLES4::_setup_directional_light(int p_index, const Transform &p_camera_inverse_transform, bool p_use_shadows) {
...
void RasterizerSceneGLES4::_setup_lights(RID *p_light_cull_result, int p_light_cull_count, const Transform &p_camera_inverse_transform, const CameraMatrix &p_camera_projection, RID p_shadow_atlas) {
...
void RasterizerSceneGLES4::_setup_reflections(RID *p_reflection_probe_cull_result, int p_reflection_probe_cull_count, const Transform &p_camera_inverse_transform, const CameraMatrix &p_camera_projection, RID p_reflection_atlas, Environment *p_env) {
...
void RasterizerSceneGLES4::_copy_screen(bool p_invalidate_color, bool p_invalidate_depth) {
#ifndef GLES_OVER_GL
...
void RasterizerSceneGLES4::_copy_texture_to_front_buffer(GLuint p_texture) {
	...
	storage->shaders.copy.set_conditional(CopyShaderGLES4::DISABLE_ALPHA, true);
	storage->shaders.copy.bind();

	_copy_screen();

	//turn off everything used
	storage->shaders.copy.set_conditional(CopyShaderGLES4::LINEAR_TO_SRGB, false);
	storage->shaders.copy.set_conditional(CopyShaderGLES4::DISABLE_ALPHA, false);
}

void RasterizerSceneGLES4::_fill_render_list(InstanceBase **p_cull_result, int p_cull_count, bool p_depth_pass, bool p_shadow_pass) {
	...
	for (int i = 0; i < p_cull_count; i++) {
		...
		switch (inst->base_type) {
			case VS::INSTANCE_MESH: {
				RasterizerStorageGLES4::Mesh *mesh = storage->mesh_owner.getptr(inst->base);
				ERR_CONTINUE(!mesh);
				...
				for (int j = 0; j < ssize; j++) {
					...
					RasterizerStorageGLES4::Surface *s = mesh->surfaces[j];
			...
			case VS::INSTANCE_MULTIMESH: {
				RasterizerStorageGLES4::MultiMesh *multi_mesh = storage->multimesh_owner.getptr(inst->base);
				...
				RasterizerStorageGLES4::Mesh *mesh = storage->mesh_owner.getptr(multi_mesh->mesh);
				if (!mesh) {
			...
			case VS::INSTANCE_IMMEDIATE: {
				RasterizerStorageGLES4::Immediate *immediate = storage->immediate_owner.getptr(inst->base);
			...
			case VS::INSTANCE_PARTICLES: {
				RasterizerStorageGLES4::Particles *particles = storage->particles_owner.getptr(inst->base);
				...
				for (int j = 0; j < particles->draw_passes.size(); j++) {
					...
					RasterizerStorageGLES4::Mesh *mesh = storage->mesh_owner.get(pmesh);
					if (!mesh) {
						continue; //mesh not assigned
					}
					...
					for (int k = 0; k < ssize; k++) {
						RasterizerStorageGLES4::Surface *s = mesh->surfaces[k];
...
void RasterizerSceneGLES4::_blur_effect_buffer() {
	//blur diffuse into effect mipmaps using separatable convolution
	//storage->shaders.copy.set_conditional(CopyShaderGLES4::GAUSSIAN_HORIZONTAL,true);
	for (int i = 0; i < storage->frame.current_rt->effects.mip_maps[1].sizes.size(); i++) {
		...
		//horizontal pass
		state.effect_blur_shader.set_conditional(EffectBlurShaderGLES4::GAUSSIAN_HORIZONTAL, true);
		state.effect_blur_shader.bind();
		state.effect_blur_shader.set_uniform(EffectBlurShaderGLES4::PIXEL_SIZE, Vector2(1.0 / vp_w, 1.0 / vp_h));
		state.effect_blur_shader.set_uniform(EffectBlurShaderGLES4::LOD, float(i));
		WRAPPED_GL_ACTIVE_TEXTURE(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, storage->frame.current_rt->effects.mip_maps[0].color); //previous level, since mipmaps[0] starts one level bigger
		glBindFramebuffer(GL_FRAMEBUFFER, storage->frame.current_rt->effects.mip_maps[1].sizes[i].fbo);
		_copy_screen(true);
		state.effect_blur_shader.set_conditional(EffectBlurShaderGLES4::GAUSSIAN_HORIZONTAL, false);

		//vertical pass
		state.effect_blur_shader.set_conditional(EffectBlurShaderGLES4::GAUSSIAN_VERTICAL, true);
		state.effect_blur_shader.bind();
		state.effect_blur_shader.set_uniform(EffectBlurShaderGLES4::PIXEL_SIZE, Vector2(1.0 / vp_w, 1.0 / vp_h));
		state.effect_blur_shader.set_uniform(EffectBlurShaderGLES4::LOD, float(i));
		WRAPPED_GL_ACTIVE_TEXTURE(GL_TEXTURE0);
		...
		_copy_screen(true);
		state.effect_blur_shader.set_conditional(EffectBlurShaderGLES4::GAUSSIAN_VERTICAL, false);
	}
}

void RasterizerSceneGLES4::_prepare_depth_texture() {
...
void RasterizerSceneGLES4::_bind_depth_texture() {
...
void RasterizerSceneGLES4::_render_mrts(Environment *env, const CameraMatrix &p_cam_projection) {
	glDepthMask(GL_FALSE);
	glDisable(GL_DEPTH_TEST);
	glDisable(GL_CULL_FACE);
	glDisable(GL_BLEND);
	...
	if (env->ssao_enabled) {
		...
		for (int i = 0; i < storage->frame.current_rt->effects.ssao.depth_mipmap_fbos.size(); i++) {
			state.ssao_minify_shader.set_conditional(SsaoMinifyShaderGLES4::MINIFY_START, i == 0);
			state.ssao_minify_shader.set_conditional(SsaoMinifyShaderGLES4::USE_ORTHOGONAL_PROJECTION, p_cam_projection.is_orthogonal());
			state.ssao_minify_shader.bind();
			state.ssao_minify_shader.set_uniform(SsaoMinifyShaderGLES4::CAMERA_Z_FAR, p_cam_projection.get_z_far());
			state.ssao_minify_shader.set_uniform(SsaoMinifyShaderGLES4::CAMERA_Z_NEAR, p_cam_projection.get_z_near());
			state.ssao_minify_shader.set_uniform(SsaoMinifyShaderGLES4::SOURCE_MIPMAP, MAX(0, i - 1));
			glUniform2iv(state.ssao_minify_shader.get_uniform(SsaoMinifyShaderGLES4::FROM_SIZE), 1, ss);
			ss[0] >>= 1;
			ss[1] >>= 1;
		...
		// do SSAO!
		state.ssao_shader.set_conditional(SsaoShaderGLES4::ENABLE_RADIUS2, env->ssao_radius2 > 0.001);
		state.ssao_shader.set_conditional(SsaoShaderGLES4::USE_ORTHOGONAL_PROJECTION, p_cam_projection.is_orthogonal());
		state.ssao_shader.set_conditional(SsaoShaderGLES4::SSAO_QUALITY_LOW, env->ssao_quality == VS::ENV_SSAO_QUALITY_LOW);
		state.ssao_shader.set_conditional(SsaoShaderGLES4::SSAO_QUALITY_HIGH, env->ssao_quality == VS::ENV_SSAO_QUALITY_HIGH);
		state.ssao_shader.bind();
		state.ssao_shader.set_uniform(SsaoShaderGLES4::CAMERA_Z_FAR, p_cam_projection.get_z_far());
		state.ssao_shader.set_uniform(SsaoShaderGLES4::CAMERA_Z_NEAR, p_cam_projection.get_z_near());
		glUniform2iv(state.ssao_shader.get_uniform(SsaoShaderGLES4::SCREEN_SIZE), 1, ss);
		float radius = env->ssao_radius;
		state.ssao_shader.set_uniform(SsaoShaderGLES4::RADIUS, radius);
		float intensity = env->ssao_intensity;
		state.ssao_shader.set_uniform(SsaoShaderGLES4::INTENSITY_DIV_R6, intensity / pow(radius, 6.0f));

		if (env->ssao_radius2 > 0.001) {
			float radius2 = env->ssao_radius2;
			state.ssao_shader.set_uniform(SsaoShaderGLES4::RADIUS2, radius2);
			float intensity2 = env->ssao_intensity2;
			state.ssao_shader.set_uniform(SsaoShaderGLES4::INTENSITY_DIV_R62, intensity2 / pow(radius2, 6.0f));
		}
		...
		glUniform4fv(state.ssao_shader.get_uniform(SsaoShaderGLES4::PROJ_INFO), 1, proj_info);
		float pixels_per_meter = float(p_cam_projection.get_pixels_per_meter(ss[0]));

		state.ssao_shader.set_uniform(SsaoShaderGLES4::PROJ_SCALE, pixels_per_meter);
		state.ssao_shader.set_uniform(SsaoShaderGLES4::BIAS, env->ssao_bias);
		...
		if (env->ssao_filter) {
			for (int i = 0; i < 2; i++) {
				state.ssao_blur_shader.set_uniform(SsaoBlurShaderGLES4::CAMERA_Z_FAR, p_cam_projection.get_z_far());
				state.ssao_blur_shader.set_uniform(SsaoBlurShaderGLES4::CAMERA_Z_NEAR, p_cam_projection.get_z_near());
				state.ssao_blur_shader.set_uniform(SsaoBlurShaderGLES4::EDGE_SHARPNESS, env->ssao_bilateral_sharpness);
				state.ssao_blur_shader.set_uniform(SsaoBlurShaderGLES4::FILTER_SCALE, int(env->ssao_filter));

				GLint axis[2] = { i, 1 - i };
				glUniform2iv(state.ssao_blur_shader.get_uniform(SsaoBlurShaderGLES4::AXIS), 1, axis);
				glUniform2iv(state.ssao_blur_shader.get_uniform(SsaoBlurShaderGLES4::SCREEN_SIZE), 1, ss);
		...
		state.effect_blur_shader.set_conditional(EffectBlurShaderGLES4::SSAO_MERGE, true);
		state.effect_blur_shader.bind();
		state.effect_blur_shader.set_uniform(EffectBlurShaderGLES4::SSAO_COLOR, env->ssao_color);
		WRAPPED_GL_ACTIVE_TEXTURE(GL_TEXTURE0);
		...
		_copy_screen(true);
		state.effect_blur_shader.set_conditional(EffectBlurShaderGLES4::SSAO_MERGE, false);
	...
	if (state.used_sss) { //sss enabled
		...
		state.sss_shader.set_conditional(SubsurfScatteringShaderGLES4::USE_ORTHOGONAL_PROJECTION, p_cam_projection.is_orthogonal());
		state.sss_shader.set_conditional(SubsurfScatteringShaderGLES4::USE_11_SAMPLES, subsurface_scatter_quality == SSS_QUALITY_LOW);
		state.sss_shader.set_conditional(SubsurfScatteringShaderGLES4::USE_17_SAMPLES, subsurface_scatter_quality == SSS_QUALITY_MEDIUM);
		state.sss_shader.set_conditional(SubsurfScatteringShaderGLES4::USE_25_SAMPLES, subsurface_scatter_quality == SSS_QUALITY_HIGH);
		state.sss_shader.set_conditional(SubsurfScatteringShaderGLES4::ENABLE_FOLLOW_SURFACE, subsurface_scatter_follow_surface);
		state.sss_shader.set_conditional(SubsurfScatteringShaderGLES4::ENABLE_STRENGTH_WEIGHTING, subsurface_scatter_weight_samples);
		state.sss_shader.bind();
		state.sss_shader.set_uniform(SubsurfScatteringShaderGLES4::MAX_RADIUS, subsurface_scatter_size);
		state.sss_shader.set_uniform(SubsurfScatteringShaderGLES4::UNIT_SIZE, unit_size);
		state.sss_shader.set_uniform(SubsurfScatteringShaderGLES4::CAMERA_Z_NEAR, p_cam_projection.get_z_near());
		state.sss_shader.set_uniform(SubsurfScatteringShaderGLES4::CAMERA_Z_FAR, p_cam_projection.get_z_far());
		state.sss_shader.set_uniform(SubsurfScatteringShaderGLES4::DIR, Vector2(1, 0));
		...
		state.sss_shader.set_uniform(SubsurfScatteringShaderGLES4::DIR, Vector2(0, 1));
		glBindFramebuffer(GL_FRAMEBUFFER, storage->frame.current_rt->effects.mip_maps[0].sizes[0].fbo); // copy to base level
		_copy_screen(true);
	...
	if (env->ssr_enabled) {
		...
		state.ssr_shader.set_conditional(ScreenSpaceReflectionShaderGLES4::REFLECT_ROUGHNESS, env->ssr_roughness);
		state.ssr_shader.set_conditional(ScreenSpaceReflectionShaderGLES4::USE_ORTHOGONAL_PROJECTION, p_cam_projection.is_orthogonal());
		...
		state.ssr_shader.set_uniform(ScreenSpaceReflectionShaderGLES4::PIXEL_SIZE, Vector2(1.0 / (ssr_w * 0.5), 1.0 / (ssr_h * 0.5)));
		state.ssr_shader.set_uniform(ScreenSpaceReflectionShaderGLES4::CAMERA_Z_NEAR, p_cam_projection.get_z_near());
		state.ssr_shader.set_uniform(ScreenSpaceReflectionShaderGLES4::CAMERA_Z_FAR, p_cam_projection.get_z_far());
		state.ssr_shader.set_uniform(ScreenSpaceReflectionShaderGLES4::PROJECTION, p_cam_projection);
		state.ssr_shader.set_uniform(ScreenSpaceReflectionShaderGLES4::INVERSE_PROJECTION, p_cam_projection.inverse());
		state.ssr_shader.set_uniform(ScreenSpaceReflectionShaderGLES4::VIEWPORT_SIZE, Size2(ssr_w, ssr_h));
		//state.ssr_shader.set_uniform(ScreenSpaceReflectionShaderGLES4::FRAME_INDEX,int(render_pass));
		state.ssr_shader.set_uniform(ScreenSpaceReflectionShaderGLES4::FILTER_MIPMAP_LEVELS, float(storage->frame.current_rt->effects.mip_maps[0].sizes.size()));
		state.ssr_shader.set_uniform(ScreenSpaceReflectionShaderGLES4::NUM_STEPS, env->ssr_max_steps);
		state.ssr_shader.set_uniform(ScreenSpaceReflectionShaderGLES4::DEPTH_TOLERANCE, env->ssr_depth_tolerance);
		state.ssr_shader.set_uniform(ScreenSpaceReflectionShaderGLES4::DISTANCE_FADE, env->ssr_fade_out);
		state.ssr_shader.set_uniform(ScreenSpaceReflectionShaderGLES4::CURVE_FADE_IN, env->ssr_fade_in);
	...
	state.resolve_shader.set_conditional(ResolveShaderGLES4::USE_SSR, env->ssr_enabled);
	state.resolve_shader.bind();
	state.resolve_shader.set_uniform(ResolveShaderGLES4::PIXEL_SIZE, Vector2(1.0 / storage->frame.current_rt->width, 1.0 / storage->frame.current_rt->height));
	...
	state.effect_blur_shader.set_conditional(EffectBlurShaderGLES4::SIMPLE_COPY, true);
	state.effect_blur_shader.bind();
	state.effect_blur_shader.set_uniform(EffectBlurShaderGLES4::LOD, float(0));
	...
	state.effect_blur_shader.set_conditional(EffectBlurShaderGLES4::SIMPLE_COPY, false);
}

void RasterizerSceneGLES4::_post_process(Environment *env, const CameraMatrix &p_cam_projection) {
	//copy to front buffer
	...
	if ((!env || storage->frame.current_rt->width < 4 || storage->frame.current_rt->height < 4) && !storage->frame.current_rt->use_fxaa && !storage->frame.current_rt->use_debanding && storage->frame.current_rt->sharpen_intensity < 0.001) { //no post process on small render targets
		...
		WRAPPED_GL_ACTIVE_TEXTURE(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, storage->frame.current_rt->effects.mip_maps[0].color);
		storage->shaders.copy.set_conditional(CopyShaderGLES4::LINEAR_TO_SRGB, !storage->frame.current_rt->flags[RasterizerStorage::RENDER_TARGET_KEEP_3D_LINEAR]);
		storage->shaders.copy.set_conditional(CopyShaderGLES4::V_FLIP, storage->frame.current_rt->flags[RasterizerStorage::RENDER_TARGET_VFLIP]);
		storage->shaders.copy.set_conditional(CopyShaderGLES4::DISABLE_ALPHA, !storage->frame.current_rt->flags[RasterizerStorage::RENDER_TARGET_TRANSPARENT]);
		storage->shaders.copy.bind();

		_copy_screen(true);

		storage->shaders.copy.set_conditional(CopyShaderGLES4::LINEAR_TO_SRGB, false);
		storage->shaders.copy.set_conditional(CopyShaderGLES4::DISABLE_ALPHA, false); //compute luminance
		storage->shaders.copy.set_conditional(CopyShaderGLES4::V_FLIP, false);
	...
	if (env && env->dof_blur_far_enabled) {
		...
		state.effect_blur_shader.set_conditional(EffectBlurShaderGLES4::USE_ORTHOGONAL_PROJECTION, p_cam_projection.is_orthogonal());
		state.effect_blur_shader.set_conditional(EffectBlurShaderGLES4::DOF_FAR_BLUR, true);
		state.effect_blur_shader.set_conditional(EffectBlurShaderGLES4::DOF_QUALITY_LOW, env->dof_blur_far_quality == VS::ENV_DOF_BLUR_QUALITY_LOW);
		state.effect_blur_shader.set_conditional(EffectBlurShaderGLES4::DOF_QUALITY_MEDIUM, env->dof_blur_far_quality == VS::ENV_DOF_BLUR_QUALITY_MEDIUM);
		state.effect_blur_shader.set_conditional(EffectBlurShaderGLES4::DOF_QUALITY_HIGH, env->dof_blur_far_quality == VS::ENV_DOF_BLUR_QUALITY_HIGH);
		...
		state.effect_blur_shader.set_uniform(EffectBlurShaderGLES4::DOF_BEGIN, env->dof_blur_far_distance);
		state.effect_blur_shader.set_uniform(EffectBlurShaderGLES4::DOF_END, env->dof_blur_far_distance + env->dof_blur_far_transition);
		state.effect_blur_shader.set_uniform(EffectBlurShaderGLES4::DOF_DIR, Vector2(1, 0));
		state.effect_blur_shader.set_uniform(EffectBlurShaderGLES4::DOF_RADIUS, radius);
		state.effect_blur_shader.set_uniform(EffectBlurShaderGLES4::PIXEL_SIZE, Vector2(1.0 / vp_w, 1.0 / vp_h));
		state.effect_blur_shader.set_uniform(EffectBlurShaderGLES4::CAMERA_Z_NEAR, p_cam_projection.get_z_near());
		state.effect_blur_shader.set_uniform(EffectBlurShaderGLES4::CAMERA_Z_FAR, p_cam_projection.get_z_far());
		...
		WRAPPED_GL_ACTIVE_TEXTURE(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, storage->frame.current_rt->color);
		state.effect_blur_shader.set_uniform(EffectBlurShaderGLES4::DOF_DIR, Vector2(0, 1));
		glBindFramebuffer(GL_FRAMEBUFFER, storage->frame.current_rt->effects.mip_maps[0].sizes[0].fbo); // copy to base level
		_copy_screen();

		state.effect_blur_shader.set_conditional(EffectBlurShaderGLES4::DOF_FAR_BLUR, false);
		state.effect_blur_shader.set_conditional(EffectBlurShaderGLES4::DOF_QUALITY_LOW, false);
		state.effect_blur_shader.set_conditional(EffectBlurShaderGLES4::DOF_QUALITY_MEDIUM, false);
		state.effect_blur_shader.set_conditional(EffectBlurShaderGLES4::DOF_QUALITY_HIGH, false);
		state.effect_blur_shader.set_conditional(EffectBlurShaderGLES4::USE_ORTHOGONAL_PROJECTION, false);
	...
	if (env && env->dof_blur_near_enabled) {
		...
		state.effect_blur_shader.set_conditional(EffectBlurShaderGLES4::USE_ORTHOGONAL_PROJECTION, p_cam_projection.is_orthogonal());
		state.effect_blur_shader.set_conditional(EffectBlurShaderGLES4::DOF_NEAR_BLUR, true);
		state.effect_blur_shader.set_conditional(EffectBlurShaderGLES4::DOF_NEAR_FIRST_TAP, true);

		state.effect_blur_shader.set_conditional(EffectBlurShaderGLES4::DOF_QUALITY_LOW, env->dof_blur_near_quality == VS::ENV_DOF_BLUR_QUALITY_LOW);
		state.effect_blur_shader.set_conditional(EffectBlurShaderGLES4::DOF_QUALITY_MEDIUM, env->dof_blur_near_quality == VS::ENV_DOF_BLUR_QUALITY_MEDIUM);
		state.effect_blur_shader.set_conditional(EffectBlurShaderGLES4::DOF_QUALITY_HIGH, env->dof_blur_near_quality == VS::ENV_DOF_BLUR_QUALITY_HIGH);
		...
		state.effect_blur_shader.set_uniform(EffectBlurShaderGLES4::DOF_BEGIN, env->dof_blur_near_distance);
		state.effect_blur_shader.set_uniform(EffectBlurShaderGLES4::DOF_END, env->dof_blur_near_distance - env->dof_blur_near_transition);
		state.effect_blur_shader.set_uniform(EffectBlurShaderGLES4::DOF_DIR, Vector2(1, 0));
		state.effect_blur_shader.set_uniform(EffectBlurShaderGLES4::DOF_RADIUS, radius);
		state.effect_blur_shader.set_uniform(EffectBlurShaderGLES4::PIXEL_SIZE, Vector2(1.0 / vp_w, 1.0 / vp_h));
		state.effect_blur_shader.set_uniform(EffectBlurShaderGLES4::CAMERA_Z_NEAR, p_cam_projection.get_z_near());
		...
		state.effect_blur_shader.set_conditional(EffectBlurShaderGLES4::DOF_NEAR_BLUR_MERGE, composite_from == storage->frame.current_rt->buffers.diffuse);
		state.effect_blur_shader.set_conditional(EffectBlurShaderGLES4::DOF_NEAR_FIRST_TAP, false);
		state.effect_blur_shader.bind();
		...
		state.effect_blur_shader.set_conditional(EffectBlurShaderGLES4::DOF_NEAR_BLUR, false);
		state.effect_blur_shader.set_conditional(EffectBlurShaderGLES4::DOF_NEAR_FIRST_TAP, false);
		state.effect_blur_shader.set_conditional(EffectBlurShaderGLES4::DOF_NEAR_BLUR_MERGE, false);
		state.effect_blur_shader.set_conditional(EffectBlurShaderGLES4::DOF_QUALITY_LOW, false);
		state.effect_blur_shader.set_conditional(EffectBlurShaderGLES4::DOF_QUALITY_MEDIUM, false);
		state.effect_blur_shader.set_conditional(EffectBlurShaderGLES4::DOF_QUALITY_HIGH, false);
		state.effect_blur_shader.set_conditional(EffectBlurShaderGLES4::USE_ORTHOGONAL_PROJECTION, false);
	...
	if (env && env->auto_exposure) {
		...
		state.exposure_shader.set_conditional(ExposureShaderGLES4::EXPOSURE_BEGIN, true);
		...
		glUniform2iv(state.exposure_shader.get_uniform(ExposureShaderGLES4::SOURCE_RENDER_SIZE), 1, ss);
		glUniform2iv(state.exposure_shader.get_uniform(ExposureShaderGLES4::TARGET_SIZE), 1, ds);
		WRAPPED_GL_ACTIVE_TEXTURE(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, composite_from);
		...
		state.exposure_shader.set_conditional(ExposureShaderGLES4::EXPOSURE_BEGIN, false);
		state.exposure_shader.bind();
		...
		state.exposure_shader.set_conditional(ExposureShaderGLES4::EXPOSURE_END, true);
		...
		if (tick_diff == 0 || tick_diff > 1000000) {
			state.exposure_shader.set_conditional(ExposureShaderGLES4::EXPOSURE_FORCE_SET, true);
		}
		...
		state.exposure_shader.set_uniform(ExposureShaderGLES4::EXPOSURE_ADJUST, env->auto_exposure_speed * (tick_diff / 1000000.0));
		state.exposure_shader.set_uniform(ExposureShaderGLES4::MAX_LUMINANCE, env->auto_exposure_max);
		state.exposure_shader.set_uniform(ExposureShaderGLES4::MIN_LUMINANCE, env->auto_exposure_min);

		_copy_screen(true);

		state.exposure_shader.set_conditional(ExposureShaderGLES4::EXPOSURE_FORCE_SET, false);
		state.exposure_shader.set_conditional(ExposureShaderGLES4::EXPOSURE_END, false);
	...
	if (env && env->glow_enabled) {
		...
		for (int i = 0; i < (max_glow_level + 1); i++) {
			...
			//horizontal pass
			if (i == 0) {
				state.effect_blur_shader.set_conditional(EffectBlurShaderGLES4::GLOW_FIRST_PASS, true);
				state.effect_blur_shader.set_conditional(EffectBlurShaderGLES4::GLOW_USE_AUTO_EXPOSURE, env->auto_exposure);
			}

			state.effect_blur_shader.set_conditional(EffectBlurShaderGLES4::GLOW_GAUSSIAN_HORIZONTAL, true);
			state.effect_blur_shader.set_conditional(EffectBlurShaderGLES4::USE_GLOW_HIGH_QUALITY, env->glow_high_quality);
			state.effect_blur_shader.bind();
			state.effect_blur_shader.set_uniform(EffectBlurShaderGLES4::PIXEL_SIZE, Vector2(1.0 / vp_w, 1.0 / vp_h));
			state.effect_blur_shader.set_uniform(EffectBlurShaderGLES4::LOD, float(i));
			state.effect_blur_shader.set_uniform(EffectBlurShaderGLES4::GLOW_STRENGTH, env->glow_strength);
			state.effect_blur_shader.set_uniform(EffectBlurShaderGLES4::LUMINANCE_CAP, env->glow_hdr_luminance_cap);

			WRAPPED_GL_ACTIVE_TEXTURE(GL_TEXTURE0);
			if (i == 0) {
				glBindTexture(GL_TEXTURE_2D, composite_from);

				state.effect_blur_shader.set_uniform(EffectBlurShaderGLES4::EXPOSURE, env->tone_mapper_exposure);
				if (env->auto_exposure) {
					state.effect_blur_shader.set_uniform(EffectBlurShaderGLES4::AUTO_EXPOSURE_GREY, env->auto_exposure_grey);
				}

				WRAPPED_GL_ACTIVE_TEXTURE(GL_TEXTURE1);
				glBindTexture(GL_TEXTURE_2D, storage->frame.current_rt->exposure.color);

				state.effect_blur_shader.set_uniform(EffectBlurShaderGLES4::GLOW_BLOOM, env->glow_bloom);
				state.effect_blur_shader.set_uniform(EffectBlurShaderGLES4::GLOW_HDR_THRESHOLD, env->glow_hdr_bleed_threshold);
				state.effect_blur_shader.set_uniform(EffectBlurShaderGLES4::GLOW_HDR_SCALE, env->glow_hdr_bleed_scale);
			...
			glBindFramebuffer(GL_FRAMEBUFFER, storage->frame.current_rt->effects.mip_maps[1].sizes[i].fbo);
			_copy_screen(true);
			state.effect_blur_shader.set_conditional(EffectBlurShaderGLES4::GLOW_GAUSSIAN_HORIZONTAL, false);
			state.effect_blur_shader.set_conditional(EffectBlurShaderGLES4::GLOW_FIRST_PASS, false);
			state.effect_blur_shader.set_conditional(EffectBlurShaderGLES4::GLOW_USE_AUTO_EXPOSURE, false);

			//vertical pass
			state.effect_blur_shader.set_conditional(EffectBlurShaderGLES4::GLOW_GAUSSIAN_VERTICAL, true);
			state.effect_blur_shader.bind();
			state.effect_blur_shader.set_uniform(EffectBlurShaderGLES4::PIXEL_SIZE, Vector2(1.0 / vp_w, 1.0 / vp_h));
			state.effect_blur_shader.set_uniform(EffectBlurShaderGLES4::LOD, float(i));
			state.effect_blur_shader.set_uniform(EffectBlurShaderGLES4::GLOW_STRENGTH, env->glow_strength);
			...
			glBindFramebuffer(GL_FRAMEBUFFER, storage->frame.current_rt->effects.mip_maps[0].sizes[i + 1].fbo); //next level, since mipmaps[0] starts one level bigger
			_copy_screen();
			state.effect_blur_shader.set_conditional(EffectBlurShaderGLES4::GLOW_GAUSSIAN_VERTICAL, false);
		}
	...
	glBindTexture(GL_TEXTURE_2D, composite_from);
	if (env) {
		state.tonemap_shader.set_conditional(TonemapShaderGLES4::USE_FILMIC_TONEMAPPER, env->tone_mapper == VS::ENV_TONE_MAPPER_FILMIC);
		state.tonemap_shader.set_conditional(TonemapShaderGLES4::USE_ACES_TONEMAPPER, env->tone_mapper == VS::ENV_TONE_MAPPER_ACES);
		state.tonemap_shader.set_conditional(TonemapShaderGLES4::USE_ACES_FITTED_TONEMAPPER, env->tone_mapper == VS::ENV_TONE_MAPPER_ACES_FITTED);
		state.tonemap_shader.set_conditional(TonemapShaderGLES4::USE_REINHARD_TONEMAPPER, env->tone_mapper == VS::ENV_TONE_MAPPER_REINHARD);
		state.tonemap_shader.set_conditional(TonemapShaderGLES4::USE_AUTO_EXPOSURE, env->auto_exposure);
		state.tonemap_shader.set_conditional(TonemapShaderGLES4::USE_GLOW_FILTER_BICUBIC, env->glow_bicubic_upscale);
	}

	state.tonemap_shader.set_conditional(TonemapShaderGLES4::KEEP_3D_LINEAR, storage->frame.current_rt->flags[RasterizerStorage::RENDER_TARGET_KEEP_3D_LINEAR]);
	state.tonemap_shader.set_conditional(TonemapShaderGLES4::USE_FXAA, storage->frame.current_rt->use_fxaa);
	state.tonemap_shader.set_conditional(TonemapShaderGLES4::USE_DEBANDING, storage->frame.current_rt->use_debanding);
	state.tonemap_shader.set_conditional(TonemapShaderGLES4::USE_SHARPENING, storage->frame.current_rt->sharpen_intensity >= 0.001);

	if (env && max_glow_level >= 0) {
		for (int i = 0; i < (max_glow_level + 1); i++) {
			if (glow_mask & (1 << i)) {
				if (i == 0) {
					state.tonemap_shader.set_conditional(TonemapShaderGLES4::USE_GLOW_LEVEL1, true);
				}
				if (i == 1) {
					state.tonemap_shader.set_conditional(TonemapShaderGLES4::USE_GLOW_LEVEL2, true);
				}
				if (i == 2) {
					state.tonemap_shader.set_conditional(TonemapShaderGLES4::USE_GLOW_LEVEL3, true);
				}
				if (i == 3) {
					state.tonemap_shader.set_conditional(TonemapShaderGLES4::USE_GLOW_LEVEL4, true);
				}
				if (i == 4) {
					state.tonemap_shader.set_conditional(TonemapShaderGLES4::USE_GLOW_LEVEL5, true);
				}
				if (i == 5) {
					state.tonemap_shader.set_conditional(TonemapShaderGLES4::USE_GLOW_LEVEL6, true);
				}
				if (i == 6) {
					state.tonemap_shader.set_conditional(TonemapShaderGLES4::USE_GLOW_LEVEL7, true);
				}
			}
		}

		state.tonemap_shader.set_conditional(TonemapShaderGLES4::USE_GLOW_SCREEN, env->glow_blend_mode == VS::GLOW_BLEND_MODE_SCREEN);
		state.tonemap_shader.set_conditional(TonemapShaderGLES4::USE_GLOW_SOFTLIGHT, env->glow_blend_mode == VS::GLOW_BLEND_MODE_SOFTLIGHT);
		state.tonemap_shader.set_conditional(TonemapShaderGLES4::USE_GLOW_REPLACE, env->glow_blend_mode == VS::GLOW_BLEND_MODE_REPLACE);
		WRAPPED_GL_ACTIVE_TEXTURE(GL_TEXTURE2);
		glBindTexture(GL_TEXTURE_2D, storage->frame.current_rt->effects.mip_maps[0].color);
	}

	if (env && env->adjustments_enabled) {
		state.tonemap_shader.set_conditional(TonemapShaderGLES4::USE_BCS, true);
		RasterizerStorageGLES4::Texture *tex = storage->texture_owner.getornull(env->color_correction);
		if (tex) {
			state.tonemap_shader.set_conditional(TonemapShaderGLES4::USE_COLOR_CORRECTION, true);
			WRAPPED_GL_ACTIVE_TEXTURE(GL_TEXTURE3);
			glBindTexture(tex->target, tex->tex_id);
		}
	}

	state.tonemap_shader.set_conditional(TonemapShaderGLES4::DISABLE_ALPHA, !storage->frame.current_rt->flags[RasterizerStorage::RENDER_TARGET_TRANSPARENT]);
	state.tonemap_shader.set_conditional(TonemapShaderGLES4::V_FLIP, storage->frame.current_rt->flags[RasterizerStorage::RENDER_TARGET_VFLIP]);
	state.tonemap_shader.bind();

	if (env) {
		state.tonemap_shader.set_uniform(TonemapShaderGLES4::EXPOSURE, env->tone_mapper_exposure);
		state.tonemap_shader.set_uniform(TonemapShaderGLES4::WHITE, env->tone_mapper_exposure_white);

		if (max_glow_level >= 0) {
			state.tonemap_shader.set_uniform(TonemapShaderGLES4::GLOW_INTENSITY, env->glow_intensity);
			int ss[2] = {
				storage->frame.current_rt->width,
				storage->frame.current_rt->height,
			};
			glUniform2iv(state.tonemap_shader.get_uniform(TonemapShaderGLES4::GLOW_TEXTURE_SIZE), 1, ss);
		}

		if (env->auto_exposure) {
			WRAPPED_GL_ACTIVE_TEXTURE(GL_TEXTURE1);
			glBindTexture(GL_TEXTURE_2D, storage->frame.current_rt->exposure.color);
			state.tonemap_shader.set_uniform(TonemapShaderGLES4::AUTO_EXPOSURE_GREY, env->auto_exposure_grey);
		}

		if (env->adjustments_enabled) {
			state.tonemap_shader.set_uniform(TonemapShaderGLES4::BCS, Vector3(env->adjustments_brightness, env->adjustments_contrast, env->adjustments_saturation));
		}
	} else {
		// No environment, so no exposure.
		state.tonemap_shader.set_uniform(TonemapShaderGLES4::EXPOSURE, 1.0);
	}

	if (storage->frame.current_rt->use_fxaa) {
		state.tonemap_shader.set_uniform(TonemapShaderGLES4::PIXEL_SIZE, Vector2(1.0 / storage->frame.current_rt->width, 1.0 / storage->frame.current_rt->height));
	}

	if (storage->frame.current_rt->sharpen_intensity >= 0.001) {
		state.tonemap_shader.set_uniform(TonemapShaderGLES4::SHARPEN_INTENSITY, storage->frame.current_rt->sharpen_intensity);
	}

	_copy_screen(true, true);

	//turn off everything used
	state.tonemap_shader.set_conditional(TonemapShaderGLES4::USE_FXAA, false);
	state.tonemap_shader.set_conditional(TonemapShaderGLES4::USE_DEBANDING, false);
	state.tonemap_shader.set_conditional(TonemapShaderGLES4::USE_SHARPENING, false);
	state.tonemap_shader.set_conditional(TonemapShaderGLES4::USE_AUTO_EXPOSURE, false);
	state.tonemap_shader.set_conditional(TonemapShaderGLES4::USE_FILMIC_TONEMAPPER, false);
	state.tonemap_shader.set_conditional(TonemapShaderGLES4::USE_ACES_TONEMAPPER, false);
	state.tonemap_shader.set_conditional(TonemapShaderGLES4::USE_ACES_FITTED_TONEMAPPER, false);
	state.tonemap_shader.set_conditional(TonemapShaderGLES4::USE_REINHARD_TONEMAPPER, false);
	state.tonemap_shader.set_conditional(TonemapShaderGLES4::USE_GLOW_LEVEL1, false);
	state.tonemap_shader.set_conditional(TonemapShaderGLES4::USE_GLOW_LEVEL2, false);
	state.tonemap_shader.set_conditional(TonemapShaderGLES4::USE_GLOW_LEVEL3, false);
	state.tonemap_shader.set_conditional(TonemapShaderGLES4::USE_GLOW_LEVEL4, false);
	state.tonemap_shader.set_conditional(TonemapShaderGLES4::USE_GLOW_LEVEL5, false);
	state.tonemap_shader.set_conditional(TonemapShaderGLES4::USE_GLOW_LEVEL6, false);
	state.tonemap_shader.set_conditional(TonemapShaderGLES4::USE_GLOW_LEVEL7, false);
	state.tonemap_shader.set_conditional(TonemapShaderGLES4::USE_GLOW_REPLACE, false);
	state.tonemap_shader.set_conditional(TonemapShaderGLES4::USE_GLOW_SCREEN, false);
	state.tonemap_shader.set_conditional(TonemapShaderGLES4::USE_GLOW_SOFTLIGHT, false);
	state.tonemap_shader.set_conditional(TonemapShaderGLES4::USE_GLOW_FILTER_BICUBIC, false);
	state.tonemap_shader.set_conditional(TonemapShaderGLES4::USE_BCS, false);
	state.tonemap_shader.set_conditional(TonemapShaderGLES4::USE_COLOR_CORRECTION, false);
	state.tonemap_shader.set_conditional(TonemapShaderGLES4::V_FLIP, false);
	state.tonemap_shader.set_conditional(TonemapShaderGLES4::DISABLE_ALPHA, false);
}

bool RasterizerSceneGLES4::_element_needs_directional_add(RenderList::Element *e) {
...
void RasterizerSceneGLES4::render_scene(const Transform &p_cam_transform, const CameraMatrix &p_cam_projection, const int p_eye, bool p_cam_ortogonal, InstanceBase **p_cull_result, int p_cull_count, RID *p_light_cull_result, int p_light_cull_count, RID *p_reflection_probe_cull_result, int p_reflection_probe_cull_count, RID p_environment, RID p_shadow_atlas, RID p_reflection_atlas, RID p_reflection_probe, int p_reflection_probe_pass) {
	...
	state.scene_shader.set_conditional(SceneShaderGLES4::USE_SHADOW, use_shadows);
	...
	if (use_depth_prepass) {
		...
		state.scene_shader.set_conditional(SceneShaderGLES4::RENDER_DEPTH, true);
		...
		state.scene_shader.set_conditional(SceneShaderGLES4::RENDER_DEPTH, false);
	...
	if (probe) {
		...
		use_mrt = false;
		state.scene_shader.set_conditional(SceneShaderGLES4::USE_MULTIPLE_RENDER_TARGETS, false);
		...
		if (use_mrt) {
			...
			glBindFramebuffer(GL_FRAMEBUFFER, storage->frame.current_rt->buffers.fbo);
			state.scene_shader.set_conditional(SceneShaderGLES4::USE_MULTIPLE_RENDER_TARGETS, true);
	...
	RasterizerStorageGLES4::Sky *sky = nullptr;
	Ref<CameraFeed> feed;
	...
	if (env) {
		switch (bg_mode) {
			...
			case VS::ENV_BG_CANVAS:
				...
				storage->shaders.copy.set_conditional(CopyShaderGLES4::SRGB_TO_LINEAR, true);
				...
				//turn off everything used
				storage->shaders.copy.set_conditional(CopyShaderGLES4::SRGB_TO_LINEAR, false);
				storage->shaders.copy.set_conditional(CopyShaderGLES4::DISABLE_ALPHA, false);
			...
			case VS::ENV_BG_CAMERA_FEED:
				if (feed.is_valid() && (feed->get_base_width() > 0) && (feed->get_base_height() > 0)) {
					...
					storage->shaders.copy.set_conditional(CopyShaderGLES4::USE_DISPLAY_TRANSFORM, true);
					storage->shaders.copy.set_conditional(CopyShaderGLES4::DISABLE_ALPHA, true);
					storage->shaders.copy.set_conditional(CopyShaderGLES4::SRGB_TO_LINEAR, true);
					...
					if (feed->get_datatype() == CameraFeed::FEED_RGB) {
						...
						storage->shaders.copy.set_conditional(CopyShaderGLES4::YCBCR_TO_SRGB, true);
						...
						storage->shaders.copy.set_conditional(CopyShaderGLES4::SEP_CBCR_TEXTURE, true);
						storage->shaders.copy.set_conditional(CopyShaderGLES4::YCBCR_TO_SRGB, true);
					};

					storage->shaders.copy.bind();
					storage->shaders.copy.set_uniform(CopyShaderGLES4::DISPLAY_TRANSFORM, feed->get_transform());

					_copy_screen(true, true);

					//turn off everything used
					storage->shaders.copy.set_conditional(CopyShaderGLES4::USE_DISPLAY_TRANSFORM, false);
					storage->shaders.copy.set_conditional(CopyShaderGLES4::DISABLE_ALPHA, false);
					storage->shaders.copy.set_conditional(CopyShaderGLES4::SRGB_TO_LINEAR, false);
					storage->shaders.copy.set_conditional(CopyShaderGLES4::SEP_CBCR_TEXTURE, false);
					storage->shaders.copy.set_conditional(CopyShaderGLES4::YCBCR_TO_SRGB, false);
	...
	//state.scene_shader.set_conditional( SceneShaderGLES4::USE_FOG,false);
...
void RasterizerSceneGLES4::render_shadow(RID p_light, RID p_shadow_atlas, int p_pass, InstanceBase **p_cull_result, int p_cull_count) {
	render_pass++;
	...
	ERR_FAIL_COND(!light_instance);
	RasterizerStorageGLES4::Light *light = storage->light_owner.getornull(light_instance->light);
	ERR_FAIL_COND(!light);
	...
	if (light->type == VS::LIGHT_DIRECTIONAL) {
		...
		if (light->type == VS::LIGHT_OMNI) {
			if (light->omni_shadow_mode == VS::LIGHT_OMNI_SHADOW_CUBE) {
				...
				state.scene_shader.set_conditional(SceneShaderGLES4::RENDER_DEPTH_DUAL_PARABOLOID, true);
			}
	...
	state.scene_shader.set_conditional(SceneShaderGLES4::RENDER_DEPTH, false);
	state.scene_shader.set_conditional(SceneShaderGLES4::RENDER_DEPTH_DUAL_PARABOLOID, false);
	...
	if (light->type == VS::LIGHT_OMNI && light->omni_shadow_mode == VS::LIGHT_OMNI_SHADOW_CUBE && p_pass == 5) {
		...
		for (int i = 0; i < 2; i++) {
			state.cube_to_dp_shader.set_uniform(CubeToDpShaderGLES4::Z_FLIP, i == 1);
			state.cube_to_dp_shader.set_uniform(CubeToDpShaderGLES4::Z_NEAR, light_projection.get_z_near());
			state.cube_to_dp_shader.set_uniform(CubeToDpShaderGLES4::Z_FAR, light_projection.get_z_far());
			state.cube_to_dp_shader.set_uniform(CubeToDpShaderGLES4::BIAS, light->param[VS::LIGHT_PARAM_SHADOW_BIAS]);
...
void RasterizerSceneGLES4::set_scene_pass(uint64_t p_pass) {
	scene_pass = p_pass;
}

bool RasterizerSceneGLES4::free(RID p_rid) {
...
void RasterizerSceneGLES4::set_debug_draw_mode(VS::ViewportDebugDraw p_debug_draw) {
	state.debug_draw = p_debug_draw;
}

void RasterizerSceneGLES4::initialize() {
	render_pass = 0;
...

	{
		...
		// blocks in hardware (although this is probably not taken advantage of in GnuChan 3.x currently).

		// This logic is thus a compromise.
		int max_ubo_size = RasterizerStorageGLES4::safe_gl_get_integer(GL_MAX_UNIFORM_BLOCK_SIZE);
	...
	while (max_exposure_shrink_size > 0) {
		RasterizerStorageGLES4::RenderTarget::Exposure e;
...
void RasterizerSceneGLES4::iteration() {
	...
	state.scene_shader.set_conditional(SceneShaderGLES4::USE_LIGHTMAP_FILTER_BICUBIC, storage->config.use_lightmap_filter_bicubic);
	state.scene_shader.set_conditional(SceneShaderGLES4::VCT_QUALITY_HIGH, GLOBAL_GET("rendering/quality/voxel_cone_tracing/high_quality"));
}

void RasterizerSceneGLES4::finalize() {
}

RasterizerSceneGLES4::RasterizerSceneGLES4() {
