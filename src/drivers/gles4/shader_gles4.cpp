...
#include "drivers/gles4/rasterizer_storage_gles4.h"
#include "drivers/gles4/shader_cache_gles4.h"
...
ShaderGLES4 *ShaderGLES4::active = nullptr;
SelfList<ShaderGLES4::Version>::List ShaderGLES4::versions_compiling;

ShaderCacheGLES4 *ShaderGLES4::shader_cache;
ThreadedCallableQueue<GLuint> *ShaderGLES4::cache_write_queue;

ThreadedCallableQueue<GLuint> *ShaderGLES4::compile_queue;
bool ShaderGLES4::parallel_compile_supported;

bool ShaderGLES4::async_hidden_forbidden;
uint32_t *ShaderGLES4::compiles_started_this_frame;
uint32_t *ShaderGLES4::max_frame_compiles_in_progress;
uint32_t ShaderGLES4::max_simultaneous_compiles;
uint32_t ShaderGLES4::active_compiles_count;
#ifdef DEBUG_ENABLED
bool ShaderGLES4::log_active_async_compiles_count;
#endif

uint64_t ShaderGLES4::current_frame;
...
GLint ShaderGLES4::get_uniform_location(int p_index) const {
	ERR_FAIL_COND_V(!version, -1);

	return version->uniform_location[p_index];
}

bool ShaderGLES4::bind() {
	return _bind(false);
}

bool ShaderGLES4::_bind(bool p_binding_fallback) {
...
bool ShaderGLES4::is_custom_code_ready_for_render(uint32_t p_code_id) {
	if (p_code_id == 0) {
		return true;
	}
...
bool ShaderGLES4::_bind_ubershader(bool p_for_warmup) {
#ifdef DEBUG_ENABLED
...
void ShaderGLES4::advance_async_shaders_compilation() {
	SelfList<ShaderGLES4::Version> *curr = versions_compiling.first();
	while (curr) {
		SelfList<ShaderGLES4::Version> *next = curr->next();

		ShaderGLES4::Version *v = curr->self();
...
void ShaderGLES4::_log_active_compiles() {
#ifdef DEBUG_ENABLED
...
bool ShaderGLES4::_process_program_state(Version *p_version, bool p_async_forbidden) {
	bool ready = false;
...
// Cache - Binary ready
ShaderGLES4::Version *ShaderGLES4::get_current_version(bool &r_async_forbidden) {
	VersionKey effective_version;
	...
	// GLES4 always uses OpenGL 4.3 Core Profile
	strings_common.push_back("#version 430 core\n");
	...
	if (shader_cache && effective_version.is_subject_to_caching()) {
		...
		v.program_binary.cache_hash = ShaderCacheGLES4::hash_program(strings_platform, strings_vertex, strings_fragment);
...
void ShaderGLES4::_set_source(Version::Ids p_ids, const LocalVector<const char *> &p_vertex_strings, const LocalVector<const char *> &p_fragment_strings) const {
...
bool ShaderGLES4::_complete_compile(Version::Ids p_ids, bool p_retrievable) const {
	GLint status;
...
bool ShaderGLES4::_complete_link(Version::Ids p_ids, GLenum *r_program_format, PoolByteArray *r_program_binary) const {
	GLint status;
...
void ShaderGLES4::_setup_uniforms(CustomCode *p_cc) const {
	//print_line("uniforms:  ");
...
void ShaderGLES4::_dispose_program(Version *p_version) {
	if (compile_queue) {
...
void ShaderGLES4::setup(const char **p_conditional_defines, int p_conditional_count, const char **p_uniform_names, int p_uniform_count, const AttributePair *p_attribute_pairs, int p_attribute_count, const TexUnitPair *p_texunit_pairs, int p_texunit_pair_count, const UBOPair *p_ubo_pairs, int p_ubo_pair_count, const Feedback *p_feedback, int p_feedback_count, const char *p_vertex_code, const char *p_fragment_code, int p_vertex_code_start, int p_fragment_code_start) {
	...
	max_image_units = RasterizerStorageGLES4::safe_gl_get_integer(GL_MAX_TEXTURE_IMAGE_UNITS, RasterizerStorageGLES4::Config::max_desired_texture_image_units);
}

void ShaderGLES4::init_async_compilation() {
	if (is_async_compilation_supported() && get_ubershader_flags_uniform() != -1) {
...
uint32_t ShaderGLES4::create_custom_shader() {
	custom_code_map[last_custom_code] = CustomCode();
	custom_code_map[last_custom_code].version = 1;
	return last_custom_code++;
}

void ShaderGLES4::set_custom_shader_code(uint32_t p_code_id, const String &p_vertex, const String &p_vertex_globals, const String &p_fragment, const String &p_light, const String &p_fragment_globals, const String &p_uniforms, const Vector<StringName> &p_texture_uniforms, const Vector<CharString> &p_custom_defines, AsyncMode p_async_mode) {
...
void ShaderGLES4::set_custom_shader(uint32_t p_code_id) {
	new_conditional_version.code_version = p_code_id;
}

void ShaderGLES4::free_custom_shader(uint32_t p_code_id) {
	ERR_FAIL_COND(!custom_code_map.has(p_code_id));
...
void ShaderGLES4::set_base_material_tex_index(int p_idx) {
	base_material_tex_index = p_idx;
}

ShaderGLES4::ShaderGLES4() {
	version = nullptr;
	last_custom_code = 1;
