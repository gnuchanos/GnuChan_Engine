...
void OS_Windows::initialize_core() {
	...
	// Add current GnuChan PID to the list of known PIDs
...
// strtoull on mingw
	...
	bool gles4_context = (p_video_driver == VIDEO_DRIVER_GLES4);

	bool editor = Engine::get_singleton()->is_editor_hint();

	gl_context = NULL;
	while (!gl_context) {
		gl_context = memnew(ContextGL_Windows(hWnd, gles4_context));
...
uint32_t OS_Windows::keyboard_get_scancode_from_physical(uint32_t p_scancode) const {
	...
	if (char_code >= 32 && char_code <= 127) {
		// GnuChan uses 'braces' instead of 'brackets'
