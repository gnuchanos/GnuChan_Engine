...
// maybe contextgl wants to be in charge of creating the window
	...
	if (getenv("DRI_PRIME") == nullptr) {
		...
		if (use_prime) {
			...
			print_line("Note: Set DRI_PRIME=0 in the environment to disable GnuChan from using the discrete GPU.");
			setenv("DRI_PRIME", "1", 1);
		}
	}

	ContextGL_X11::ContextType opengl_api_type = ContextGL_X11::GLES_4_0_COMPATIBLE;
	...
	while (true) {
		if (RasterizerGLES4::is_viable() == OK) {
			RasterizerGLES4::register_config();
			RasterizerGLES4::make_current();
			break;
		} else {
			gl_initialization_error = true;
			break;
	...
	if (gl_initialization_error) {
		OS::get_singleton()->alert("Your video card driver does not support any of the supported OpenGL versions.\n"
								   ...
								   "Alternatively, you can force software rendering by running GnuChan with the `LIBGL_ALWAYS_SOFTWARE=1`\n"
	...
	XStoreName(x11_display, x11_window, "GnuChan");
					...
					case XI_TouchEnd: {
						...
						if (is_begin) {
							...
							if (xi.state.size() == 1) {
								...
								// in a spurious mouse motion event being sent to GnuChan; remember it to be able to filter it out
