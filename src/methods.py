...
# Get the project root folder name ahead of time
...
def get_version_info(module_version_string="", silent=False):
    ...
    if os.getenv("GNCHAN_VERSION_STATUS") != None:
        version_info["status"] = str(os.getenv("GNCHAN_VERSION_STATUS"))
        if not silent:
            print(
...
#define VERSION_DOCS_URL "https://github.com/gnuchanos/GnuChan_Engine/wiki/" VERSION_DOCS_BRANCH
...
def detect_modules(search_path, recursive=False):
    ...
    def is_engine(path):
        ...
        # GnuChan sources when using `custom_modules` build option.
        version_path = os.path.join(path, "version.py")
        if os.path.exists(version_path):
            with open(version_path) as f:
                if 'short_name = "gnuchan"' in f.read():
                    return True
        return False
...
def detect_visual_c_compiler_version(tools_env):
    ...
    # There are other architectures, but GnuChan does not support them currently, so this function does not detect arm/amd64_arm
...
def generate_vs_project(env, num_jobs):
    ...
    if batch_file:
        ...
        class ModuleConfigs(Mapping):
            ...
            def add_mode(
                ...
                self.arg_dict["runfile"] += [
                    f'bin\\gnuchan.windows.{config_id}.{plat_id}{f".{name}" if name else ""}.exe'
        ...
        env.MSVSProject(
            target=["#gnuchan" + env["MSVSPROJECTSUFFIX"]],
            incs=env.vs_incs,
            srcs=env.vs_srcs,
