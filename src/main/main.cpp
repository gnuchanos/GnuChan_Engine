...
void initialize_physics() {
	...
	GLOBAL_DEF("physics/3d/physics_engine", "DEFAULT");
	GLOBAL_DEF("physics/3d/gnuchan_physics/use_bvh", true);
	GLOBAL_DEF("physics/3d/gnuchan_physics/bvh_collision_margin", 0.1);
	ProjectSettings::get_singleton()->set_custom_property_info("physics/3d/gnuchan_physics/bvh_collision_margin", PropertyInfo(Variant::REAL, "physics/3d/gnuchan_physics/bvh_collision_margin", PROPERTY_HINT_RANGE, "0.0,2.0,0.01"));
...
void Main::print_help(const char *p_binary) {
	...
	OS::get_singleton()->print("Free and open source software under the terms of the GNU General Public License v3.0 or later.\n");
	OS::get_singleton()->print("(c) 2025 GnuChan Engine contributors.\n");
	OS::get_singleton()->print("\n");
	OS::get_singleton()->print("Usage: %s [options] [path to scene or 'project.gnuchan' file]\n", p_binary);
	OS::get_singleton()->print("\n");
	...
	OS::get_singleton()->print("  --path <directory>               Path to a project (<directory> must contain a 'project.gnuchan' file).\n");
	OS::get_singleton()->print("  -u, --upwards                    Scan folders upwards for project.gnuchan file.\n");
	...
	OS::get_singleton()->print("  --debug-shader-fallbacks         Use the fallbacks of the shaders which have one when running the scene (OpenGL 4.3 only).\n");
#endif
	...
	OS::get_singleton()->print("  --gdnative-generate-json-api     Generate JSON dump of the GnuChan API for GDNative bindings.\n");
#endif
...
Error Main::setup(const char *execpath, int argc, char *argv[], bool p_second_phase) {
	...
	GLOBAL_DEF("debug/settings/crash_handler/message.editor",
			String("Please include this when reporting the bug on: https://github.com/gnuchanos/GnuChan_Engine/issues"));
	...
	while (I) {
		...
		// Otherwise, GnuChan would try to open a non-existent project on the first start and abort.
		...
		if (I->get() == "-h" || I->get() == "--help" || I->get() == "/?") { // display help
			...
			auto_quit = true;
		} else if (I->get().ends_with("project.gnuchan") || I->get().ends_with("project.godot")) {
			String path;
			String file = I->get();
	...
	GLOBAL_DEF("logging/file_logging/log_path", "user://logs/gnuchan.log");
	...
	GLOBAL_DEF_RST("rendering/quality/driver/driver_name", "opengl4_3");
	ProjectSettings::get_singleton()->set_custom_property_info("rendering/quality/driver/driver_name", PropertyInfo(Variant::STRING, "rendering/quality/driver/driver_name", PROPERTY_HINT_ENUM, "opengl4_3"));
	if (video_driver == "") {
	...
	if (!Engine::get_singleton()->is_editor_hint()) {
			GLOBAL_DEF("rendering/gles4/shaders/debug_shader_fallbacks", debug_shader_fallbacks);
		ProjectSettings::get_singleton()->set_hide_from_editor("rendering/gles4/shaders/debug_shader_fallbacks", true);
	}
#endif
...
Error Main::setup2(Thread::ID p_main_tid_override) {
	...
	// Ensure physics_engine property exists BEFORE modules register (which trigger PhysicsServerManager::on_servers_changed)
	GLOBAL_DEF("physics/3d/physics_engine", "DEFAULT");
...
bool Main::start() {
	...
	for (int i = 0; i < args.size(); i++) {
		...
		if (args[i] == "--check-only") {
			...
			if (args[i].ends_with(".scn") ||
				...
				// a file extension associated with GnuChan scenes. This makes it possible
	...
	if (doc_tool_path != "") {
		...
		// English-only: no doc translations loaded.
