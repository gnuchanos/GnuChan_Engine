...
/*
 * Based on GnuChan's patched VHACD-version of Bullet's btConvexHullComputer.
 ...
 * - adapted to GnuChan's code style
 * - replaced Bullet's types (e.g. vectors) with GnuChan's
...
Error ConvexHullComputer::convex_hull(const Vector3 *p_points, int32_t p_point_count, Geometry::MeshData &r_mesh) {
	...
	for (uint32_t i = 0; i < ch.faces.size(); i++) {
		...
		// reverse indices: GnuChan wants clockwise, but this is counter-clockwise
