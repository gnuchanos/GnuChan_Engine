...
void MergeGroup::_delete_node(Node *p_node) {
	p_node->queue_delete();
	// This is only a problem in the editor, GnuChan saving code cannot currently deal with
	...
	// calling of notifications (GnuChan doesn't deal well with large numbers of nodes).
#ifdef TOOLS_ENABLED
