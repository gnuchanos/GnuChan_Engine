	...
	String _test_path() {
		...
		if (mode == MODE_IMPORT || mode == MODE_RENAME) {
			if (valid_path != "" && !d->file_exists("project.godot")) {
				...
				set_message(TTR("This directory already contains a GnuChan project."), MESSAGE_ERROR, INSTALL_PATH);
				memdelete(d);
				get_ok()->set_disabled(true);
				return "";
			}
		...
		if (mode == MODE_RENAME) {
			...
			if (mode == MODE_IMPORT) {
				...
				if (mode == MODE_NEW) {
					...
					initial_settings["rendering/quality/driver/driver_name"] = "opengl4_3";
...
public:
	...
	ProjectDialog() {
		...
		// Use OpenGL 4.3 by default (only option).
		Container *rvb = memnew(VBoxContainer);
		rvb->set_h_size_flags(SIZE_EXPAND_FILL);
		rshb->add_child(rvb);
		Button *rs_button = memnew(CheckBox);
		rs_button->set_button_group(rasterizer_button_group);
		rs_button->set_text(TTR("OpenGL 4.3 Core Profile"));
		rs_button->set_meta("driver_name", "opengl4_3");
		rs_button->set_pressed(true);
		rs_button->set_disabled(true);
		rvb->add_child(rs_button);
		l = memnew(Label);
		l->set_text(TTR("Modern rendering\nAll features available\nRequires OpenGL 4.3+ capable GPU"));
		rvb->add_child(l);
...
void ProjectList::load_project_data(const String &p_property_key, Item &p_item, bool p_favorite) {
	...
	if (config_version > ProjectSettings::CONFIG_VERSION) {
		// Comes from an incompatible (more recent) GnuChan version, grey it out
		grayed = true;
	}
...
void ProjectList::load_projects() {
	...
	for (List<PropertyInfo>::Element *E = properties.front(); E; E = E->next()) {
		// This is actually something like "projects/C:::Documents::GnuChan::Projects::MyGame"
		String property_key = E->get().name;
...
void ProjectManager::_open_selected_projects_ask() {
	...
	if (config_version == 0) {
		ask_update_settings->set_text(vformat(TTR("The following project settings file does not specify the version of GnuChan through which it was created.\n\n%s\n\nIf you proceed with opening it, it will be converted to GnuChan's current configuration file format.\nWarning: You won't be able to open the project with previous versions of the engine anymore."), conf));
	...
	if (folders_set.size() > 0) {
		...
		if (confirm) {
			...
			multi_scan_ask->set_text(
					vformat(TTR("Are you sure to scan %s folders for existing GnuChan projects?\nThis could take a while."), folders.size()));
...
ProjectManager::ProjectManager() {
	...
	String cp;
	cp += 0xA9;
	// TRANSLATORS: This refers to the application where users manage their GnuChan projects.
