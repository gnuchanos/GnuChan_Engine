...
Error EditorRun::run(const String &p_scene, const String &p_custom_args, const List<String> &p_breakpoints, const bool &p_skip_breakpoints) {
	...
	if (p_custom_args != "") {
		...
		// In this case, GnuChan will no longer be run directly; it's up to the underlying command
		...
		if (placeholder_pos != -1) {
			...
			if (exec_args.size() >= 1) {
				...
				// Append the GnuChan executable name before we append executable arguments
			...
			// Append GnuChan-specific custom arguments.
