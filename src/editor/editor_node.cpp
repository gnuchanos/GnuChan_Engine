...
void EditorNode::_scan_external_changes() {
	...
	String project_settings_path = ProjectSettings::get_singleton()->get_resource_path().plus_file("project.gnuchan");
	if (FileAccess::get_modified_time(project_settings_path) > ProjectSettings::get_singleton()->get_last_saved_time()) {
		TreeItem *ti = disk_changed_list->create_item(r);
		ti->set_text(0, "project.gnuchan");
		need_reload = true;
	}
...
EditorNode::EditorNode() {
	...
	p->set_item_tooltip(
			p->get_item_count() - 1,
			TTR("When this option is enabled, using one-click deploy will only export an executable without the project data.\nThe filesystem will be packed and sent over the network on each run."));
