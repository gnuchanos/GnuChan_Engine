...
namespace GodotTools.IdeMessaging
{
    ...
    public sealed class Client : IDisposable
    {
        ...
        private async Task ConnectToServer()
        {
            ...
            try
            {
                logger.LogInfo("Connecting to GnuChan Ide Server");

                await tcpClient.ConnectAsync(IPAddress.Loopback, godotIdeMetadata.Port);

                logger.LogInfo("Connection open with GnuChan Ide Server");
            ...
            catch (SocketException e)
            {
                if (e.SocketErrorCode == SocketError.ConnectionRefused)
                    logger.LogError("The connection to the GnuChan Ide Server was refused");
                else
                    throw;
            }
        }
        ...
        public async void Start()
        {
            ...
            using (await connectionSem.UseAsync())
            {
                ...
                if (!File.Exists(MetaFilePath))
                {
                    logger.LogInfo("There is no GnuChan Ide Server running");
                    return;
                }
                ...
                {
                    logger.LogError("Failed to read GnuChan Ide metadata file");
                }
            }
        }
            ...
            where TResponse : Response, new()
        {
            if (!IsConnected)
            {
                logger.LogError("Cannot write request. Not connected to the GnuChan Ide Server.");
                return null;
            }
