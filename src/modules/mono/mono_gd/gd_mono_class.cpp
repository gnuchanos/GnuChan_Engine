...
void GDMonoClass::fetch_methods_with_godot_api_checks(GDMonoClass *p_native_base) {
	...
	while ((raw_method = mono_class_get_methods(get_mono_ptr(), &iter)) != NULL) {
		...
		if (method->get_name() != name) {
			...
			WARN_PRINT("Method '" + fullname + "' is hidden by GnuChan API method. Should be '" +
		...
		// Virtual method of GnuChan Object derived type, let's try to find GodotMethod attribute
...
GDMonoClass::~GDMonoClass() {
	...
	{
		// Ugly workaround...
		// We may have duplicated values, because we redirect snake_case methods to PascalCasel (only GnuChan API methods).
