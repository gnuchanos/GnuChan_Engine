...
Node *EditorSceneImporterFBX::import_scene(const String &p_path, uint32_t p_flags, int p_bake_fps, uint32_t p_compress_flags,
...

	{
		...
		if (doc.IsSafeToImport()) {
			...
			ERR_PRINT(vformat("Cannot import FBX file: %s. It uses file format %d which is unsupported by GnuChan. Please re-export it or convert it to a newer format.", p_path, doc.FBXVersion()));
		}
	}
