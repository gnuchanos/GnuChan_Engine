#ifndef JOLT_PHYSICS_SERVER_H
#define JOLT_PHYSICS_SERVER_H

// Jolt.h MUST be included before any Godot headers, to define JPH macros and
// avoid Godot's global Ref<T> conflicting with JPH::Ref<T>
#include <Jolt/Jolt.h>
...
class JoltPhysicsServer : public PhysicsServer {
	static JoltPhysicsServer *singleton;

	mutable RID_Owner<JoltShapeData> shape_owner;
	mutable RID_Owner<JoltSpaceData> space_owner;
	mutable RID_Owner<JoltAreaData> area_owner;
	mutable RID_Owner<JoltBodyData> body_owner;
	mutable RID_Owner<JoltBodyData> soft_body_owner;
	mutable RID_Owner<JoltJointData> joint_owner;

	JoltSpace space_state_instance;
	JoltSpace *direct_state;
	bool flushing_queries_flag;
	int collision_iterations;

public:
	static PhysicsServer *create_callback();
	...
	virtual bool body_test_motion(RID p_body, const Transform &p_from, const Vector3 &p_motion, bool p_infinite_inertia, MotionResult *r_result = nullptr, bool p_exclude_raycast_shapes = true, const Set<RID> &p_exclude = Set<RID>()) override;
	...
	virtual void generic_6dof_joint_set_param(RID p_joint, Vector3::Axis, G6DOFJointAxisParam p_param, float p_value) override;
	virtual float generic_6dof_joint_get_param(RID p_joint, Vector3::Axis, G6DOFJointAxisParam p_param) override;
	...
	virtual void step(float p_step) override;
