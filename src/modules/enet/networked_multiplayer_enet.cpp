...
Error NetworkedMultiplayerENet::create_client(const String &p_address, int p_port, int p_in_bandwidth, int p_out_bandwidth, int p_client_port) {
	...
	if (!ip.is_ipv4()) {
		enet_host_destroy(host);
		ERR_FAIL_V_MSG(ERR_INVALID_PARAMETER, "Connecting to an IPv6 server isn't supported when using vanilla ENet. Recompile GnuChan with the bundled ENet library.");
	}
